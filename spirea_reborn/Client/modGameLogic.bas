...
Dim rec_back As RECT
    ...
    Do While InGame
        ...
        Dim Bars As Long
        If GetTickCount > Bars + 500 Then
            Call SendData("getnpchp" & END_CHAR)
            Bars = GetTickCount
        End If
...
Dim Color As Long
    ...
    ' Draw name
    TextX = GetPlayerX(Index) * PIC_X + Player(Index).XOffset + Int(PIC_X / 2) - ((Len(GetPlayerName(Index)) / 3) * 8)
...
DC = DD_TileSurf.GetDC
    Call BitBlt(frmMapEditor.picBackSelect.hDC, 0, 0, DDSD_Tile.lWidth, DDSD_Tile.lHeight, DC, 0, 0, vbSrcCopy)
...
If frmMapEditor.optMask2.Value = True Then .Mask2 = EditorTileY * 7 + EditorTileX
If frmMapEditor.optM2Anim.Value = True Then .M2Anim = EditorTileY * 7 + EditorTileX
If frmMapEditor.optFringe.Value = True Then .Fringe = EditorTileY * 7 + EditorTileX
If frmMapEditor.optFAnim.Value = True Then .FAnim = EditorTileY * 7 + EditorTileX
If frmMapEditor.optFringe2.Value = True Then .Fringe2 = EditorTileY * 7 + EditorTileX
If frmMapEditor.optF2Anim.Value = True Then .F2Anim = EditorTileY * 7 + EditorTileX
                End With
            Else
                With Map.Tile(x1, y1)
