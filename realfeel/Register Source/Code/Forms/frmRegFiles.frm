...
Begin VB.Form frmRegFiles 
   ...
   Caption         =   "Dual Solace Registering Service"
   ClientHeight    =   2085
   ClientLeft      =   45
   ClientTop       =   315
   ClientWidth     =   4500
   ...
   Begin MSComctlLib.ProgressBar pbRegProgress 
      ...
      Appearance      =   1
   End
   Begin VB.Label lblRegFileStatus 
      Alignment       =   2  'Center
      Caption         =   "REG FILE STATUS"
      Height          =   255
      Left            =   120
      TabIndex        =   6
      Top             =   1800
...
For n = 0 To frmRegFiles.lstRegFiles.ListCount - 1
        ...
        DoEvents
        Call MakeRegBatch(FileName, FileExt, Ext_Path)
        frmRegFiles.lblRegStatus.Caption = "Executing " & FileName & " batch file..."
        DoEvents
        Call RunRegBatch(FileName, Ext_Path, True)
...
' Raise the form up
frmRegFiles.Height = 2445

' Clear the loading info
frmRegFiles.lblRegStatus.Caption = ""
frmRegFiles.pbRegProgress.Value = 0
End Sub

Public Sub lstRegFiles_Click()
Dim Info As String
' Load up the information
frmRegFiles.lblRegName.Caption = frmRegFiles.lstRegFiles.List(frmRegFiles.lstRegFiles.ListIndex)

If RegFiles(frmRegFiles.lstRegFiles.ListIndex + 1) = True Then
    frmRegFiles.lblRegFileStatus.ForeColor = &H8000&
    frmRegFiles.lblRegFileStatus.Caption = "REGISTERED"
Else
    frmRegFiles.lblRegFileStatus.ForeColor = &HC0&
    frmRegFiles.lblRegFileStatus.Caption = "UNREGISTERED"
End If

If Not Exists(App.Path & "\exts.ini") Then
...
End Select
Else
    Info = Read(App.Path & "\exts.ini", UCase$(frmRegFiles.lstRegFiles.List(frmRegFiles.lstRegFiles.ListIndex)) & "=")
    If Info <> Chr(215) Then
        frmRegFiles.lblRegDesc.Caption = Info
    Else
        frmRegFiles.lblRegDesc.Caption = "No file description found!"
    End If
End If
