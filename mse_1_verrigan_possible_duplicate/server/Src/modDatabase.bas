...
Dim ClassName() As Byte
    ...
    For i = 0 To Max_Classes
        TmpStr = GetVar(FileName, "CLASS" & i, "Name")
        ClassName = StrConv(TmpStr, vbFromUnicode)
        ReDim Preserve ClassName(NAME_LENGTH - 1)
        Call FillMemory(ClassName(Len(TmpStr)), NAME_LENGTH - Len(TmpStr), 32)
        Call CopyMemory(Class(i).ClassName(0), ClassName(0), NAME_LENGTH)
...
Dim i As Long
    ...
    For i = 0 To Max_Classes
        Call PutVar(FileName, "CLASS" & i, "Name", Trim(StrConv(Class(i).ClassName, vbUnicode)))
...
Dim FileName As String
    ...
    Call PutVar(FileName, "ITEM" & ItemNum, "Name", Trim(StrConv(Item(ItemNum).ItemName, vbUnicode)))
...
Dim TmpStr As String
    ...
    For i = 1 To MAX_ITEMS
        TmpStr = GetVar(FileName, "ITEM" & i, "Name")
        Buffer = StrConv(TmpStr, vbFromUnicode)
        If Len(TmpStr) < NAME_LENGTH Then
          ReDim Preserve Buffer(NAME_LENGTH - 1)
          Call FillMemory(Buffer(Len(TmpStr)), NAME_LENGTH - Len(TmpStr), 32)
        End If
        Call CopyMemory(Item(i).ItemName(0), Buffer(0), NAME_LENGTH)
...
Dim MapName() As Byte
    ...
    For i = 1 To MAX_MAPS
        ...
        ' Convert
        MapName = StrConv(OldMap.Name, vbFromUnicode)
        Call CopyMemory(NewMap.MapName(0), MapName(0), NAME_LENGTH)
