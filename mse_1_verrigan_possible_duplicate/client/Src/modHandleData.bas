...
Dim Buffer() As Byte
    ...
    If LCase(Parse(0)) = "newcharclasses" Then
        ...
        For i = 0 To Max_Classes
            Buffer = StrConv(Parse(n), vbFromUnicode)
            ReDim Preserve Buffer(NAME_LENGTH - 1)
            If NAME_LENGTH - Len(Parse(n)) > 0 Then
              Call FillMemory(Buffer(Len(Parse(n))), NAME_LENGTH - Len(Parse(n)), 32)
            End If
            Call CopyMemory(Class(i).ClassName(0), Buffer(0), NAME_LENGTH)
        ...
        For i = 0 To Max_Classes
            frmNewChar.cmbClass.AddItem Trim(StrConv(Class(i).ClassName, vbUnicode))
        Next i
    ...
    If (LCase(Parse(0)) = "checkformap") Then
        ...
        Buffer = ""
        Buffer = AddByteToBuffer(Buffer, CByte(i))
        
        Call SendDataNew(Buffer, SMsgNeedMap)
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "mapdata" Then
        ...
        Buffer = StrConv(Parse(n + 1), vbFromUnicode)
        ReDim Preserve Buffer(NAME_LENGTH - 1)
        Call FillMemory(Buffer(Len(Parse(n + 1))), NAME_LENGTH - Len(Parse(n + 1)), 32)
        Call CopyMemory(SaveMap.MapName(0), Buffer(0), NAME_LENGTH)
