...
Dim rec_back As RECT
    ...
    Do While InGame
        ...
        If Map.Moral = MAP_MORAL_NONE Then
            Call DrawText(TexthDC, Int((MAX_MAPX + 1) * PIC_X / 2) - (Int(Len(Trim(StrConv(Map.MapName, vbUnicode))) / 2) * 8), 1, Trim(StrConv(Map.MapName, vbUnicode)), QBColor(BrightRed))
        Else
            Call DrawText(TexthDC, Int((MAX_MAPX + 1) * PIC_X / 2) - (Int(Len(Trim(StrConv(Map.MapName, vbUnicode))) / 2) * 8), 1, Trim(StrConv(Map.MapName, vbUnicode)), QBColor(White))
        End If
...
Dim Buffer() As Byte
    ...
    If (KeyAscii = vbKeyReturn) Then
        ...
        If LCase(Mid(MyText, 1, 5)) = "/info" Then
            ...
            Buffer = ""
            Buffer = AddStringToBuffer(Buffer, ChatText)
            
            'Call SendData("playerinforequest" & SEP_CHAR & ChatText & SEP_CHAR & END_CHAR)
            Call SendDataNew(Buffer, SMsgPlayerInfoRequest)
            MyText = ""
            Exit Sub
        End If
        ...
        If LCase(Mid(MyText, 1, 6)) = "/stats" Then
            ...
            Call SendDataNew(Buffer, SMsgGetStats)
            MyText = ""
            Exit Sub
        End If
        ...
        If LCase(Mid(MyText, 1, 6)) = "/trade" Then
            ...
            Call SendDataNew(Buffer, SMsgTrade)
            MyText = ""
            Exit Sub
        End If
        ...
        If GetPlayerAccess(MyIndex) >= ADMIN_MAPPER Then
            ...
            If LCase(Mid(MyText, 1, 10)) = "/mapreport" Then
                ...
                Call SendDataNew(Buffer, SMsgMapReport)
                MyText = ""
                Exit Sub
            End If
...
Sub CheckMapGetItem()
  ...
  If GetTickCount > Player(MyIndex).MapGetTimer + 250 And Trim(MyText) = "" Then
    ...
    Call SendDataNew(Buffer, SMsgMapGetItem)
  End If
End Sub
...
'****************************************************************
  ...
  If ControlDown = True And Player(MyIndex).AttackTimer + 1000 < GetTickCount And Player(MyIndex).Attacking = 0 Then
    ...
    Call SendDataNew(Buffer, SMsgAttack)
  End If
End Sub
...
Dim i As Long
    ...
    For i = 0 To Max_Classes
        frmSpellEditor.cmbClassReq.AddItem Trim(StrConv(Class(i).ClassName, vbUnicode))
    Next i
...
Sub PlayerSearch(Button As Integer, Shift As Integer, x As Single, y As Single)
  ...
  If (x1 >= 0) And (x1 <= MAX_MAPX) And (y1 >= 0) And (y1 <= MAX_MAPY) And Not InEditor Then
    Buffer = ""
    Buffer = AddByteToBuffer(Buffer, CByte(x1))
    Buffer = AddByteToBuffer(Buffer, CByte(y1))
    
    'Call SendData("search" & SEP_CHAR & x1 & SEP_CHAR & y1 & SEP_CHAR & END_CHAR)
    Call SendDataNew(Buffer, SMsgSearch)
  End If
End Sub
...
Dim y As Long
    ...
    Call FillMemory(Map.MapName(0), NAME_LENGTH, 32)
    Map.Revision = 0
    Map.Moral = 0
    Map.Up = 0
    Map.Down = 0
