...
Dim Level As Long
Dim i As Long, n As Long, X As Long, Y As Long, p As Long, W As Long, H As Long, I2 As Long
...
Dim ThisIsANumber As Long
    ...
    If LCase(Parse(0)) = "maxinfo" Then
        ...
        MAX_FORMS = Val(Parse(12))
        MAX_OBJECTS = Val(Parse(13))
        ShowRsText = Val(Parse(14))
        ShowMiniMap = Val(Parse(15))
        ...
        App.Title = GAME_NAME
        ReDim CustomForm(1 To MAX_FORMS) As New frmCustom
        For i = 1 To MAX_FORMS
            CustomForm(i).frmNum = i
        Next i
        Exit Sub
    End If
    ...
    ' ::  Custom Form  ::
    ' :::::::::::::::::::
    If LCase(Parse(0)) = "customformshow" Then
        i = Val#(Parse$(1))
        If i < 0 Or i > MAX_FORMS Then Exit Sub
        CustomForm(i).Show vbModeless, frmMirage
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformhide" Then
        i = Val#(Parse$(1))
        If i < 0 Or i > MAX_FORMS Then Exit Sub
        CustomForm(i).Hide
        Exit Sub
    ...
    If LCase(Parse(0)) = "customformreset" Then
        i = Val#(Parse$(1))
        If i < 1 Or i > MAX_FORMS Then Exit Sub
        Unload CustomForm(i)
        Set CustomForm(i) = New frmCustom
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformtitle" Then
        i = Val#(Parse$(1))
        If i < 1 Or i > MAX_FORMS Then Exit Sub
        CustomForm(i).Caption = Trim$(Parse$(2))
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformresize" Then
        i = Val#(Parse$(1))
        W = Val#(Parse$(2))
        H = Val#(Parse$(3))
        If i < 1 Or i > MAX_FORMS Then Exit Sub
        CustomForm(i).Width = W * Screen.TwipsPerPixelX + (CustomForm(i).Width - CustomForm(i).ScaleWidth * Screen.TwipsPerPixelX)
        CustomForm(i).Height = H * Screen.TwipsPerPixelY + (CustomForm(i).Height - CustomForm(i).ScaleHeight * Screen.TwipsPerPixelY)
        Exit Sub
    End If

    If LCase(Parse(0)) = "customformbuttonshow" Then
        i = Val#(Parse$(1))
        I2 = Val#(Parse$(2))
        X = Val#(Parse$(3))
        Y = Val#(Parse$(4))
        W = Val#(Parse$(5))
        H = Val#(Parse$(6))
        If i < 1 Or i > MAX_FORMS Or I2 < 1 Or I2 > MAX_OBJECTS Then Exit Sub
        CustomForm(i).cmdCustom(I2).Visible = True
        CustomForm(i).cmdCustom(I2).Move X, Y, W, H
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformbuttonhide" Then
        i = Val#(Parse$(1))
        I2 = Val#(Parse$(2))
        If i < 1 Or i > MAX_FORMS Or I2 < 1 Or I2 > MAX_OBJECTS Then Exit Sub
        CustomForm(i).cmdCustom(I2).Visible = False
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformbuttontext" Then
        i = Val#(Parse$(1))
        I2 = Val#(Parse$(2))
        Stuff = Parse$(3)
        If i < 1 Or i > MAX_FORMS Or I2 < 1 Or I2 > MAX_OBJECTS Then Exit Sub
        CustomForm(i).cmdCustom(I2).Caption = Stuff
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformtextshow" Then
        i = Val#(Parse$(1))
        I2 = Val#(Parse$(2))
        X = Val#(Parse$(3))
        Y = Val#(Parse$(4))
        ...
        H = Val#(Parse$(6))
        If i < 1 Or i > MAX_FORMS Or I2 < 1 Or I2 > MAX_OBJECTS Then Exit Sub
        CustomForm(i).lblCustom(I2).Visible = True
        CustomForm(i).lblCustom(I2).Move X, Y, W, H
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformtexthide" Then
        i = Val#(Parse$(1))
        I2 = Val#(Parse$(2))
        If i < 1 Or i > MAX_FORMS Or I2 < 1 Or I2 > MAX_OBJECTS Then Exit Sub
        CustomForm(i).lblCustom(I2).Visible = False
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformtexttext" Then
        i = Val#(Parse$(1))
        I2 = Val#(Parse$(2))
        Stuff = Parse$(3)
        If i < 1 Or i > MAX_FORMS Or I2 < 1 Or I2 > MAX_OBJECTS Then Exit Sub
        CustomForm(i).lblCustom(I2).Caption = Stuff
        Exit Sub
    End If
    
    If LCase(Parse(0)) = "customformbackground" Then
        i = Val#(Parse$(1))
        Stuff = Parse$(2)
        If i < 1 Or i > MAX_FORMS Or I2 < 1 Or I2 > MAX_OBJECTS Or Not FileExist("gui\" & Stuff) Then Exit Sub
        CustomForm(i).Picture = LoadPicture(App.Path & "\gui\" & Stuff)
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "newcharclasses" Then
        ...
        frmNewChar.lblSPEED.Caption = STR(Class(0).Luck)
    ...
    If (LCase(Parse(0)) = "playerstatspacket") Then
        Dim SubStr As Long, SubDef As Long, SubMagi As Long, SubLuck As Long
        SubStr = 0
        SubDef = 0
        SubMagi = 0
        SubLuck = 0
        ...
        If GetPlayerWeaponSlot(MyIndex) > 0 Then
            ...
            SubLuck = SubLuck + Item(GetPlayerInvItemNum(MyIndex, GetPlayerWeaponSlot(MyIndex))).AddLuck
        End If
        If GetPlayerArmorSlot(MyIndex) > 0 Then
            ...
            SubLuck = SubLuck + Item(GetPlayerInvItemNum(MyIndex, GetPlayerArmorSlot(MyIndex))).AddLuck
        End If
        If GetPlayerShieldSlot(MyIndex) > 0 Then
            ...
            SubLuck = SubLuck + Item(GetPlayerInvItemNum(MyIndex, GetPlayerShieldSlot(MyIndex))).AddLuck
        End If
        If GetPlayerHelmetSlot(MyIndex) > 0 Then
            ...
            SubLuck = SubLuck + Item(GetPlayerInvItemNum(MyIndex, GetPlayerHelmetSlot(MyIndex))).AddLuck
        End If
        ...
        End If
        If SubLuck > 0 Then
            frmMirage.lblSPEED.Caption = Val(Parse(3)) - SubLuck & " (+" & SubLuck & ")"
        Else
    ...
    If LCase(Parse(0)) = "playerdata" Then
        ...
        Call SetPlayerColor(i, Val(Parse(13)))
        Call SetPlayerGhost(i, Val(Parse(14)))
        Player(i).HP = Val(Parse(15))
        Player(i).MaxHP = Val(Parse(16))
        Player(i).Level = Val(Parse(17))
    ...
    If (LCase(Parse(0)) = "weather") Then
        ...
        If MAX_RAINDROPS <> RainIntensity Then
            ...
            ReDim DropSnow(1 To MAX_RAINDROPS) As DropSnowRec
        End If
    End If
    ...
    If LCase(Parse(0)) = "qtrade" Then
        ...
        frmPlayerTrade.Command1.ForeColor = vbRed
        frmPlayerTrade.Command2.ForeColor = vbRed
        frmPlayerTrade.Command2.Caption = "Other player isn't ready!"
    ...
    If LCase(Parse(0)) = "trading" Then
        n = Val(Parse(1))
            If n = 0 Then
                ...
                frmPlayerTrade.Command2.Caption = "Other player isn't ready!"
            ElseIf n = 1 Then
                frmPlayerTrade.Command2.ForeColor = vbGreen
                frmPlayerTrade.Command2.Caption = "Other player is ready to trade!"
            End If
        Exit Sub
    End If
...
' :::::::::::::::::::::::::::::
    ...
    If LCase(Parse(0)) = "sound" Then
        ...
        Select Case Trim(s)
            ...
            Case "panned"
                Call PlaySoundAnyBuffer(Trim$(Parse$(2)), Val(Parse$(3)), Val(Parse$(4)))
        End Select
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "notes" Then
        frmKeepNotes.Show
        frmKeepNotes.Notetext.Text = Parse$(1)
        Exit Sub
    End If
End Sub
...
Dim Packet As String
    ...
    Packet = "SAVEARROW" & SEP_CHAR & ArrowNum & SEP_CHAR & Trim(Arrows(ArrowNum).Name) & SEP_CHAR & Arrows(ArrowNum).Pic & SEP_CHAR & Arrows(ArrowNum).Range & SEP_CHAR & Arrows(ArrowNum).Amount & SEP_CHAR & Arrows(ArrowNum).Ammo & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub
...
Dim i As Long
    ...
    Packet = "SAVENPC" & SEP_CHAR & NpcNum & SEP_CHAR & Trim(Npc(NpcNum).Name) & SEP_CHAR & Trim(Npc(NpcNum).AttackSay) & SEP_CHAR & Npc(NpcNum).Sprite & SEP_CHAR & Npc(NpcNum).SpawnSecs & SEP_CHAR & Npc(NpcNum).Behavior & SEP_CHAR & Npc(NpcNum).Range & SEP_CHAR & Npc(NpcNum).STR & SEP_CHAR & Npc(NpcNum).DEF & SEP_CHAR & Npc(NpcNum).Luck & SEP_CHAR & Npc(NpcNum).MAGI & SEP_CHAR & Npc(NpcNum).Big & SEP_CHAR & Npc(NpcNum).MaxHP & SEP_CHAR & Npc(NpcNum).EXP & SEP_CHAR & Npc(NpcNum).SpawnTime & SEP_CHAR
    For i = 1 To MAX_NPC_DROPS
