...
Begin VB.Form frmMirage 
   ...
   Begin VB.PictureBox picPlayerSpells 
      ...
      Begin VB.ListBox lstSpells 
         ...
         ForeColor       =   &H00000000&
         Height          =   2190
         ItemData        =   "frmMirage.frx":BCB5
         Left            =   45
         List            =   "frmMirage.frx":BCB7
         TabIndex        =   5
         Top             =   60
         Width           =   2535
      End
   ...
   Begin VB.PictureBox picWhosOnline 
      ...
      Begin VB.ListBox lstOnline 
         ...
         EndProperty
         Height          =   2340
         ItemData        =   "frmMirage.frx":BCB9
         Left            =   75
         List            =   "frmMirage.frx":BCBB
         TabIndex        =   25
         Top             =   75
         Width           =   2460
      End
...
Dim i As Long
    ...
    For i = 0 To mnuChatOpt.UBound
        If LCase(lblRoom.Caption) = LCase(mnuChatOpt(i).Caption) Then
            mnuChatOpt(i).Checked = True
            lblRoom.Caption = mnuChatOpt(i).Caption
        Else
            mnuChatOpt(i).Checked = False
        End If
        If i >= 4 Then
            If GetPlayerAccess(MyIndex) <= 1 Then
                mnuChatOpt(i).Visible = False
            Else
                mnuChatOpt(i).Visible = True
            End If
        End If
    Next i
    PopupMenu mnuChat
...
Private Sub cmdMiniClose_Click()
    picMiniMap.Visible = False
End Sub
...
Dim Ending As String
    ...
    For i = 1 To 3
        ...
        If FileExist("GUI\Minimap" & Ending) Then frmMirage.picMiniMap.Picture = LoadPicture(App.Path & "\GUI\Minimap" & Ending)
    Next i
End Sub
...
Private Sub Label20_Click()
    Call SendData("GETNOTES" & SEP_CHAR & END_CHAR)
End Sub
...
Private Sub lblMiniClose_Click()
    picMiniMap.Visible = False
End Sub

Private Sub lblMiniMap_Click()
    picMiniMap.Visible = (ShowMiniMap = 1)
End Sub

Private Sub lblRoomC_Click(Index As Integer)
...
Private Sub mnuChatOpt_Click(Index As Integer)
Dim Begin As Long
Begin = 525

    lblRoom.Caption = mnuChatOpt(Index).Caption
    lblRoom.Tag = Index
    
    If mnuChatOpt(Index).Caption = "Private" Then
        txtMyTextBox.Left = txtMyNameBox.Left + txtMyNameBox.Width + 8
        txtMyTextBox.Width = Begin - (txtMyNameBox.Width + 8)
    Else
        txtMyTextBox.Left = lblRoomToo.Left
        txtMyTextBox.Width = Begin
    End If
    txtMyTextBox.Height = 20
End Sub

Private Sub NightTimer_Timer()
...
d = Index
    ...
    If Player(MyIndex).Inv(d + 1).Num > 0 Then
        ...
        descSpeed.Caption = Item(GetPlayerInvItemNum(MyIndex, d + 1)).LuckReq & " Luck"
        ...
        descMS.Caption = "Magi: " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).AddMagi & " Luck: " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).AddLuck
...
Private Sub picMini_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 1 Then
        If X > picMini.ScaleWidth * 2 / 3 Then
            If Map(MiniMapMap).Right > 0 Then
                MiniMapMap = Map(MiniMapMap).Right
            End If
        ElseIf X < picMini.ScaleWidth * 1 / 3 Then
            If Map(MiniMapMap).Left > 0 Then
                MiniMapMap = Map(MiniMapMap).Left
            End If
        ElseIf Y > picMini.ScaleHeight * 2 / 3 Then
            If Map(MiniMapMap).Down > 0 Then
                MiniMapMap = Map(MiniMapMap).Down
            End If
        ElseIf Y < picMini.ScaleHeight * 1 / 3 Then
            If Map(MiniMapMap).Up > 0 Then
                MiniMapMap = Map(MiniMapMap).Up
            End If
        Else
...
Private Sub picMiniMap_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    Call MovePicture(frmMirage.picMiniMap, Button, Shift, X, Y)
End Sub
