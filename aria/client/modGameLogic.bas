...
Public NoticeVolume As Byte
Public NoticePan As Byte

Public ScriptNum As Long

Public IconNum As Long
...
'Fancy Name Crap
Public OldMapName As String
Public MapMoveTick As Long
Public MapMovePos As Long
Public MapMoveStage As Long
...
Dim Ending As String
    ...
    Call SetStatus("Checking tilesheets...")
    DoEvents

    While FileExist("GFX\Tiles" & (ExtraSheets + 1) & ".bmp")
        ExtraSheets = ExtraSheets + 1
        DoEvents
    Wend
    
    ReDim DD_TileSurf(0 To ExtraSheets) As DirectDrawSurface7
    ReDim DDSD_Tile(0 To ExtraSheets) As DDSURFACEDESC2
    ReDim TileFile(0 To ExtraSheets) As Byte
    
    Call SetStatus("Configuring...")
    DoEvents
    ...
    frmMirage.txtMyNameBox.BackColor = RGB(R1, G1, B1)
...
Dim rec_back As RECT
            ...
            For Q = 0 To MAX_INV - 1
                ...
                Else
                    ...
                    Else
                        ...
                        If Item(Qq).Type = ITEM_TYPE_CURRENCY Then
                            Call DrawInvText(frmMirage.picInv(Q).hDC, 32 - (Len(CStr(GetPlayerInvItemValue(MyIndex, Qq))) * 5), 0, GetPlayerInvItemValue(MyIndex, Qq), RGB(255, 128, 0))
                        End If
                    End If
                End If
            Next Q
        ...
        If ReadINI("CONFIG", "PlayerBar", App.Path & "\config.ini") = 1 Then
            ...
            For i = 1 To MAX_PLAYERS
                If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
                    If Not (GetPlayerGhost(i) And (GetPlayerAccess(MyIndex) < 4 Or i = MyIndex)) Then
    ...
    For i = 1 To MAX_PLAYERS
            If IsPlaying(i) = True Then
                If Player(i).LevelUpT + 3000 > GetTickCount Then
                    ...
                    If i = MyIndex Then
                        X = NewX + sx
                        Y = NewY + sx
                        Call DD_BackBuffer.BltFast(X - 32, Y - 10 - Player(i).LevelUp, DD_TileSurf(ExtraSheets), Rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
                    Else
                        X = GetPlayerX(i) * PIC_X + sx + Player(i).XOffset
                        Y = GetPlayerY(i) * PIC_Y + sx + Player(i).YOffset
                        Call DD_BackBuffer.BltFast(X - (NewPlayerX * PIC_X) - 32 - NewXOffset, Y - (NewPlayerY * PIC_Y) - 10 - Player(i).LevelUp - NewYOffset, DD_TileSurf(ExtraSheets), Rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
                    End If
        ...
        If GettingMap = False Then
            If GameTime = TIME_NIGHT And Map(GetPlayerMap(MyIndex)).Indoors = 0 And InEditor = False Then
                Call Night
            ElseIf Map(GetPlayerMap(MyIndex)).Indoors = 2 And InEditor = False Then
                Call Night
            ElseIf frmMapEditor.mnuDayNight.Checked = True And InEditor = True Then
                Call Night
            End If
    ...
    If GettingMap = False Then
        If ScreenMode = 0 Then
                ...
                For Y = 0 To MAX_MAPY
                    For X = 0 To MAX_MAPX
                        With Map(GetPlayerMap(MyIndex)).Tile(X, Y)
                            ...
                            If .Type = TILE_TYPE_SHOP Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "S", QBColor(BrightCyan))
                            If .Type = TILE_TYPE_CBLOCK Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "C", QBColor(Black))
                            ...
                            If .Type = TILE_TYPE_SOUND Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "P", QBColor(Yellow))
                            If .Type = TILE_TYPE_SPRITE_CHANGE Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "S", QBColor(Grey))
                            If .Type = TILE_TYPE_SIGN Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "S", QBColor(White))
                            ...
                            If .Type = TILE_TYPE_CLASS_CHANGE Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "C", QBColor(White))
                            If .Type = TILE_TYPE_SCRIPTED Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "S", QBColor(Yellow))
                            If .Type = TILE_TYPE_MINICON Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "M", QBColor(BrightCyan))
                            If .Type = TILE_TYPE_BLOCKICON Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "M", QBColor(BrightRed))
                            If .Type = TILE_TYPE_NPCSPAWN Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "N" & .Data1, QBColor(Grey))
            ...
            If ShowRsText = 1 Then
                For i = 1 To MAX_PLAYERS
                    If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
                        If Not (GetPlayerGhost(i) And (GetPlayerAccess(MyIndex) < 4 Or i = MyIndex)) And MouseX = GetPlayerX(i) And MouseY = GetPlayerY(i) Then
                            RsText = GetPlayerName(i) & " is a level " & GetPlayerLevel(i) & " " & Class(GetPlayerClass(i)).Name
                            If GetPlayerLevel(MyIndex) > GetPlayerLevel(i) + 5 Then
                                RsColor = QBColor(BrightGreen)
                            ElseIf GetPlayerLevel(MyIndex) > GetPlayerLevel(i) Then
                                RsColor = RGB(128, 255, 0)
                            ElseIf GetPlayerLevel(MyIndex) = GetPlayerLevel(i) Then
                                RsColor = QBColor(Yellow)
                            ElseIf GetPlayerLevel(MyIndex) > GetPlayerLevel(i) + 5 Then
                                RsColor = RGB(255, 128, 0)
                            Else
                                RsColor = QBColor(Red)
                            End If
                            Call DrawText(TexthDC, 2, 2, RsText, RsColor)
                        End If
                    End If
                Next i
            End If
            ...
            ' Draw map name
            If OldMapName <> Trim(Map(GetPlayerMap(MyIndex)).Name) Then
                MapMovePos = 0
                If MapMovePos < 50 Then MapMovePos = MapMovePos + 1
                    If Map(GetPlayerMap(MyIndex)).Moral = MAP_MORAL_NONE Then
                        Call DrawText(TexthDC, Int((20.5) * PIC_X / 2) - (Int(Len(Trim(Map(GetPlayerMap(MyIndex)).Name)) / 2) * 8) + sx, -48 + MapMovePos + sx, Trim(Map(GetPlayerMap(MyIndex)).Name), QBColor(BrightRed))
        ...
        If GettingMap = True Then
            If MiniMapMap <> GetPlayerMap(MyIndex) Then
                MiniMapMap = GetPlayerMap(MyIndex)
            End If
        ...
        ' Blit out MiniMap
        If frmMirage.picMiniMap.Visible = True Then
            Call BltMiniMap(MiniMapMap)
        End If
        ...
        If ShowMiniMap = 1 Then
            If MiniMapMap <> GetPlayerMap(MyIndex) And MiniMapMap <> Map(GetPlayerMap(MyIndex)).Down And MiniMapMap <> Map(GetPlayerMap(MyIndex)).Up And MiniMapMap <> Map(GetPlayerMap(MyIndex)).Left And MiniMapMap <> Map(GetPlayerMap(MyIndex)).Right Then
                MiniMapMap = GetPlayerMap(MyIndex)
            End If
        End If
...
Dim Color As Long
    ...
    If GetPlayerGhost(Index) Then
        Color = RGB(120, 130, 145)
    Else
        If GetPlayerColor(Index) <> 0 Then
            Color = QBColor(GetPlayerColor(Index) - 1)
        Else
...
MyText = frmMirage.txtMyTextBox.Text
    ...
    frmMirage.txtMyTextBox.Text = ""
        ' Broadcast message
        If frmMirage.lblRoom.Caption = "Broadcast" Then
            ChatText = MyText
        ...
        ' Emote message
        If frmMirage.lblRoom.Caption = "Emote" Then
            ChatText = MyText
            If Len(Trim(ChatText)) > 0 Then
        ...
        ' Player message
        If frmMirage.lblRoom.Caption = "Private" Then
            ChatText = MyText
            Name = ""
            ...
            'For I = 1 To Len(ChatText)
                'If Mid(ChatText, I, 1) <> " " Then
                    Name = frmMirage.txtMyNameBox.Text
                'Else
                    'Exit For
                'End If
           ' Next I
            ...
            Else
                Call AddText("No message entered!", AlertColor)
            End If
            MyText = ""
            Exit Sub
        End If
        ...
        ' Checking fps
        If LCase(Trim(MyText)) = "/fps" Then
            Call AddText("FPS: " & GameFPS, Pink)
            MyText = ""
            Exit Sub
        End If
        
        If LCase(Trim(MyText)) = "/spleen" Then
            Call AddText("Emblem: What, your giving your spleen to me? *yoink* Thank you! :D", Red)
            MyText = ""
            Exit Sub
        End If
        
        ' Checking MiniMapMap
        If LCase(Trim(MyText)) = "/minimapmap" Then
            Call AddText("MiniMap Map: " & MiniMapMap, Pink)
            MyText = ""
            Exit Sub
        End If
        ...
        If GetPlayerAccess(MyIndex) > 0 Then
            ...
            ' Global Message
            If frmMirage.lblRoom.Caption = "Global" Then
                ChatText = MyText
                If Len(Trim(ChatText)) > 0 Then
            ...
            ' Admin Message
            If frmMirage.lblRoom.Caption = "Admin" Then
                ChatText = MyText
                If Len(Trim(ChatText)) > 0 Then
...
Dim X As Long, Y As Long
    ...
    If DirUp Then
        ...
        If GetPlayerY(MyIndex) > 0 Then
            ...
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_BLOCKED Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_SIGN Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_BLOCKICON Then
                CanMove = False
    ...
    If DirLeft Then
        ...
        If GetPlayerX(MyIndex) > 0 Then
            ' Check to see if the map tile is blocked or not
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_BLOCKED Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_SIGN Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_BLOCKICON Then
                CanMove = False
            ...
            ElseIf frmMapEditor.mnuType(2).Checked = True Then
                With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
                    ...
                    If .Type = TILE_TYPE_NPCSPAWN Then
                        NPCSpawnNum = .Data1
                        frmAttributes.optNPCSpawn.Value = True
                    End If
                    If .Type = TILE_TYPE_MINICON Then
                        IconNum = .Data1
                        frmAttributes.optIcon.Value = True
                    End If
                    If .Type = TILE_TYPE_BLOCKICON Then
                        IconNum = .Data1
                        frmAttributes.optBlockIcon.Value = True
                    End If
                End With
            End If
                    ...
                    ElseIf frmMapEditor.mnuType(2).Checked = True Then
                        With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
                            ...
                            If frmAttributes.optScripted.Value = True Then
                                ...
                                .String3 = ""
                            End If
                            If frmAttributes.optNPCSpawn.Value = True Then
                                ...
                                .String3 = ""
                            End If
                            If frmAttributes.optIcon.Value = True Then
                                .Type = TILE_TYPE_MINICON
                                .Data1 = IconNum
                                .Data2 = 0
                                .Data3 = 0
                                .String1 = ""
                                ...
                                .String3 = ""
                            End If
                            If frmAttributes.optBlockIcon.Value = True Then
                                .Type = TILE_TYPE_BLOCKICON
                                .Data1 = IconNum
                                .Data2 = 0
                                .Data3 = 0
                                .String1 = ""
...
Public Sub ArrowEditorInit()
    ...
    frmEditArrows.scrAmmo.Value = Arrows(EditorIndex).Ammo
    If Arrows(EditorIndex).Ammo > 0 Then
        frmEditArrows.lblAmmo.Caption = "Ammo: " & Arrows(EditorIndex).Ammo & " - " & Item(Arrows(EditorIndex).Ammo).Name
    Else
        frmEditArrows.lblAmmo.Caption = "Ammo: None"
    End If
...
Public Sub ArrowEditorOk()
    ...
    Arrows(EditorIndex).Ammo = frmEditArrows.scrAmmo.Value
...
Dim i As Long
    ...
    If (frmItemEditor.cmbType.ListIndex >= ITEM_TYPE_WEAPON) And (frmItemEditor.cmbType.ListIndex <= ITEM_TYPE_SHIELD) Then
        ...
        frmItemEditor.scrlSpeedReq.Value = Item(EditorIndex).LuckReq
        ...
        frmItemEditor.scrlAddSpeed.Value = Item(EditorIndex).AddLuck
...
Public Sub ItemEditorOk()
    ...
    If (frmItemEditor.cmbType.ListIndex >= ITEM_TYPE_WEAPON) And (frmItemEditor.cmbType.ListIndex <= ITEM_TYPE_SHIELD) Then
        ...
        Item(EditorIndex).LuckReq = frmItemEditor.scrlSpeedReq.Value
        ...
        Item(EditorIndex).AddLuck = frmItemEditor.scrlAddSpeed.Value
...
Public Sub NpcEditorInit()
    ...
    frmNpcEditor.scrlSPEED.Value = Npc(EditorIndex).Luck
...
Public Sub NpcEditorOk()
    ...
    Npc(EditorIndex).Luck = frmNpcEditor.scrlSPEED.Value
...
If TileFile(ExtraSheets) = 0 Then Exit Sub
    ...
    Rec.Right = Rec.Left + PIC_X
    Call DD_BackBuffer.BltFast(X - (NewPlayerX * PIC_X) + sx - NewXOffset, Y - (NewPlayerY * PIC_Y) + sx - NewYOffset, DD_TileSurf(ExtraSheets), Rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
End Sub
...
If Player(Index).HP = 0 Then Exit Sub
    ...
    Call DD_BackBuffer.DrawRoundedBox(X, Y + 2, X + 32, Y - 2, 2, 2)
    
    'draws HP
    Call DD_BackBuffer.SetFillColor(RGB(0, 255, 0))
    Call DD_BackBuffer.DrawRoundedBox(X, Y + 2, X + ((Player(Index).HP / 100) / (Player(Index).MaxHP / 100) * 32), Y - 2, 2, 2)
End Sub
...
If MapNpc(Index).Num < 1 Then Exit Sub
    ...
    If Npc(MapNpc(Index).Num).Big = 1 Then
        ...
        Call DD_BackBuffer.SetFillColor(RGB(255, 0, 0))
        Call DD_BackBuffer.DrawRoundedBox(X, Y + 32, X + 50, Y + 36, 2, 2)
        Call DD_BackBuffer.SetFillColor(RGB(0, 255, 0))
        If MapNpc(Index).MaxHP < 1 Then
            Call DD_BackBuffer.DrawRoundedBox(X, Y + 32, X + ((MapNpc(Index).HP / 100) / ((MapNpc(Index).MaxHP + 1) / 100) * 50), Y + 36, 2, 2)
        Else
            Call DD_BackBuffer.DrawRoundedBox(X, Y + 32, X + ((MapNpc(Index).HP / 100) / (MapNpc(Index).MaxHP / 100) * 50), Y + 36, 2, 2)
        End If
    Else
        ...
        Call DD_BackBuffer.SetFillColor(RGB(255, 0, 0))
        Call DD_BackBuffer.DrawRoundedBox(X, Y + 32, X + 32, Y + 36, 2, 2)
        Call DD_BackBuffer.SetFillColor(RGB(0, 255, 0))
        If MapNpc(Index).MaxHP < 1 Then
            Call DD_BackBuffer.DrawRoundedBox(X, Y + 32, X + ((MapNpc(Index).HP / 100) / ((MapNpc(Index).MaxHP + 1) / 100) * 32), Y + 36, 2, 2)
        Else
            Call DD_BackBuffer.DrawRoundedBox(X, Y + 32, X + ((MapNpc(Index).HP / 100) / (MapNpc(Index).MaxHP / 100) * 32), Y + 36, 2, 2)
        End If
    End If
End Sub
...
index2 = Trade(Selected).Items(Index).ItemGetNum
    ...
    frmTrade.descDef.Caption = "Luck Req: " & Item(index2).LuckReq
    ...
    frmTrade.descASpeed.Caption = "Luck: " & Item(index2).AddLuck
