...
Public Sub SetupDX7Sound(CurrentForm As Form)
     Set m_dxs = m_dx.DirectSoundCreate(vbNullString) 'create a DSound object
    ...
    m_dxs.SetCooperativeLevel frmMirage.hwnd, DSSCL_PRIORITY 'THIS MUST BE SET BEFORE WE CREATE ANY BUFFERS
...
Public Function IsPlayingSound(Buffer As Integer) As Long
  IsPlayingSound = SB(Buffer).Buffer.GetStatus
End Function

Public Sub StopDXSound(Buffer As Integer)
    SB(Buffer).Buffer.Stop
End Sub
