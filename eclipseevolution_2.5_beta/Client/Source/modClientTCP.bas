...
Dim j As Long
    ...
    If casestring = "giftreturn" Then
    
        Select Case LCase$(parse$(1))
            
            Case "success"
                Call MsgBox("Gift sent!")
                Exit Sub
            
            Case "error"
                Call MsgBox("Error sending gift!")
                Exit Sub
                
            Case "offline"
                Call MsgBox("That Player is Offline!")
                Exit Sub
            
            Case "notfound"
                Call MsgBox("Player not found!")
                Exit Sub
                
            Case "nospace"
                Call MsgBox("Player has no space!")
                Exit Sub
            
        End Select
    End If
    
    If casestring = "receivegift" Then
        Call MsgBox("You received a Gift!" & vbNewLine & "From: " & parse$(1) & vbNewLine & "Contents: " & parse$(3) & " " & item(val(parse$(2))).Name)
    End If
