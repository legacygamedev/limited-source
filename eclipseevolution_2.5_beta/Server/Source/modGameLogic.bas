...
Dim AttackSpeed As Long, MinPkLvl As Integer
...
Dim a As Long
   ...
   End If
    ...
    If Map(GetPlayerMap(index)).tile(GetPlayerX(index), GetPlayerY(index)).Type = TILE_TYPE_CANON Then
        Call canon(index)
    Else
        Call SendDataTo(index, "canonoff" & SEP_CHAR & END_CHAR)
    End If
...
'        Call JoinWarp(iindex, GetPlayerMap(iindex), GetPlayerX(iindex), GetPlayerY(iindex))
    ...
    Call SendDataTo(index, "INGAME" & SEP_CHAR & year & SEP_CHAR & month & SEP_CHAR & day & SEP_CHAR & weekday & SEP_CHAR & END_CHAR)
...
End If
    ...
    For i = 1 To MAX_PARTY_MEMBERS
        If player(index).Party.Member(i) > 0 And player(index).Party.Member(i) = index Then
            Call SendDataTo(index, "removemembers" & SEP_CHAR & SEP_CHAR & END_CHAR)
        End If
        If player(index).Party.Member(i) > 0 And player(index).Party.Member(i) <> index Then
            Call SendDataTo(index, "updatemembers" & SEP_CHAR & i & SEP_CHAR & 0 & SEP_CHAR & END_CHAR)
        End If
    Next i
...
Dim Necklace As Long
    ...
    If GetPlayerArmorSlot(from) > 0 Then Armor = GetPlayerInvItemNum(from, GetPlayerArmorSlot(from))
    If GetPlayerHelmetSlot(from) > 0 Then Helmet = GetPlayerInvItemNum(from, GetPlayerHelmetSlot(from))
    If GetPlayerShieldSlot(from) > 0 Then Shield = GetPlayerInvItemNum(from, GetPlayerShieldSlot(index))
    If GetPlayerWeaponSlot(from) > 0 Then Weapon = GetPlayerInvItemNum(from, GetPlayerWeaponSlot(from))
    If GetPlayerLegsSlot(from) > 0 Then Legs = GetPlayerInvItemNum(from, GetPlayerLegsSlot(from))
    If GetPlayerRingSlot(from) > 0 Then Ring = GetPlayerInvItemNum(from, GetPlayerRingSlot(from))
    If GetPlayerNecklaceSlot(from) > 0 Then Necklace = GetPlayerInvItemNum(from, GetPlayerNecklaceSlot(from))
   
    Packet = "itemworn" & SEP_CHAR & from & SEP_CHAR & Armor & SEP_CHAR & Weapon & SEP_CHAR & Helmet & SEP_CHAR & Shield & SEP_CHAR & Legs & SEP_CHAR & Ring & SEP_CHAR & Necklace & SEP_CHAR & END_CHAR
    Call SendDataTo(index, Packet)
End Sub
...
Call SendDataTo(index, "oncanon" & SEP_CHAR & END_CHAR)
End Sub
