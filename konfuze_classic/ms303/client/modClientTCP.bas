...
Sub TcpInit()
    ...
    IP = ReadIniValue(App.Path & "\Core Files\Configuration.ini", "server information", "IP")
    Port = ReadIniValue(App.Path & "\Core Files\Configuration.ini", "server information", "PORT")
        
    frmMainGame.Socket.RemoteHost = IP
    frmMainGame.Socket.RemotePort = Val(Port)
End Sub
...
Dim Level As Long
Dim i As Long, n As Long, X As Long, y As Long, z As Long
...
Next i
    ...
    If LCase(Parse(0)) = "playerhp" Then
        ...
        If GetPlayerMaxHP(MyIndex) > 0 Then
            ...
            frmMainGame.picHP.Width = (((GetPlayerHP(MyIndex) / 100) / (GetPlayerMaxHP(MyIndex) / 100)) * 32)
        End If
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "playermp" Then
        ...
        If GetPlayerMaxMP(MyIndex) > 0 Then
            ...
            frmMainGame.PicMP.Width = (((GetPlayerMP(MyIndex) / 100) / (GetPlayerMaxMP(MyIndex) / 100)) * 32)
        End If
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "playersp" Then
        ...
        If GetPlayerMaxSP(MyIndex) > 0 Then
            frmMainGame.lblSP.Caption = Int(GetPlayerSP(MyIndex) / GetPlayerMaxSP(MyIndex) * 100) & "%"
        End If
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "mapitemdata" Then
        ...
        For i = 1 To MAX_MAP_ITEMS
            ...
            SaveMapItem(i).X = Val(Parse(n + 2))
            SaveMapItem(i).y = Val(Parse(n + 3))
...
Dim Packet As String
    ...
    Packet = "LISTBANS" & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub
...
Dim i As Long
    ...
    Packet = "SAVESHOP" & SEP_CHAR & ShopNum & SEP_CHAR & Trim(Shop(ShopNum).Name) & SEP_CHAR & Trim(Shop(ShopNum).JoinSay) & SEP_CHAR & Trim(Shop(ShopNum).LeaveSay) & SEP_CHAR
    For i = 1 To MAX_TRADES
