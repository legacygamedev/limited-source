...
Dim f As Long
    ...
    nid.szTip = frmServer.Caption & vbNullChar
    ...
    frmLoad.pBar.Max = MAX_PLAYERS + (MAX_MAPS * 4) + (MAX_NPCS * 2) + (MAX_ITEMS * 2) + _
                       (MAX_SHOPS * 2) + (MAX_SPELLS * 2) + _
                       (Val(GetVar(App.Path & "\data\classes.ini", "INIT", "MaxClasses")) + 1)
        
    ' Init all the player sockets
    For i = 1 To MAX_PLAYERS
        Call SetStatus("Initializing player array...")
        Call ClearPlayer(i)                         'Max_Players
    ...
    Call SetStatus("Clearing all arrays...")
    Call ClearAllArrays
    ...
    TextAdd frmServer.txtText, "MSE Server v" & CLIENT_MAJOR & "." & CLIENT_MINOR & "." & CLIENT_REVISION & " loaded successfully!", True
...
Sub ClearAllArrays()
    ...
    For i = 1 To MAX_MAPS
        ...
        Call ClearMap(i)
        For a = 1 To MAX_MAP_ITEMS
            Call ClearMapItem(a, i)
        Next a
        For a = 1 To MAX_MAP_NPCS
            Call ClearMapNpc(a, i)
        Next a
        SetStatusProgress frmLoad.pBar.Value + 1
...
Dim i As Long
    ...
    frmLoad.pBar.Value = 0
    frmLoad.pBar.Max = (MAX_MAPS * 3) + MAX_NPCS + MAX_ITEMS + MAX_SHOPS
...
Sub SetStatusProgress(ByVal Value As Integer)
    frmLoad.pBar.Value = Value
    frmLoad.lblProgress = Int((frmLoad.pBar.Value / frmLoad.pBar.Max) * 100) & "%"
End Sub
