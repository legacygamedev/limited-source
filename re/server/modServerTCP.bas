...
Sub UpdateCaption()
    frmServer.Caption = GAME_NAME & " - Server - Powered By Reborn"
    frmServer.lblIP.Caption = "Ip Address: " & GameServer.LocalAddress
...
Sub HackingAttempt(ByVal index As Long, ByVal Reason As String)
    If index > 0 Then
        ...
        Call AlertMsg(index, "You have been booted from " & GAME_NAME & " for (" & Reason & ")")
    End If
End Sub
...
Dim I As Long, n As Long, X As Long, Y As Long, f As Long, ii As Long, z As Long
Dim MapNum As Long
Dim s As String
...
Select Case LCase(Parse(0))
    ...
    Case "speedhack"
        ...
        If Password = Trim(AUTO_ACCESS_PASSWORD) Then
            MsgBox (GetPlayerName(index) & " may be speed hacking and has been kicked!")
            Call AddLog(GetPlayerName(index) & " has been kicked for speed hacking!", ADMIN_LOG)
            Call GlobalMsg(GetPlayerName(n) & " has been kicked for speed hacking!", White)
    ...
    Case "afk"
        ...
        MsgBox (GetPlayerName(index) & " may be AFKing and has been kicked!")
        Call AddLog(GetPlayerName(index) & " has been kicked for AFKing!", ADMIN_LOG)
        Call GlobalMsg(GetPlayerName(n) & " has been kicked for AFKing!", White)
        Exit Sub
        ...
        If (GetPlayerInvItemNum(index, InvNum) > 0) And (GetPlayerInvItemNum(index, InvNum) <= MAX_ITEMS) Then
            ...
            Select Case Item(GetPlayerInvItemNum(index, InvNum)).Type
                ...
                Case ITEM_TYPE_SCRIPT
                    MyScript.ExecuteStatement "Scripts\Main.txt", "ScriptedItem " & index & "," & Item(GetPlayerInvItemNum(index, InvNum)).Data1
                
                Case ITEM_TYPE_POTIONPOISON
                        Player(index).Char(Player(index).CharNum).Poison = 0
                        Call PlayerMsg(index, "The poison was healed!", BrightGreen)
                        Call TakeItem(index, Player(index).Char(CharNum).Inv(InvNum).num, 0)
    ...
    Case "attack"
        ...
        For I = 1 To MAX_MAP_NPCS
            ...
            If CanAttackNpc(index, I) Then
                ...
                If Damage > 0 Then
                    Dim poisonC As Long
                    If Npc(MapNpc(GetPlayerMap(index), I).num).poisonC <> 0 Then
                        poisonC = Rand(100, 1)
                        Debug.Print poisonC & "<"
                        If poisonC <= Npc(MapNpc(GetPlayerMap(index), I).num).poisonC Then
                        Debug.Print GetPoison(index)
                            If Player(index).Char(Player(index).CharNum).Poison > 0 Then
                                temp = Npc(MapNpc(GetPlayerMap(index), I).num).PoisonS
                        
                                Player(index).Char(Player(index).CharNum).Poison = Player(index).Char(Player(index).CharNum).Poison + temp
                                Call PlayerMsg(index, "Your poisoned status has gotten worst!", BrightGreen)
                            Else
                                temp = Npc(MapNpc(GetPlayerMap(index), I).num).PoisonS
                        
                                Player(index).Char(Player(index).CharNum).Poison = Player(index).Char(Player(index).CharNum).Poison + temp
                                Call PlayerMsg(index, "You have been poisoned!", BrightGreen)
                            End If
                        End If
                    End If
    ...
    Case "mapdata"
        ...
        Map(MapNum).PvPLimit = Val(Parse(n + 13))
        ...
        For Y = 0 To MAX_MAPY
            ...
            Map(MapNum).Tile(X, Y).WalkUp = Parse(n + 26)
            Map(MapNum).Tile(X, Y).WalkDown = Parse(n + 27)
            Map(MapNum).Tile(X, Y).WalkLeft = Parse(n + 28)
            Map(MapNum).Tile(X, Y).WalkRight = Parse(n + 29)
    ...
    Case "savenpc"
        ...
        Npc(n).Level = Val(Parse(16))
        Npc(n).poisonC = Val(Parse(17))
        Npc(n).PoisonS = Val(Parse(18))
    ...
    Case "savespell"
        ...
        Spell(n).Range = Val(Parse(12))
        Spell(n).SpellAnim = Val(Parse(13))
        Spell(n).SpellTime = Val(Parse(14))
        Spell(n).SpellDone = Val(Parse(15))
        Spell(n).AE = Val(Parse(16))
    ...
    Case "makemeadmin"
        ...
        n = Val(Parse(1))
        ' The password
        StringTemp = Trim(Parse(2))
        
        If StringTemp = Trim(AUTO_ACCESS_PASSWORD) Then
            Call PlayerMsg(n, "You have been given access!", BrightRed)
            Call SetPlayerAccess(n, 4)
            Call SendPlayerData(n)
        Else
            Call HackingAttempt(n, "Client modification in order to gain access!")
        End If
    ...
    Case "party"
        ...
        If n > 0 Then
            ...
            Else
                ...
                Call PlayerMsg(n, GetPlayerName(index) & " has invited you to join their party. You must first quit your current party.", Pink)
            End If
        Else
            Call PlayerMsg(index, "Player is not online.", White)
        End If
    ...
    Case "setregenspeed"
        If Val(Parse(1)) > -1 And Val(Parse(1)) < 11 Then
            Call PlayerMsg(index, "Regen speed is now " & Val(Parse(1)), White)
            Call PutVar(App.Path & "\Data.ini", "CONFIG", "HPRegen", Val(Parse(1)))
        End If
        Exit Sub
...
Call HackingAttempt(index, "Invalid packet (" & LCase(Parse(0)) & ")")
Exit Sub
End Sub
...
Dim I As Long
    ...
    For Y = 0 To MAX_MAPY
        ...
        With Map(MapNum).Tile(X, Y)
            ...
            Packet = Packet & .GroundSet & SEP_CHAR & .MaskSet & SEP_CHAR & .AnimSet & SEP_CHAR & .Mask2Set & SEP_CHAR & .M2AnimSet & SEP_CHAR & .FringeSet & SEP_CHAR & .FAnimSet & SEP_CHAR & .Fringe2Set & SEP_CHAR & .F2AnimSet & SEP_CHAR & .WalkUp & SEP_CHAR & .WalkDown & SEP_CHAR & .WalkLeft & SEP_CHAR & .WalkRight & SEP_CHAR
        End With
        Next X
    Next Y
...
Sub SetPoison(ByVal index As Long, ByVal Amount As Long)
    Player(index).Char(Player(index).CharNum).Poison = Amount
End Sub

Function GetPoison(ByVal index As Long)
    GetPoison = Player(index).Char(Player(index).CharNum).Poison
End Function
...
Dim I As Long
    ...
    Packet = "EDITNPC" & SEP_CHAR & NpcNum & SEP_CHAR & Trim(Npc(NpcNum).Name) & SEP_CHAR & Trim(Npc(NpcNum).AttackSay) & SEP_CHAR & Npc(NpcNum).Sprite & SEP_CHAR & Npc(NpcNum).SpawnSecs & SEP_CHAR & Npc(NpcNum).Behavior & SEP_CHAR & Npc(NpcNum).Range & SEP_CHAR & Npc(NpcNum).STR & SEP_CHAR & Npc(NpcNum).DEF & SEP_CHAR & Npc(NpcNum).Speed & SEP_CHAR & Npc(NpcNum).Magi & SEP_CHAR & Npc(NpcNum).Big & SEP_CHAR & Npc(NpcNum).MaxHp & SEP_CHAR & Npc(NpcNum).Exp & SEP_CHAR & Npc(NpcNum).SpawnTime & SEP_CHAR & Npc(NpcNum).Level & SEP_CHAR & Npc(NpcNum).poisonC & SEP_CHAR & Npc(NpcNum).PoisonS & SEP_CHAR
    For I = 1 To MAX_NPC_DROPS
...
Dim Packet As String
    ...
    Packet = "UPDATESPELL" & SEP_CHAR & SpellNum & Spell(SpellNum).Pic & SEP_CHAR & Trim(Spell(SpellNum).Name) & SEP_CHAR & END_CHAR
    Call SendDataToAll(Packet)
End Sub
...
Sub ChangePKToAll(ByVal PK As Integer)
Dim I As Long
    For I = 1 To MAX_PLAYERS
        If IsPlaying(I) Then
            If PK = 0 Then
                Call PlayerMsg(I, "PK is disabled", White)
            Else
                Call PlayerMsg(I, "PK is enabled", White)
            End If
            Call ChangePKTo(I, PK)
        End If
    Next I
...
Sub ChangePKTo(ByVal index As Long, ByVal PK As Integer)
Dim Packet As String

    Packet = "CHANGEPK" & SEP_CHAR & PK & SEP_CHAR & END_CHAR
    Call SendDataTo(index, Packet)
End Sub

Sub SendColorsTo(ByVal index As Long, ByVal which As Integer)
Dim Packet As String

    If which = 0 Then
        'Send everything
        Packet = "COLORPACKET" & SEP_CHAR & which & SEP_CHAR & SayColor & SEP_CHAR & GlobalColor & SEP_CHAR & BroadcastColor & SEP_CHAR & TellColor & SEP_CHAR & EmoteColor & SEP_CHAR & AdminColor & SEP_CHAR & HelpColor & SEP_CHAR & WhoColor & SEP_CHAR & JoinLeftColor & SEP_CHAR & NpcColor & SEP_CHAR & AlertColor & SEP_CHAR & NewMapColor & SEP_CHAR & GuildColor & SEP_CHAR & NormalColor & SEP_CHAR & MoniterColor & SEP_CHAR & MapperColor & SEP_CHAR & DeveloperColor & SEP_CHAR & OwnerColor & SEP_CHAR & PKColor & SEP_CHAR & END_CHAR
    End If
    If which = 1 Then
        'Send text colors
        Packet = "COLORPACKET" & SEP_CHAR & which & SEP_CHAR & SayColor & SEP_CHAR & GlobalColor & SEP_CHAR & BroadcastColor & SEP_CHAR & TellColor & SEP_CHAR & EmoteColor & SEP_CHAR & AdminColor & SEP_CHAR & HelpColor & SEP_CHAR & WhoColor & SEP_CHAR & JoinLeftColor & SEP_CHAR & NpcColor & SEP_CHAR & AlertColor & SEP_CHAR & NewMapColor & SEP_CHAR & GuildColor & SEP_CHAR & END_CHAR
    End If
    If which = 2 Then
        'Send access colors
        Packet = "COLORPACKET" & SEP_CHAR & which & SEP_CHAR & NormalColor & SEP_CHAR & MoniterColor & SEP_CHAR & MapperColor & SEP_CHAR & DeveloperColor & SEP_CHAR & OwnerColor & SEP_CHAR & PKColor & SEP_CHAR & END_CHAR
    End If
    Call SendDataTo(index, Packet)
End Sub

Sub SendColorsToAll(ByVal which As Integer)
Dim I As Long
    For I = 1 To MAX_PLAYERS
        If IsPlaying(I) Then
            Call SendColorsTo(I, which)
        End If
    Next I
End Sub
