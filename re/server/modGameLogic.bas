...
Dim AttackSpeed As Long
    ...
    ' Check if PvPLimit is enabled and if so, check if an attack is allowed
    If Map(GetPlayerMap(Attacker)).PvPLimit > 0 Then
        If GetPlayerLevel(Attacker) > GetPlayerLevel(Victim) + Map(GetPlayerMap(Attacker)).PvPLimit Or GetPlayerLevel(Attacker) < GetPlayerLevel(Victim) - Map(GetPlayerMap(Attacker)).PvPLimit Then
            Call PlayerMsg(Attacker, "You can only attack people that are " & Map(GetPlayerMap(Attacker)).PvPLimit & " levels higher or lower than you in this map", BrightRed)
            Exit Function
        End If
    End If
            ...
            Case DIR_UP
                If (GetPlayerY(Victim) + 1 = GetPlayerY(Attacker)) And (GetPlayerX(Victim) = GetPlayerX(Attacker)) Then
                    If Map(GetPlayerMap(Victim)).Tile(GetPlayerX(Victim), GetPlayerY(Victim)).Type <> TILE_TYPE_ARENA And Map(GetPlayerMap(Attacker)).Tile(GetPlayerX(Attacker), GetPlayerY(Attacker)).Type <> TILE_TYPE_ARENA Then
                        ...
                        If GetPlayerAccess(Attacker) > ADMIN_MONITER And AdminAttack = 0 Then
                            Call PlayerMsg(Attacker, "You cannot attack any player for thou art an admin!", BrightBlue)
                        Else
                            ' Check to make sure the victim isn't an admin
                            If GetPlayerAccess(Victim) > ADMIN_MONITER And AdminAttack = 0 Then
                            ...
                            Else
                                ...
                                If Map(GetPlayerMap(Attacker)).Moral = MAP_MORAL_NONE Or Map(GetPlayerMap(Attacker)).Moral = MAP_MORAL_NO_PENALTY Then
                                    If GetPlayerPK(Victim) > 0 And PKEnabled = 1 Then
        ...
        If PKEnabled = 1 Then
            If GetPlayerPK(Victim) = 0 Then
                If GetPlayerPK(Attacker) = 0 Then
                    Call SetPlayerPK(Attacker, Player(Victim).Char(Player(Victim).CharNum).Level * 2)
                    Call SendPlayerData(Attacker)
                    Call PlayerMsg(Attacker, "You have " & Player(Victim).Char(Player(Victim).CharNum).Level * 2 & " PK points against you!", BrightRed)
                    Call GlobalMsg(GetPlayerName(Attacker) & " has been deemed a Player Killer!", BrightRed)
                Else
                    Call PlayerMsg(Attacker, "You have " & GetPlayerPK(Attacker) + Player(Victim).Char(Player(Victim).CharNum).Level * 2 & " PK points against you!", BrightRed)
                    Call SetPlayerPK(Attacker, GetPlayerPK(Attacker) + Player(Victim).Char(Player(Victim).CharNum).Level * 2)
                    Call SendPlayerData(Attacker)
                    Call GlobalMsg(GetPlayerName(Attacker) & " is on a player killing rampage!", BrightRed)
                End If
            Else
...
Dim STR As Long, DEF As Long, MapNum As Long, NpcNum As Long
    ...
    If Damage >= MapNpc(MapNum, MapNpcNum).HP Then
        ...
        If PKEnabled = 1 Then
            If GetPlayerPK(Attacker) > 0 Then
                If GetPlayerPK(Attacker) < Npc(MapNpc(MapNum, MapNpcNum).num).Level Then
                    Call SetPlayerPK(Attacker, 0)
                    Call PlayerMsg(Attacker, "Your PK points are back to 0", BrightRed)
                Else
                    Call SetPlayerPK(Attacker, GetPlayerPK(Attacker) - Npc(MapNpc(MapNum, MapNpcNum).num).Level)
                    Call PlayerMsg(Attacker, "Your PK points have gone from " & GetPlayerPK(Attacker) + Npc(MapNpc(MapNum, MapNpcNum).num).Level & " to " & GetPlayerPK(Attacker), BrightRed)
                End If
            End If
        End If
        ...
        ' Check if in party, if so divide the exp up with the attacker getting 75% and the other divide 25%
        ...
        Else
            q = 0
            ' gets the amount of members and gives the attacker his %
            For X = 1 To MAX_PARTY_MEMBERS
                If Player(Attacker).Party.Member(X) > 0 Then q = q + 1
                If Player(Attacker).Party.Member(X) = Attacker Then
                    If Player(Attacker).Party.ShareExp <> True Then
                        Exp = Exp * (3 / (q * 2)) ' 3/4 experience if in a party of two, 1/2 in a party of 3...
                    End If
                    If GetPlayerLevel(Attacker) = MAX_LEVEL Then
                        Call SetPlayerExp(Attacker, Experience(MAX_LEVEL))
                        Call BattleMsg(Attacker, "You cant gain anymore experience!", BrightBlue, 0)
                    Else
                        Call SetPlayerExp(Attacker, GetPlayerExp(Attacker) + Exp)
                        Call BattleMsg(Attacker, "You have gained " & Exp & " party experience.", BrightBlue, 0)
                    End If
                End If
            Next X
            ...
            For X = 1 To MAX_PARTY_MEMBERS
                If Player(Attacker).Party.Member(X) <> Attacker Then
                    Exp = ((1 - (3 / (q * 2))) * (Exp / (3 / (q * 2)))) / (q - 1) ' Bring it back up to 100% and then take it to 25% and finally divide by the number of remaining members
...
Dim Moved As Byte
    ...
    If GetPlayerX(index) < 0 Or GetPlayerY(index) < 0 Or GetPlayerX(index) > MAX_MAPX Or GetPlayerY(index) > MAX_MAPY Or GetPlayerMap(index) <= 0 Then
        Call HackingAttempt(index, "Invalid location")
        Exit Sub
    End If
    ...
    Select Case Dir
        Case DIR_UP
            ...
            If GetPlayerY(index) > 0 Then
                ...
                If Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Type <> TILE_TYPE_BLOCKED And Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index)).WalkUp <> False And Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Type <> TILE_TYPE_VANTAGE Then
        ...
        Case DIR_DOWN
            ...
            If GetPlayerY(index) < MAX_MAPY Then
                ' Check to make sure that the tile is walkable
                If Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) + 1).Type <> TILE_TYPE_BLOCKED And Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index)).WalkDown <> False And Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) + 1).Type <> TILE_TYPE_VANTAGE Then
                    ' Check to see if the tile is a key and if it is check if its opened
                    If (Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) + 1).Type <> TILE_TYPE_KEY Or Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) + 1).Type <> TILE_TYPE_DOOR) Or ((Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) + 1).Type = TILE_TYPE_DOOR Or Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) + 1).Type = TILE_TYPE_KEY) And TempTile(GetPlayerMap(index)).DoorOpen(GetPlayerX(index), GetPlayerY(index) + 1) = YES) Then
                        Call SetPlayerY(index, GetPlayerY(index) + 1)
        ...
        Case DIR_LEFT
            ...
            If GetPlayerX(index) > 0 Then
                ' Check to make sure that the tile is walkable
                If Map(GetPlayerMap(index)).Tile(GetPlayerX(index) - 1, GetPlayerY(index)).Type <> TILE_TYPE_BLOCKED And Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index)).WalkLeft <> False And Map(GetPlayerMap(index)).Tile(GetPlayerX(index) - 1, GetPlayerY(index)).Type <> TILE_TYPE_VANTAGE Then
                    ' Check to see if the tile is a key and if it is check if its opened
        ...
        Case DIR_RIGHT
            ...
            If GetPlayerX(index) < MAX_MAPX Then
                ' Check to make sure that the tile is walkable
                If Map(GetPlayerMap(index)).Tile(GetPlayerX(index) + 1, GetPlayerY(index)).Type <> TILE_TYPE_BLOCKED And Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index)).WalkRight <> False And Map(GetPlayerMap(index)).Tile(GetPlayerX(index) + 1, GetPlayerY(index)).Type <> TILE_TYPE_VANTAGE Then
                    ' Check to see if the tile is a key and if it is check if its opened
                    If (Map(GetPlayerMap(index)).Tile(GetPlayerX(index) + 1, GetPlayerY(index)).Type <> TILE_TYPE_KEY Or Map(GetPlayerMap(index)).Tile(GetPlayerX(index) + 1, GetPlayerY(index)).Type <> TILE_TYPE_DOOR) Or ((Map(GetPlayerMap(index)).Tile(GetPlayerX(index) + 1, GetPlayerY(index)).Type = TILE_TYPE_DOOR Or Map(GetPlayerMap(index)).Tile(GetPlayerX(index) + 1, GetPlayerY(index)).Type = TILE_TYPE_KEY) And TempTile(GetPlayerMap(index)).DoorOpen(GetPlayerX(index) + 1, GetPlayerY(index)) = YES) Then
                        Call SetPlayerX(index, GetPlayerX(index) + 1)
    ...
    If GetPoison(index) > 0 Then
        Call SetPlayerHP(index, GetPlayerHP(index) - 1)
        Call SendHP(index)
        Call SetPoison(index, GetPoison(index) - 1)
    End If
    
    ' Poison
    If Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index)).Type = TILE_TYPE_POISON Then
        Call SetPoison(index, Val(Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index)).Data1))
        Call PlayerMsg(index, "You are poisoned!", BrightGreen)
    End If
    ...
    Select Case Dir
        Case DIR_UP
            ...
            If Y > 0 Then
                ...
                ' Check direction blocks
                If Map(MapNum).Tile(X, Y - 1).WalkUp = False Then
                    CanNpcMove = False
                    Exit Function
                End If
        ...
        Case DIR_DOWN
            ...
            If Y < MAX_MAPY Then
                ...
                ' Check direction blocks
                If Map(MapNum).Tile(X, Y + 1).WalkDown = False Then
                    CanNpcMove = False
                    Exit Function
                End If
        ...
        Case DIR_LEFT
            ...
            If X > 0 Then
                ...
                ' Check direction blocks
                If Map(MapNum).Tile(X - 1, Y).WalkRight = False Then
                    CanNpcMove = False
                    Exit Function
                End If
        ...
        Case DIR_RIGHT
            ...
            If X < MAX_MAPX Then
                ...
                ' Check direction blocks
                If Map(MapNum).Tile(X + 1, Y).WalkLeft = False Then
                    CanNpcMove = False
                    Exit Function
                End If
...
Dim f As Long
    ...
    Call ChangePKTo(index, frmServer.Check2.Value)
    Call SendColorsTo(index, 0)
    ...
    Else
        ...
        If GetPlayerAccess(index) <= ADMIN_MONITER Then
            Call GlobalMsg(GetPlayerName(index) & " has joined " & GAME_NAME & "!", AdminJoinLeaveColor)
        Else
            Call GlobalMsg(GetPlayerName(index) & " has joined " & GAME_NAME & "!", JoinLeftColor)
        End If
    
        ' Send them welcome
        Call PlayerMsg(index, "Welcome to " & GAME_NAME & "!", BroadcastColor)
        
        ' Send motd
        If Trim(MOTD) <> "" Then
            Call PlayerMsg(index, "MOTD: " & MOTD, AdminJoinLeaveColor)
        End If
    End If
...
Dim I As Long
    ...
    If GetVar(App.Path & "\Data.ini", "CONFIG", "HPRegen") > 0 Then
        ' Prevent subscript out of range
        If IsPlaying(index) = False Or index <= 0 Or index > MAX_PLAYERS Then
            GetPlayerHPRegen = 0
            Exit Function
        ...
        I = Int(GetPlayerDEF(index) / 4)
        
        GetPlayerHPRegen = I + GetVar(App.Path & "\Data.ini", "CONFIG", "HPRegen")
    End If
End Function
...
Dim I As Long
    ...
    If GetVar(App.Path & "\Data.ini", "CONFIG", "MPRegen") > 0 Then
        ' Prevent subscript out of range
        If IsPlaying(index) = False Or index <= 0 Or index > MAX_PLAYERS Then
            GetPlayerMPRegen = 0
            Exit Function
        ...
        I = Int(GetPlayerMAGI(index) / 4)
        
        GetPlayerMPRegen = I + GetVar(App.Path & "\Data.ini", "CONFIG", "MPRegen")
    End If
End Function
...
Dim I As Long
    ...
    If GetVar(App.Path & "\Data.ini", "CONFIG", "SPRegen") > 0 Then
        ' Prevent subscript out of range
        If IsPlaying(index) = False Or index <= 0 Or index > MAX_PLAYERS Then
            GetPlayerSPRegen = 0
            Exit Function
        ...
        I = Int(GetPlayerSPEED(index) / 4)
        
        GetPlayerSPRegen = I + GetVar(App.Path & "\Data.ini", "CONFIG", "SPRegen")
    End If
End Function
...
Dim SpellNum As Long, I As Long, n As Long, Damage As Long, temp As Long
            ...
            For I = 1 To MAX_PLAYERS
                If IsPlaying(I) = True Then
                    If GetPlayerMap(index) = GetPlayerMap(I) Then
                        ' Check if PvPLimit is enabled and if so, check if an attack is allowed
                        If Map(GetPlayerMap(index)).PvPLimit = 0 Or (GetPlayerLevel(index) > GetPlayerLevel(I) + Map(GetPlayerMap(index)).PvPLimit Or GetPlayerLevel(index) < GetPlayerLevel(I) - Map(GetPlayerMap(index)).PvPLimit) Then
                    ...
                    If IsPlaying(n) Then
                        If n <> index Then
                            ...
                            If GetPlayerHP(n) > 0 And GetPlayerMap(index) = GetPlayerMap(n) And GetPlayerLevel(index) >= 10 And GetPlayerLevel(n) >= 10 And (Map(GetPlayerMap(index)).Moral = MAP_MORAL_NONE Or Map(GetPlayerMap(index)).Moral = MAP_MORAL_NO_PENALTY) And GetPlayerAccess(index) <= 0 And GetPlayerAccess(n) <= 0 Then
                                If Map(GetPlayerMap(index)).PvPLimit = 0 Or (GetPlayerLevel(index) > GetPlayerLevel(n) + Map(GetPlayerMap(index)).PvPLimit Or GetPlayerLevel(index) < GetPlayerLevel(n) - Map(GetPlayerMap(index)).PvPLimit) Then
                                    ...
                                    Select Case Spell(SpellNum).Type
                                        ...
                                        Case SPELL_TYPE_POISON
                                            Call PlayerMsg(n, "You have been poisoned!", BrightGreen)
                                            If Player(n).Char(Player(n).CharNum).Poison > 0 Then
                                                temp = Player(n).Char(Player(n).CharNum).Poison
                                                Call SetPoison(n, Spell(SpellNum).Data1)
                                                Player(n).Char(Player(n).CharNum).Poison = Player(n).Char(Player(n).CharNum).Poison + temp
                                            Else
                                                Call SetPoison(n, Spell(SpellNum).Data1)
                                            End If
                                    End Select
                                    ...
                                    Casted = True
                                Else
                                    Call PlayerMsg(index, "You are not allowed to attack " & GetPlayerName(n), BrightRed)
                                End If
                            Else
...
Function CanPlayerHit(ByVal Attacker As Long, ByVal Victim As Long) As Boolean

    CanPlayerHit = True
    If Int(Rnd * GetPlayerSPEED(Attacker)) < Int(Rnd * GetPlayerSPEED(Victim)) And Int(Rnd * 3) > 2 Then
        CanPlayerHit = False
    End If
End Function
...
For I = q To MAX_PARTY_MEMBERS - 1 ' dommino effect as the gaps are filled
    If Player(index).Party.Member(I) = 0 And Player(index).Party.Member(I + 1) > 0 Then
        Player(index).Party.Member(I) = Player(Player(index).Party.Member(I)).Party.Member(I + 1)
        Player(Player(index).Party.Member(I)).Party.Member(I + 1) = 0
    End If
Next I
...
Function CanAttackPlayerWithArrow(ByVal Attacker As Long, ByVal Victim As Long) As Boolean
    ...
    Else
        ...
        Else
            ' Check If map Is attackable
            If Map(GetPlayerMap(Attacker)).Moral = MAP_MORAL_NONE Or Map(GetPlayerMap(Attacker)).Moral = MAP_MORAL_NO_PENALTY Or GetPlayerPK(Victim) > 0 Then
            ' Make sure they are high enough level
                If GetPlayerLevel(Attacker) < 10 Then
                    Call PlayerMsg(Attacker, "You are below level 10, you cannot attack another player yet!", BrightRed)
                Else
