...
' Used for weather effects and gameclock
...
Public RandomLength As Long

Public AdminAttack As Integer
...
Dim f As Long
    ...
    If Not FileExist("Data.ini") Then
        PutVar App.Path & "\Data.ini", "CONFIG", "GameName", "Elysium Reborn"
        PutVar App.Path & "\Data.ini", "CONFIG", "WebSite", "http://www.splamm.com/elysium"
        ...
        PutVar App.Path & "\Data.ini", "CONFIG", "AdminAttack", 1
        PutVar App.Path & "\Data.ini", "CONFIG", "Auto-Access", "apasswordhere"
        ...
        PutVar App.Path & "\Data.ini", "MAX", "MAX_MAPX", 30
        PutVar App.Path & "\Data.ini", "MAX", "MAX_MAPY", 30
    End If
    
    If Not FileExist("text.ini") Then
        PutVar App.Path & "\text.ini", "COLOR", "SayColor", "7"
        PutVar App.Path & "\text.ini", "COLOR", "GlobalColor", "2"
        PutVar App.Path & "\text.ini", "COLOR", "BroadcastColor", "15"
        PutVar App.Path & "\text.ini", "COLOR", "TellColor", "15"
        PutVar App.Path & "\text.ini", "COLOR", "EmoteColor", "15"
        PutVar App.Path & "\text.ini", "COLOR", "AdminColor", "11"
        PutVar App.Path & "\text.ini", "COLOR", "HelpColor", "15"
        PutVar App.Path & "\text.ini", "COLOR", "WhoColor", "7"
        PutVar App.Path & "\text.ini", "COLOR", "JoinLeftColor", "7"
        PutVar App.Path & "\text.ini", "COLOR", "NpcColor", "15"
        PutVar App.Path & "\text.ini", "COLOR", "AlertColor", "15"
        PutVar App.Path & "\text.ini", "COLOR", "NewMapColor", "7"
        PutVar App.Path & "\text.ini", "COLOR", "GuildColor", "10"
        PutVar App.Path & "\text.ini", "COLOR", "NormalColor", "6"
        PutVar App.Path & "\text.ini", "COLOR", "MoniterColor", "8"
        PutVar App.Path & "\text.ini", "COLOR", "MapperColor", "3"
        PutVar App.Path & "\text.ini", "COLOR", "DeveloperColor", "1"
        PutVar App.Path & "\text.ini", "COLOR", "OwnerColor", "13"
        PutVar App.Path & "\text.ini", "COLOR", "PKColor", "12"
    ...
    AdminAttack = Val(GetVar(App.Path & "\Data.ini", "CONFIG", "AdminAttack"))
    ...
    SCRIPTING_ENABLED = Val(GetVar(App.Path & "\Data.ini", "CONFIG", "Scripting"))
    MAX_PARTY_MEMBERS = GetVar(App.Path & "\Data.ini", "MAX", "MAX_PARTY_MEMBERS")
    AUTO_ACCESS_PASSWORD = GetVar(App.Path & "\Data.ini", "CONFIG", "Auto-Access")
    ...
    frmServer.Label12.BackColor = QBColor(SayColor)
    frmServer.Label13.BackColor = QBColor(GlobalColor)
    frmServer.Label14.BackColor = QBColor(BroadcastColor)
    frmServer.Label15.BackColor = QBColor(TellColor)
    frmServer.Label16.BackColor = QBColor(EmoteColor)
    frmServer.Label17.BackColor = QBColor(AdminColor)
    frmServer.Label18.BackColor = QBColor(HelpColor)
    frmServer.Label19.BackColor = QBColor(WhoColor)
    frmServer.Label20.BackColor = QBColor(JoinLeftColor)
    frmServer.Label21.BackColor = QBColor(NpcColor)
    frmServer.Label22.BackColor = QBColor(AlertColor)
    frmServer.Label23.BackColor = QBColor(NewMapColor)
    frmServer.Label24.BackColor = QBColor(AdminJoinLeaveColor)
    frmServer.Label25.BackColor = QBColor(NormalColor)
    frmServer.Label26.BackColor = QBColor(MoniterColor)
    frmServer.Label27.BackColor = QBColor(MapperColor)
    frmServer.Label28.BackColor = QBColor(DeveloperColor)
    frmServer.Label29.BackColor = QBColor(OwnerColor)
    frmServer.Label30.BackColor = QBColor(PKColor)
    frmServer.Label31.BackColor = QBColor(GuildColor)

    MAX_MAPX = Val(GetVar(App.Path & "\Data.ini", "MAX", "MAX_MAPX"))
    MAX_MAPY = Val(GetVar(App.Path & "\Data.ini", "MAX", "MAX_MAPX"))
    ...
    frmServer.lstTopics.AddItem "SCRIPTING_ENABLED Commands"
                            ...
                            If IsPlaying(I) Then
                                If GetPlayerMap(I) = Y And MapNpc(Y, X).Target = 0 And GetPlayerAccess(I) <= ADMIN_MONITER Then
                                    ...
                                    If DistanceX <= n And DistanceY <= n Then
                                        If Npc(NpcNum).Behavior = NPC_BEHAVIOR_ATTACKONSIGHT Or GetPlayerPK(I) > 0 Then
                                            If PKEnabled = 1 Or Npc(NpcNum).Behavior = NPC_BEHAVIOR_ATTACKONSIGHT Then
...
Sub CheckGiveHP()
Dim I As Long, n As Long, RainDamage As Integer
    ...
    If GameTime = WEATHER_ACID Then
        For I = 1 To MAX_PLAYERS
            If IsPlaying(I) Then
                Call PlayerMsg(I, "Check!", White)
            End If
        Next I
        For I = 1 To MAX_PLAYERS
            If IsPlaying(I) Then
                If GetPlayerY(I) <> 0 Then
                    'If Map(GetPlayerMap(i)).Tile(GetPlayerX(i), GetPlayerY(i) + 1).Fringe = 0 And Map(GetPlayerMap(i)).Tile(GetPlayerX(i), GetPlayerY(i) + 1).Fringe2 = 0 And Map(GetPlayerMap(i)).Tile(GetPlayerX(i), GetPlayerY(i) + 1).FAnim = 0 And Map(GetPlayerMap(i)).Tile(GetPlayerX(i), GetPlayerY(i) + 1).F2Anim = 0 And Map(GetPlayerMap(i)).Indoors <> 1 Then
                        ' If the tile above has a fringe layer (or indoors), sheild them from the acid. Otherwise, hurt them!
                        If GetPlayerMaxHP(I) < 30 Then RainDamage = 1
                        Else: RainDamage = GetPlayerMaxHP(I) / 30
                        Call SetPlayerHP(I, GetPlayerHP(I) - RainDamage)
                        Call SendHP(I)
                    'End If
                End If
            End If
