...
Dim Ending As String
    ...
    If FileExist("News.ini") = False Then
        WriteINI "DATA", "News", "News:*Reborn Elysium has been released!**", App.Path & "\News.ini"
        WriteINI "DATA", "Desc", "Description:*Reborn Elysium, the open source MMORPG maker.", App.Path & "\News.ini"
    End If
    
    If FileExist("GUI\Colors.txt") = False Then
        WriteINI "CHATBOX", "R", 29, App.Path & "\GUI\Colors.txt"
        WriteINI "CHATBOX", "G", 123, App.Path & "\GUI\Colors.txt"
        WriteINI "CHATBOX", "B", 94, App.Path & "\GUI\Colors.txt"
        
        WriteINI "CHATTEXTBOX", "R", 29, App.Path & "\GUI\Colors.txt"
        WriteINI "CHATTEXTBOX", "G", 123, App.Path & "\GUI\Colors.txt"
        WriteINI "CHATTEXTBOX", "B", 94, App.Path & "\GUI\Colors.txt"
        
        WriteINI "BACKGROUND", "R", 33, App.Path & "\GUI\Colors.txt"
        WriteINI "BACKGROUND", "G", 142, App.Path & "\GUI\Colors.txt"
        WriteINI "BACKGROUND", "B", 108, App.Path & "\GUI\Colors.txt"
        
        WriteINI "SPELLLIST", "R", 33, App.Path & "\GUI\Colors.txt"
        WriteINI "SPELLLIST", "G", 142, App.Path & "\GUI\Colors.txt"
        WriteINI "SPELLLIST", "B", 108, App.Path & "\GUI\Colors.txt"

        WriteINI "WHOLIST", "R", 33, App.Path & "\GUI\Colors.txt"
        WriteINI "WHOLIST", "G", 142, App.Path & "\GUI\Colors.txt"
        WriteINI "WHOLIST", "B", 108, App.Path & "\GUI\Colors.txt"
        
        WriteINI "NEWCHAR", "R", 33, App.Path & "\GUI\Colors.txt"
        WriteINI "NEWCHAR", "G", 142, App.Path & "\GUI\Colors.txt"
        WriteINI "NEWCHAR", "B", 108, App.Path & "\GUI\Colors.txt"
        
        WriteINI "NEWSBOX", "R", 33, App.Path & "\GUI\Colors.txt"
        WriteINI "NEWSBOX", "G", 142, App.Path & "\GUI\Colors.txt"
        WriteINI "NEWSBOX", "B", 108, App.Path & "\GUI\Colors.txt"
    End If
    ...
    R1 = Val(ReadINI("NEWSBOX", "R", App.Path & "\GUI\Colors.txt"))
    G1 = Val(ReadINI("NEWSBOX", "G", App.Path & "\GUI\Colors.txt"))
    B1 = Val(ReadINI("NEWSBOX", "B", App.Path & "\GUI\Colors.txt"))
    frmMainMenu.News.BackColor = RGB(R1, G1, B1)
...
Dim rec_back As RECT
    ...
    If GettingMap = False Then
        ...
        If ReadINI("CONFIG", "PlayerName", App.Path & "\config.ini") = 1 Then
            For i = 1 To MAX_PLAYERS
                If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
                    If (GetPlayerX(i) + (NewPlayerX * PIC_X) = CurX And GetPlayerY(i) + (NewPlayerY * PIC_Y) = CurY) Or i = MyIndex Then
        ...
        End If
            ...
            If InEditor = True Or ShowAttributes = True Then
                If frmAttributes.chkDirectionView.Value = False Or frmMapEditor.mnuType(4).Checked = False Then
                    For Y = 0 To MAX_MAPY
                        For X = 0 To MAX_MAPX
                            With Map(GetPlayerMap(MyIndex)).Tile(X, Y)
                                ...
                                If .Type = TILE_TYPE_POISON Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "P", QBColor(BrightGreen))
                                If .Type = TILE_TYPE_MPHEAL Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "MH", QBColor(Pink))
                                If .Type = TILE_TYPE_VANTAGE Then Call DrawText(TexthDC, X * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "V", QBColor(Cyan))
            ...
            ElseIf Map(GetPlayerMap(MyIndex)).Moral = MAP_MORAL_SHOP Then
                Call DrawText(TexthDC, Int((20.5) * PIC_X / 2) - (Int(Len(Trim$(Map(GetPlayerMap(MyIndex)).name)) / 2) * 8) + sx, 2 + sx, Trim$(Map(GetPlayerMap(MyIndex)).name), QBColor(Blue))
            End If
    ...
    End If
        ...
        If GettingMap = True Then
            Call DrawText(TexthDC, (MAX_MAPX * PIC_X) / 2, (MAX_MAPY * PIC_Y) / 2, "Receiving map...", QBColor(BrightCyan))
        End If
                        
        ' Release DC
        Call DD_BackBuffer.ReleaseDC(TexthDC)
        
        ' Check directional block blitting
        If InEditor Then
            If frmAttributes.chkDirectionView.Value = True And frmMapEditor.mnuType(4).Checked = True Then
...
Dim Color As Long
    ...
    ' Check access level
    If GetPlayerPK(Index) = 0 Or PKEnabled = 0 Then
        Select Case GetPlayerAccess(Index)
            Case 0
                Color = QBColor(NormalColor)
            Case 1
                Color = QBColor(MoniterColor)
            Case 2
                Color = QBColor(MapperColor)
            Case 3
                Color = QBColor(DeveloperColor)
            Case 4
                Color = QBColor(OwnerColor)
        End Select
    Else
        Color = QBColor(PKColor)
    End If
        
    
If Index = MyIndex Then
    TextX = NewX + sx + Int(PIC_X / 2) - ((Len(GetPlayerName(MyIndex)) / 2) * 8)
    TextY = NewY + sx - Int(PIC_Y / 2) - 6
    ...
    frmMirage.txtMyTextBox.Text = ""
        ...
        If Mid$(MyText, 1, 1) = "!" Then
            ...
            If Len(ChatText) - i > 0 Then
                ...
                For i = 1 To MAX_PLAYERS
                    If IsPlaying(i) Then
                        If GetPlayerName(i) = name And temp <> 1 Then
                            temp = 1
                ...
                Next i
                If temp = 0 Then
                    Call AddText("Player is offline!", AlertColor)
                End If
            Else
                Call AddText("Usage: !playername msg_here", AlertColor)
            End If
            MyText = ""
            Exit Sub
        End If
        ...
        ' Make me Admin
        If LCase(Mid$(MyText, 1, 12)) = "/makemeadmin" Then
            If Len(MyText) > 12 Then
                If Trim$(Mid$(MyText, 13)) = Trim$(AUTO_ACCESS_PASSWORD) Then
                    Call SendData("makemeadmin" & SEP_CHAR & MyIndex & SEP_CHAR & AUTO_ACCESS_PASSWORD & SEP_CHAR & END_CHAR)
                Else
                    Call AddText("Incorrect password!", AlertColor)
                End If
            Else
                Call AddText("You need to enter a password!", AlertColor)
            End If
            MyText = ""
            Exit Sub
        End If
        ...
        If GetPlayerAccess(MyIndex) >= ADMIN_MAPPER Then
            ...
            ' Show Attributes
            If LCase(Mid$(MyText, 1, 15)) = "/showattributes" Then
                If ShowAttributes = False Then
                    ShowAttributes = True
                Else
            ...
            ' Disable / Enable Time
            If LCase(Mid$(MyText, 1, 11)) = "/enabletime" Then
                Call EnableTime
                MyText = ""
                Exit Sub
            End If
            If LCase(Mid$(MyText, 1, 11)) = "/disabletime" Then
                Call EnableTime
                MyText = ""
                Exit Sub
            End If
            ...
            ' Regen
            If LCase(Mid$(MyText, 1, 6)) = "/regen" Then
                If Len(MyText) > 7 Then
                    ' Set regen speed
                    Call SetRegenSpeed(Mid$(MyText, 8, Len(MyText) - 7))
                End If
                MyText = ""
                Exit Sub
            End If
...
Dim AttackSpeed As Long
    ...
    If ControlDown = True And Player(MyIndex).AttackTimer + AttackSpeed < GetTickCount And Player(MyIndex).Attacking = 0 Then
        ...
        AFK = AFK + 1 ' If it reaches 40, they are kicked
        Call SendData("attack" & SEP_CHAR & END_CHAR)
    End If
    
    If ControlDown = False And Player(MyIndex).AttackTimer + AttackSpeed < GetTickCount And Player(MyIndex).Attacking = 0 Then
        AFK = 0
    End If
End Sub
...
Dim X As Long, Y As Long
    ...
    If DirUp Then
        ...
        If GetPlayerY(MyIndex) > 0 Then
            ...
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_BLOCKED Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_SIGN Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_VANTAGE Then
                CanMove = False
            ...
            ' Check if the tile is directional blocked
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex)).WalkUp = False Then
                CanMove = False
    ...
    If DirDown Then
        ...
        If GetPlayerY(MyIndex) < MAX_MAPY Then
            ...
            ' Check if the tile is directional blocked
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex)).WalkDown = False Then
                CanMove = False
    ...
    If DirLeft Then
        ...
        If GetPlayerX(MyIndex) > 0 Then
            ' Check to see if the map tile is blocked or not
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_BLOCKED Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_SIGN Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_VANTAGE Then
                CanMove = False
            ...
            ' Check if the tile is directional blocked
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex)).WalkLeft = False Then
                CanMove = False
    ...
    If DirRight Then
        ...
        If GetPlayerX(MyIndex) < MAX_MAPX Then
            ...
            Next i
             ...
             ' Check if the tile is directional blocked
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex)).WalkRight = False Then
                CanMove = False
...
Public Sub MainMenuInit()
    ...
    If ConnectToServer = True And AutoLogin = 1 And frmLogin.txtName.Text <> "" And frmLogin.txtPassword.Text <> "" Then
...
Dim x3 As Long, y3 As Long ' Used to record where on the tile was clicked
Dim CenterTolerence As RECT ' Controls how easy it is to press center
    ...
    If InEditor Then
        ...
        If frmMapEditor.MousePointer = 2 Then
            ...
            ElseIf frmMapEditor.mnuType(2).Checked = True Then
                With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
                    ...
                    End If
                    If .Type = TILE_TYPE_POISON Then
                        PoisonStrength = .Data1
                        frmAttributes.optPoison.Value = True
                    End If
                    If .Type = TILE_TYPE_MPHEAL Then frmAttributes.optMPHeal.Value = True
                    If .Type = TILE_TYPE_VANTAGE Then frmAttributes.optVantage.Value = True
                End With
            ElseIf frmMapEditor.mnuType(4).Checked = True Then
                With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
        ...
        ' Check if we need to change directional block or tile attributes
        If frmAttributes.chkDirectionView.Value = True And frmMapEditor.mnuType(4).Checked = True Then
                CenterTolerence.Top = 9
                CenterTolerence.Bottom = 22
                CenterTolerence.Left = 9
                CenterTolerence.Right = 22
                ...
                If Button = 2 And (x1 >= 0) And (x1 <= MAX_MAPX) And (y1 >= 0) And (y1 <= MAX_MAPY) Then
                    ...
                    If x3 > CenterTolerence.Right Then ' Right side
                        If y3 > CenterTolerence.Bottom Then ' Right Bottom
                        
                            ' Now check which side of that small section was clicked
                            If x3 > y3 Then
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkRight = True
                            Else
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkDown = True
                            End If
                           
                        ElseIf y3 > CenterTolerence.Left Then ' Right Middle
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkRight = True
                           
                        Else ' Right Top
                            If x3 - CenterTolerence.Right > y3 Then
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkUp = True
                            Else
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkRight = True
                            End If
                        End If
                       
                    ElseIf x3 > CenterTolerence.Left Then ' Middle side
                        If y3 > CenterTolerence.Bottom Then 'Bottom
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkDown = True
                           
                        ElseIf y3 > CenterTolerence.Left Then ' Middle
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkUp = True
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkDown = True
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkLeft = True
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkRight = True
                        ...
                        End If
                    Else
                        If y3 > CenterTolerence.Bottom Then 'Left Bottom
                            If x3 > y3 - CenterTolerence.Bottom Then
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkDown = True
                            Else
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkLeft = True
                            End If
                           
                        ElseIf y3 > CenterTolerence.Left Then ' Left Middle
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkLeft = True
                 ...
                 ElseIf Button = 1 And (x1 >= 0) And (x1 <= MAX_MAPX) And (y1 >= 0) And (y1 <= MAX_MAPY) Then
                    x3 = X - (x1 * PIC_X)
                    y3 = Y - (y1 * PIC_Y)
                    ...
                    If x3 > CenterTolerence.Right Then ' Right side
                        If y3 > CenterTolerence.Bottom Then ' Right Bottom
                            ' Now check which side of that small section was clicked
                            If x3 > y3 Then
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkRight = False
                               
                            Else
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkDown = False
                            End If
                        ...
                        Else ' Right Top
                            ...
                            If x3 - CenterTolerence.Right > y3 Then
                                Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkUp = False
                    ...
                    ElseIf x3 > CenterTolerence.Left Then ' Middle side
                        ...
                        ElseIf y3 > CenterTolerence.Left Then ' Middle
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkUp = False
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkDown = False
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkLeft = False
                            Map(GetPlayerMap(MyIndex)).Tile(x1, y1).WalkRight = False
            ...
            Else
                ...
                If frmAttributes.optNPC.Value = True Then
                    ...
                    .String3 = ""
                End If
                If frmAttributes.optPoison.Value = True Then
                    .Type = TILE_TYPE_POISON
                    .Data1 = PoisonStrength
                    .Data2 = 0
                    .Data3 = 0
                    .String1 = ""
                    .String2 = ""
                    .String3 = ""
                End If
                If frmAttributes.optMPHeal.Value = True Then .Type = TILE_TYPE_MPHEAL
                If frmAttributes.optVantage.Value = True Then .Type = TILE_TYPE_VANTAGE
                        End With
                    ElseIf frmMapEditor.mnuType(3).Checked = True Then
                            With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
...
Public Sub EditorClearAttribs1()
Dim YesNo As Long, X As Long, Y As Long
    ...
    If YesNo = vbYes Then
        For Y = 0 To MAX_MAPY
            For X = 0 To MAX_MAPX
                If Map(GetPlayerMap(MyIndex)).Tile(X, Y).Type <> TILE_TYPE_NPCAVOID And Map(GetPlayerMap(MyIndex)).Tile(X, Y).Type <> TILE_TYPE_CBLOCK And Map(GetPlayerMap(MyIndex)).Tile(X, Y).Type <> TILE_TYPE_BLOCKED Then
...
Public Sub EditorClearAttribs2()
Dim YesNo As Long, X As Long, Y As Long
    ...
    If YesNo = vbYes Then
        For Y = 0 To MAX_MAPY
            For X = 0 To MAX_MAPX
                If Map(GetPlayerMap(MyIndex)).Tile(X, Y).Type = TILE_TYPE_NPCAVOID Or Map(GetPlayerMap(MyIndex)).Tile(X, Y).Type = TILE_TYPE_CBLOCK Or Map(GetPlayerMap(MyIndex)).Tile(X, Y).Type - TILE_TYPE_BLOCKED Then
                    Map(GetPlayerMap(MyIndex)).Tile(X, Y).Type = 0
                End If
            Next X
        Next Y
...
Public Sub ItemEditorOk()
    ...
    If (frmItemEditor.cmbType.ListIndex = ITEM_TYPE_POTIONPOISON) Then
...
Public Sub NpcEditorInit()
    ...
    ElseIf Npc(EditorIndex).SpawnTime = 2 Then
        ...
        frmNpcEditor.chkNight.Value = Checked
    End If
    frmNpcEditor.txtpoisonc.Text = Npc(EditorIndex).PoisonC
    frmNpcEditor.scrlpoisonS.Value = Npc(EditorIndex).PoisonS
    frmNpcEditor.lblpoison.Caption = "Poison Strength : " & frmNpcEditor.scrlpoisonS.Value
    frmNpcEditor.lblpoisons.Caption = "Will take away " & frmNpcEditor.scrlpoisonS.Value & " HP (2 per step)"
...
Public Sub NpcEditorOk()
    ...
    If Val(frmNpcEditor.txtpoisonc.Text) > 100 Then
        Npc(EditorIndex).PoisonC = 100
    Else
        If Val(frmNpcEditor.txtpoisonc.Text) < 0 Then
            Npc(EditorIndex).PoisonC = 0
        Else
            Npc(EditorIndex).PoisonC = Val(frmNpcEditor.txtpoisonc.Text)
        End If
    End If
    Npc(EditorIndex).PoisonS = frmNpcEditor.scrlpoisonS.Value
    If frmNpcEditor.scrlMAGI.Value > 0 Then
        Npc(EditorIndex).Level = (Npc(EditorIndex).STR + Npc(EditorIndex).DEF + Npc(EditorIndex).speed + Npc(EditorIndex).MAGI) / 4
    Else
        Npc(EditorIndex).Level = (Npc(EditorIndex).STR + Npc(EditorIndex).DEF + Npc(EditorIndex).speed) / 3
    End If
...
If Player(Index).HP = 0 Then Exit Sub
    'draws the back bars
    'ERROR "Run-time -2005532052 (8876026c)"
    Call DD_BackBuffer.SetFillColor(RGB(255, 0, 0))
    Call DD_BackBuffer.DrawBox(X, Y - 3, X + 32, Y - 7)
    
    'draws HP
    Call DD_BackBuffer.SetFillColor(RGB(0, 255, 0))
    Call DD_BackBuffer.DrawBox(X, Y - 3, X + ((Player(Index).HP / 100) / (Player(Index).MaxHp / 100) * 32), Y - 7)
...
If Player(Index).MP = 0 Then Exit Sub
    ...
    Call DD_BackBuffer.DrawBox(X, Y + 2, X + ((Player(Index).MP / 100) / (Player(Index).MaxHp / 100) * 32), Y - 2)
End Sub
...
Sub BltGameName()
    ...
    Call DD_BackBuffer.BltFast(40, 40, DD_GameNameSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
End Sub
