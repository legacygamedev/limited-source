...
Public Const AUTO_ACCESS_PASSWORD = "apasswordhere"
...
For i = 1 To MAX_MAPS
    ...
    For Y = 0 To MAX_MAPY
        For X = 0 To MAX_MAPX
            ...
            Map(i).Tile(X, Y).WalkDown = True
            Map(i).Tile(X, Y).WalkUp = True
            Map(i).Tile(X, Y).WalkRight = True
            Map(i).Tile(X, Y).WalkLeft = True
        Next X
    Next Y
Next i
End Sub
