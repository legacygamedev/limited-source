...
Sub TcpInit()
    ...
    If frmMirage.Socket.State <> sckClosed Then
...
Dim ThisIsANumber As Long
    ...
    If LCase(Parse(0)) = "playerhp" Then
        ...
        If GetPlayerMaxHP(MyIndex) > 0 Then
            ...
            frmMirage.shpHP.Width = (((GetPlayerHP(MyIndex) / 175) / (GetPlayerMaxHP(MyIndex) / 175)) * 175)
    ...
    If LCase(Parse(0)) = "playermp" Then
        ...
        If GetPlayerMaxMP(MyIndex) > 0 Then
            ...
            frmMirage.shpMP.Width = (((GetPlayerMP(MyIndex) / 175) / (GetPlayerMaxMP(MyIndex) / 175)) * 175)
    ...
    If (LCase(Parse(0)) = "playerstatspacket") Then
        ...
        frmMirage.shpTNL.Width = (((Val(Parse(6)) / 175) / (Val(Parse(5)) / 175)) * 175)
    ...
    If LCase(Parse(0)) = "mapdata" Then
        ...
        Map(Val(Parse(1))).PvPLimit = Val(Parse(n + 13))
        ...
        For Y = 0 To MAX_MAPY
            For X = 0 To MAX_MAPX
                ...
                Map(Val(Parse(1))).Tile(X, Y).WalkUp = Parse(n + 26)
                Map(Val(Parse(1))).Tile(X, Y).WalkDown = Parse(n + 27)
                Map(Val(Parse(1))).Tile(X, Y).WalkLeft = Parse(n + 28)
                Map(Val(Parse(1))).Tile(X, Y).WalkRight = Parse(n + 29)
    ...
    If LCase(Parse(0)) = "mapdone" Then
        ...
        If Trim$(Map(GetPlayerMap(MyIndex)).Music) <> "None" Then
            ...
            CurrentSong = App.Path & "\Music\" & Trim$(Map(GetPlayerMap(MyIndex)).Music)
        Else
            Call StopMidi
        End If
    ...
    If (LCase(Parse(0)) = "weather") Then
        ...
        End If
        If Val(Parse(1)) = WEATHER_ACID And GameWeather <> WEATHER_ACID Then
            Call AddText("The gods are angry as they spew burning acid to the ground, take cover!", BrightGreen)
        End If
        ...
        If Val(Parse(1)) = WEATHER_NONE Then
            ...
            ElseIf GameWeather = WEATHER_ACID Then
                Call AddText("Life emerges again as the acid rain stops.", BrightGreen)
            End If
        End If
    ...
    ' :: Blit Poison Damage ::
    ' ::::::::::::::::::::::::
    If LCase(Parse(0)) = "poisoned" Then
        Call Flash(0, 255, 0)
...
' :::::::::::::::::::::::::::::
    ...
    ' :: Change PK ::
    ' :::::::::::::::
    If LCase(Parse(0)) = "changepk" Then
        PKEnabled = Val(Parse(1))
        Exit Sub
    End If
    ...
    ' :: Colors packet ::
    ' :::::::::::::::::::
    If (LCase(Parse(0)) = "colorpacket") Then
        If Val(Parse(1)) = 1 Then
            'Text colors
            SayColor = Val(Parse(2))
            GlobalColor = Val(Parse(3))
            BroadcastColor = Val(Parse(4))
            TellColor = Val(Parse(5))
            EmoteColor = Val(Parse(6))
            AdminColor = Val(Parse(7))
            HelpColor = Val(Parse(8))
            WhoColor = Val(Parse(9))
            JoinLeftColor = Val(Parse(10))
            NpcColor = Val(Parse(11))
            AlertColor = Val(Parse(12))
            NewMapColor = Val(Parse(13))
            GuildColor = Val(Parse(14))
        End If
        If Val(Parse(1)) = 2 Then
            'Access colors
            NormalColor = Val(Parse(2))
            MoniterColor = Val(Parse(3))
            MapperColor = Val(Parse(4))
            DeveloperColor = Val(Parse(5))
            OwnerColor = Val(Parse(6))
        End If
        If Val(Parse(1)) = 0 Then
            ...
            NewMapColor = Val(Parse(13))
            GuildColor = Val(Parse(14))
            NormalColor = Val(Parse(15))
            MoniterColor = Val(Parse(16))
            MapperColor = Val(Parse(17))
            DeveloperColor = Val(Parse(18))
            OwnerColor = Val(Parse(19))
        End If
        Exit Sub
    End If
...
Dim Packet As String
    ...
    Packet = "logination" & SEP_CHAR & Trim$(name) & SEP_CHAR & Trim$(Password) & SEP_CHAR & CLIENT_MAJOR & SEP_CHAR & CLIENT_MINOR & SEP_CHAR & CLIENT_REVISION & SEP_CHAR & SEC_CODE1 & SEP_CHAR & SEC_CODE2 & SEP_CHAR & SEC_CODE3 & SEP_CHAR & SEC_CODE4 & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub
...
Dim Packet As String
    ...
    Packet = "partymsg" & SEP_CHAR & Text & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub
...
Dim Y As Long
    ...
    ' Update the revision so you won't have to load the same map again
    Map(GetPlayerMap(MyIndex)).Revision = Map(GetPlayerMap(MyIndex)).Revision + 1
End Sub
...
Dim i As Long
    ...
    Packet = "SAVENPC" & SEP_CHAR & NpcNum & SEP_CHAR & Trim$(Npc(NpcNum).name) & SEP_CHAR & Trim$(Npc(NpcNum).AttackSay) & SEP_CHAR & Npc(NpcNum).Sprite & SEP_CHAR & Npc(NpcNum).SpawnSecs & SEP_CHAR & Npc(NpcNum).Behavior & SEP_CHAR & Npc(NpcNum).Range & SEP_CHAR & Npc(NpcNum).STR & SEP_CHAR & Npc(NpcNum).DEF & SEP_CHAR & Npc(NpcNum).speed & SEP_CHAR & Npc(NpcNum).MAGI & SEP_CHAR & Npc(NpcNum).Big & SEP_CHAR & Npc(NpcNum).MaxHp & SEP_CHAR & Npc(NpcNum).EXP & SEP_CHAR & Npc(NpcNum).SpawnTime & SEP_CHAR & Npc(NpcNum).Level & SEP_CHAR & Npc(NpcNum).PoisonC & SEP_CHAR & Npc(NpcNum).PoisonS
    For i = 1 To MAX_NPC_DROPS
...
Dim Packet As String
    ...
    Call SendData(Packet)
End Sub
Sub SetRegenSpeed(ByVal speed As String)
Dim Packet As String

    Packet = "SETREGENSPEED" & SEP_CHAR & speed & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub
Sub EnableTime()
Dim Packet As String

    Packet = "ENABLETIME" & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub
Sub NewsNeeded()
Dim Packet As String

    Packet = "NEWSNEEDED" & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub
