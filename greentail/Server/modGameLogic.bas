...
Dim GetMaxDamage As Long
    ...
    GetBaseDamage = Int(GetPlayerSTR(index) / 2)
    GetMaxDamage = Int(GetPlayerSTR(index))
            ...
            If (MapItem(MapNum, I).X = GetPlayerX(index)) And (MapItem(MapNum, I).y = GetPlayerY(index)) Then
                ...
                If n <> 0 Then
                    ...
                    Else
                        ...
                        Msg = "You picked up a(n) " & Trim(Item(GetPlayerInvItemNum(index, n)).Name) & "."
                    End If
                ...
                If (GetPlayerY(Victim) + 1 = GetPlayerY(Attacker)) And (GetPlayerX(Victim) = GetPlayerX(Attacker)) Then
                    If Map(GetPlayerMap(Victim)).Tile(GetPlayerX(Victim), GetPlayerY(Victim)).Type <> TILE_TYPE_ARENA And Map(GetPlayerMap(Attacker)).Tile(GetPlayerX(Attacker), GetPlayerY(Attacker)).Type <> TILE_TYPE_ARENA Then
                        ...
                        If GetPlayerAccess(Attacker) > ADMIN_MONITER Then
                            Call PlayerMsg(Attacker, "You cannot attack any player; you are an admin!", BrightBlue)
                        Else
...
Function CanRangePlayer(ByVal Attacker As Long, ByVal Victim As Long) As Boolean
Dim AttackSpeed As Long
...
Function CanRangeNpc(ByVal Attacker As Long, ByVal MapNpcNum As Long) As Boolean
Dim MapNum As Long, NpcNum As Long
Dim AttackSpeed As Long
...
Dim Moved As Byte
    ...
    If Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index)).Type = TILE_TYPE_SELL Then
        Call SendDataTo(index, "opensell" & SEP_CHAR & END_CHAR)
    End If
...
Dim I As Long
    ...
    If GetVar(App.Path & "\Data.ini", "CONFIG", "HPRegen") = 1 Then
        ...
        If GetVar(App.Path & "\resting.ini", "STATUS", GetPlayerName(index)) = 1 Then
        I = Int(I * 1.75)
        End If
