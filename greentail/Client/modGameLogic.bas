...
If ReadINI("CLIENT", "Server", App.Path & "\client.ini") = 1 Then
    If LCase(Dir(App.Path & "\DevMaps", vbDirectory)) <> "devmaps" Then
        Call MkDir(App.Path & "\DevMaps")
    End If
    End If
    End If
    ...
    'Do the checks for my new commands.
    If ReadINI("CONFIG", "FPS", App.Path & "\config.ini") = "" Then
    WriteINI "CONFIG", "FPS", 1, App.Path & "\config.ini"
    End If
    If ReadINI("CONFIG", "MapDisp", App.Path & "\config.ini") = "" Then
    WriteINI "CONFIG", "MapDisp", 1, App.Path & "\config.ini"
    End If
    If ReadINI("CONFIG", "XYCoord", App.Path & "\config.ini") = "" Then
    WriteINI "CONFIG", "XYCoord", 1, App.Path & "\config.ini"
    End If
    If ReadINI("CONFIG", "Jump", App.Path & "\config.ini") = "" Then
    WriteINI "CONFIG", "Jump", 1, App.Path & "\config.ini"
    End If
    
    'Now the usual checks.
    If FileExist("config.ini") Then
        ...
        frmMirage.FPSCheck.value = ReadINI("CONFIG", "FPS", FileName)
        frmMirage.MapCheck.value = ReadINI("CONFIG", "MapDisp", FileName)
        frmMirage.XYCheck.value = ReadINI("CONFIG", "XYCoord", FileName)
    ...
    Else
        ...
        WriteINI "CONFIG", "WebSite", "http://www.steelw.com/", App.Path & "\config.ini"
    ...
    frmSpriteChange.picSprites.Picture = LoadPNG(App.Path & "\GFX\sprites.png")
...
Sub MenuState(ByVal State As Long)
    ...
    Select Case State
        ...
        Case MENU_STATE_ADDCHAR
            ...
            If ConnectToServer = True Then
                ...
                If frmNewChar.MaleFemale.ListIndex = 0 Then
    ...
    If Not IsConnected And Connucted = True Then
        frmMainMenu.Visible = True
        frmSendGetData.Visible = False
        Call MsgBox("The server is currently offline. Please try again in a few minutes.", vbOKOnly, GAME_NAME)
    End If
End Sub

Sub GameInit()
    frmMirage.Visible = True
    frmMirage.Checkwindow.Enabled = True
    frmSendGetData.Visible = False
    Call InitDirectX
                If Player(MyIndex).Access > 0 Then
frmMirage.CheatCheck.Enabled = False
Else
frmMirage.CheatCheck.Enabled = True
        End If
End Sub
...
Dim rec_back As RECT
    ...
    If ScreenMode = 0 Then
        ...
        ' Blit out players
        For i = 1 To MAX_PLAYERS
            If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) And GetPlayerGuild(i) <> "Stealth" Then
                Call BltPlayer(i)
            End If
        Next i
    ...
    If GettingMap = False Then
        If ScreenMode = 0 Then
If ReadINI("CONFIG", "FPS", App.Path & "\config.ini") = 1 And GetPlayerAccess(MyIndex) > 0 Then
Call DrawText(TexthDC, 36, 36, "FPS: " & GameFPS, QBColor(White))
End If
If ReadINI("CONFIG", "MapDisp", App.Path & "\config.ini") = 1 And GetPlayerAccess(MyIndex) > 0 Then
Call DrawText(TexthDC, 36, 46, "Map #: " & GetPlayerMap(MyIndex), QBColor(White))
End If
If ReadINI("CONFIG", "XYCoord", App.Path & "\config.ini") = 1 And GetPlayerAccess(MyIndex) > 0 Then
Call DrawText(TexthDC, 36, 56, "X: " & GetPlayerX(MyIndex) & ", Y: " & GetPlayerY(MyIndex), QBColor(White))
End If
If ReadINI("CONFIG", "Resting", App.Path & "\config.ini") = 1 Then
Call DrawText(TexthDC, 300, 250, "Resting...", QBColor(White))
ControlDown = False
End If
                ...
                For Y = 0 To MAX_MAPY
                    For x = 0 To MAX_MAPX
                        With Map(GetPlayerMap(MyIndex)).Tile(x, Y)
                            ...
                            If .Type = TILE_TYPE_CBLOCK Then Call DrawText(TexthDC, x * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "CB", QBColor(Grey))
                            ...
                            If .Type = TILE_TYPE_DOOR Then Call DrawText(TexthDC, x * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "D", QBColor(Grey))
                            ...
                            If .Type = TILE_TYPE_BANK Then Call DrawText(TexthDC, x * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "BANK", QBColor(Grey))
                            If .Type = TILE_TYPE_SELL Then Call DrawText(TexthDC, x * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, Y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "SELL", QBColor(White))
            ...
            ElseIf Map(GetPlayerMap(MyIndex)).Moral = MAP_MORAL_NO_PENALTY Then
                Call DrawText(TexthDC, Int((20.5) * PIC_X / 2) - (Int(Len(Trim(Map(GetPlayerMap(MyIndex)).name)) / 2) * 8) + sx, 2 + sx, Trim(Map(GetPlayerMap(MyIndex)).name), QBColor(Grey))
            End If
    ...
    End If
        ...
        If GetPlayerAccess(MyIndex) > 0 Then
            Call DrawText(TexthDC, 36, 76, "Receiving map...", QBColor(BrightCyan))
            Else
...
Dim a As Long
    ...
    'I added an On Error Resume Next, Alex. This will keep it from crashing because of speed... not sure what's causing it, though.
...
MyText = frmMirage.txtMyTextBox.Text
    ...
    frmMirage.txtMyTextBox.Text = ""
        ...
        If LCase(Mid(MyText, 1, 5)) = "/save" Then
            ...
            Call AddText("Character, stats, inventory, and bank saved.", Yellow)
            MyText = ""
            Exit Sub
        End If
        ...
        ' Decline GuildReq
        If LCase(Mid(MyText, 1, 5)) = "/gdec" Then
            Call SendData("decguild" & SEP_CHAR & END_CHAR)
            MyText = ""
            Exit Sub
        End If
        
        ' Accept GuildReq
        If LCase(Mid(MyText, 1, 5)) = "/gacc" Then
            Call SendData("guildaccept" & SEP_CHAR & END_CHAR)
            MyText = ""
            Exit Sub
        End If
...
Public Sub EditorInit()
    ...
    frmMapEditor.picBackSelect.Picture = LoadPNG(App.Path + "\GFX\tiles0.png")
        ...
        If frmMapEditor.MousePointer = 2 Then
            ...
            ElseIf frmMapEditor.mnuType(2).Checked = True Then
                With Map(GetPlayerMap(MyIndex)).Tile(X1, Y1)
                    ...
                    End If
                    If .Type = TILE_TYPE_BANK = True Then
                           frmAttributes.optBank.value = True
                    End If
                    If .Type = TILE_TYPE_SELL = True Then
                           frmAttributes.optSell.value = True
                    End If
                End With
            End If
...
Dim i As Long
    ...
    If (frmItemEditor.cmbType.ListIndex >= ITEM_TYPE_WEAPON) And (frmItemEditor.cmbType.ListIndex <= ITEM_TYPE_SHIELD) Then
        ...
        frmItemEditor.scrlSpeedReq.value = 0
        ...
        frmItemEditor.scrlAddSpeed.value = 0
...
Dim X1 As Long, Y1 As Long
    ...
    If Button = 2 Then
    'If ReadINI("CONFIG", "Jump", App.Path & "\config.ini") = 1 Then ' Don't know why you want this, but here it is..
        If GetPlayerAccess(MyIndex) < 3 Then Exit Sub
        If InEditor = True Then Exit Sub
        Call Transport(GetPlayerMap(MyIndex), X1, Y1)
        Exit Sub
    '  End If
    End If
End Sub
...
Dim strWords() As String
    ...
    For intLoop = 0 To (MAX_LINES - 1)
        If strLine(intLoop) <> "" Then
            Call DrawTextBubble(TexthDC, TextX - (NewPlayerX * PIC_X) + sx - NewXOffset + (((DISPLAY_BUBBLE_WIDTH) * PIC_X) / 2) - ((Len(strLine(intLoop)) * 8) \ 2) - 7, TextY - (NewPlayerY * PIC_Y) + sx - NewYOffset, strLine(intLoop), QBColor(Black), QBColor(White))
            TextY = TextY + 16
        End If
    Next intLoop
End Sub
...
If Player(index).HP = 0 Then Exit Sub
    ...
    Call DD_BackBuffer.DrawBox(x, Y + 5, x + 32, Y + 1)
    ...
    Call DD_BackBuffer.DrawBox(x, Y + 5, x + ((Player(index).MP / 100) / (Player(index).MaxMP / 100) * 32), Y + 1)
End Sub
...
For i = 1 To MAX_INV
On Error Resume Next
If GetPlayerInvItemValue(MyIndex, i) > 0 And Item(GetPlayerInvItemNum(MyIndex, i)).Type = ITEM_TYPE_CURRENCY Then
...
For i = 1 To MAX_INV
            frmBank.lstInventory.AddItem "Updating..."
    DoEvents
Next i

For i = 1 To MAX_BANK
            frmBank.lstBank.AddItem "Updating..."
    DoEvents
Next i
