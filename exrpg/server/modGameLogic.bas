...
Dim i As Long
    ...
    If i <> 0 And ItemNum >= 0 And ItemNum <= MAX_ITEMS Then
        ...
        packet = "SPAWNITEM" & SEP_CHAR & i & SEP_CHAR & ItemNum & SEP_CHAR & ItemVal & SEP_CHAR & X & SEP_CHAR & y & SEP_CHAR & END_CHAR
...
Dim i As Long
    ...
    If (GetPlayerInvItemNum(index, InvNum) > 0) And (GetPlayerInvItemNum(index, InvNum) <= MAX_ITEMS) Then
        i = FindOpenMapItemSlot(GetPlayerMap(index))
            ...
            Call SpawnItemSlot(i, MapItem(GetPlayerMap(index), i).Num, Ammount, GetPlayerMap(index), GetPlayerX(index), GetPlayerY(index))
        Else
...
Dim MapNum As Long, NpcNum As Long
    ...
    If IsPlaying(Attacker) Then
      ...
      CanAttackNpc = True
Else
      Call PlayerMsg(Attacker, Trim(Npc(NpcNum).Name) & ": " & Trim(Npc(NpcNum).AttackSay), BrightBlue)
End If
                    End If
...
Dim ShopNum As Long, OldMap As Long
    ...
    If ShopNum > 0 Then
        If Trim(Shop(ShopNum).JoinSay) <> "" Then
            Call PlayerMsg(index, Trim(Shop(ShopNum).Name) & ": " & Trim(Shop(ShopNum).JoinSay), SayColor)
        End If
    End If
