...
Dim GoldNeeded As Long
    ...
    If LCase(Parse(0)) = "login" Then
        ...
        Next I
            ...
            If Val(Parse(3)) < CLIENT_MAJOR Or Val(Parse(4)) < CLIENT_MINOR Or Val(Parse(5)) < CLIENT_REVISION Then
                Call AlertMsg(Index, "Version outdated, please visit " & GAME_NAME)
                Exit Sub
            End If
 ...
 ' Find out what kind of item it is
           Select Case Item(GetPlayerInvItemNum(Index, InvNum)).Type
               ...
               Case ITEM_TYPE_HELMET
                   ...
                   If InvNum <> GetPlayerShieldSlot(Index) Then
                       If Int(GetPlayerSTR(Index)) < n Then
                           Call PlayerMsg(Index, "Your strength  is to low to wear this sheild!  Required STR (" & n & ")", BrightRed)
                           Exit Sub
                       End If
    ...
    ' :: Ban list packet ::
    ' :::::::::::::::::::::
    If LCase(Parse(0)) = "listbans" Then
        ' Prevent hacking
        If GetPlayerAccess(Index) < ADMIN_MAPPER Then
            Call HackingAttempt(Index, "Admin Cloning")
            Exit Sub
   ...
   End If
    ...
    If LCase(Parse(0)) = "setmotd" Then
        ...
        Call PutVar(App.Path & "\Core Files\Configuration.ini", "message of the day", "MOTD", Parse(1))
...
Dim I As Long
    ...
    For I = 1 To MAX_MAP_ITEMS
        Packet = Packet & MapItem(MapNum, I).Num & SEP_CHAR & MapItem(MapNum, I).Value & SEP_CHAR & MapItem(MapNum, I).X & SEP_CHAR & MapItem(MapNum, I).y & SEP_CHAR
    Next I
    Packet = Packet & END_CHAR
...
Dim I As Long
    ...
    For I = 1 To MAX_INV
        Packet = Packet & GetPlayerInvItemNum(Index, I) & SEP_CHAR & GetPlayerInvItemValue(Index, I) & SEP_CHAR
    Next I
    Packet = Packet & END_CHAR
...
Dim Packet As String
    ...
    Packet = "PLAYERINVUPDATE" & SEP_CHAR & InvSlot & SEP_CHAR & GetPlayerInvItemNum(Index, InvSlot) & SEP_CHAR & GetPlayerInvItemValue(Index, InvSlot) & SEP_CHAR & END_CHAR
    Call SendDataTo(Index, Packet)
End Sub
...
Dim f As Long
    ...
    ' Send them MOTD
    MOTD = GetVar(App.Path & "\Core Files\motd.ini", "MOTD", "Msg")
    If Trim(MOTD) <> "" Then
...
For I = 0 To Max_Classes
Packet = Packet & GetClassName(I) & SEP_CHAR & GetClassMaxHP(I) & SEP_CHAR & GetClassMaxMP(I) & SEP_CHAR & GetClassMaxSP(I) & SEP_CHAR & Class(I).STR & SEP_CHAR & Class(I).DEF & SEP_CHAR & Class(I).SPEED & SEP_CHAR & Class(I).MAGI & SEP_CHAR & Class(I).MaleSprite & SEP_CHAR & Class(I).FemaleSprite & SEP_CHAR
Next I
Packet = Packet & END_CHAR
...
Dim I As Long
    ...
    Packet = "EDITSHOP" & SEP_CHAR & ShopNum & SEP_CHAR & Trim(Shop(ShopNum).Name) & SEP_CHAR & Trim(Shop(ShopNum).JoinSay) & SEP_CHAR & Trim(Shop(ShopNum).LeaveSay) & SEP_CHAR
    For I = 1 To MAX_TRADES
...
Dim I As Long, X As Long, y As Long
    ...
    Packet = "TRADE" & SEP_CHAR & ShopNum & SEP_CHAR
    For I = 1 To MAX_TRADES
        Packet = Packet & Shop(ShopNum).TradeItem(I).GiveItem & SEP_CHAR & Shop(ShopNum).TradeItem(I).GiveValue & SEP_CHAR & Shop(ShopNum).TradeItem(I).GetItem & SEP_CHAR & Shop(ShopNum).TradeItem(I).GetValue & SEP_CHAR
