...
Sub MenuState(ByVal State As Long)
    ...
    If Not IsConnected Then
        frmMainMenu.Visible = True
        frmSendGetData.Visible = False
        Call MsgBox("Sorry, it appears the " & (GAME_NAME) & " server is not online. Please visit the website at " & (GAME_WEBSITE) & " for more information.", vbOKOnly, GAME_NAME)
    End If
End Sub
...
frmMainGame.txtMyTextBox.SelStart = Len(frmMainGame.txtMyTextBox.Text) + 1
End If
...
TextY = MapNpc(Index).y * PIC_Y + MapNpc(Index).YOffset - CLng(PIC_Y / 2) - 7
...
Public Sub EditorInit()
    ...
    With frmMainGame.picBackSelect
        .Width = 7 * PIC_X
        .Height = 255 * PIC_Y
        .Picture = LoadPicture(App.Path + "\core files/graphics/Tiles.bmp")
    End With
End Sub
...
Public Sub EditorChooseTile(Button As Integer, Shift As Integer, X As Single, y As Single)
    ...
    End If
    Call BitBlt(frmMainGame.picSelect.hdc, 0, 0, PIC_X, PIC_Y, frmMainGame.picBackSelect.hdc, EditorTileX * PIC_X, EditorTileY * PIC_Y, SRCCOPY)
End Sub
...
On Error Resume Next
    ...
    frmItemEditor.picItems.Picture = LoadPicture(App.Path & "\core files/graphics/Items.bmp")
...
On Error Resume Next
    ...
    frmNpcEditor.picSprites.Picture = LoadPicture(App.Path & "\core files\graphics\sprites.bmp")
...
Sub BltHP()
    ...
    Call DD_BackBuffer.BltFast(10, 10, DD_HPSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
End Sub
...
Sub BltMP()
    ...
    Call DD_BackBuffer.BltFast(100, 10, DD_MPSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
End Sub
