...
Sub UpdateCaption()
    frmServer.Caption = "Maikone Engine Server // ( " & GAME_NAME & " )"
    frmServer.ServerBar.Panels(1).text = "Server IP: " & GetIP & " "
    frmServer.ServerBar.Panels(2).text = "Server Port: " & STR(GameServer.LocalPort) & " "
    frmServer.ServerBar.Panels(3).text = "Online Players: " & TotalOnlinePlayers & " "
End Sub
...
Dim DurNeeded As Long, GoldNeeded As Long
                ...
                If Not CanPlayerCriticalHit(Index) Then
                    n = Int((((GetPlayerDamage(Index) / 5) * 4) * Rnd) + 1)
                        If n < (n * 4 / 5) Then
                            n = (n * 4 / 5)
                        End If
                    Damage = n - Int(Npc(MapNpc(GetPlayerMap(Index), n).num).DEF / 2)
                Else
                    n = GetPlayerDamage(Index)
    ...
    If LCase(Parse(0)) = "banlist" Then
        ...
        n = 1
        f = FreeFile
        Open App.Path & "\System\Banlist.txt" For Input As #f
        Do While Not EOF(f)
            Input #f, s
            Input #f, Name
    ...
    If LCase(Parse(0)) = "bandestroy" Then
        ...
        Call Kill(App.Path & "\System\Banlist.txt")
        Call PlayerMsg(Index, "Banlist destroyed.", White)
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "setmotd" Then
        ...
        Call PutVar(App.Path & "\System\Config\MOTD.ini", "MOTD", "Msg", Parse(1))
...
Dim f As Long
    ...
    ' Send them MOTD
    MOTD = GetVar(App.Path & "\System\Config\MOTD.ini", "MOTD", "Msg")
    If Trim(MOTD) <> "" Then
