...
Sub UpdateCaption()
    frmServer.Caption = GAME_NAME & " - Server - Powered By Bevrias"
...
Dim BX As Long, BY As Long
    ...
    If LCase(Parse(0)) = "bankitem" Then
     Player(index).Char(Player(index).CharNum).bank(Parse(1)).num = Parse(2)
     If Parse(4) = "" Then Parse(4) = 0
     Player(index).Char(Player(index).CharNum).bank(Parse(1)).value = Parse(4)
     Player(index).Char(Player(index).CharNum).bank(Parse(1)).Dur = Parse(3)
     Exit Sub
     End If
    ...
    If LCase(Parse(0)) = "invitem" Then
     Player(index).Char(Player(index).CharNum).Inv(Parse(1)).num = Parse(2)
     Player(index).Char(Player(index).CharNum).Inv(Parse(1)).value = Parse(4)
     Player(index).Char(Player(index).CharNum).Inv(Parse(1)).Dur = Parse(3)
     Exit Sub
     End If
    ...
    If LCase(Parse(0)) = "requestbank" Then
      Packet = "bank" & SEP_CHAR & Parse(1) & SEP_CHAR & Player(index).Char(Player(index).CharNum).bank(Parse(1)).num & SEP_CHAR & Player(index).Char(Player(index).CharNum).bank(Parse(1)).value & SEP_CHAR & Player(index).Char(Player(index).CharNum).bank(Parse(1)).Dur & SEP_CHAR & END_CHAR
      Call SendDataTo(index, Packet)
     Exit Sub
     End If
...
' :: Strenght Increase                  ::
' ::::::::::::::::::::::::::::::::::::::::
If LCase(Parse(0)) = "strength" Then
'Secutirty 1 Start
...
End If
'Secutirty 1 End
strength = Val(Parse(1))
Call SetPlayerSTR(index, GetPlayerSTR(index) + strength)
Exit Sub
End If

' ::::::::::::::::::::::::::::::::::::::::
' :: 32*32 and 32*64 Char Window        ::
' ::::::::::::::::::::::::::::::::::::::::
If LCase(Parse(0)) = "charwindow" Then
Packet = ""
Packet = "charwindow" & SEP_CHAR & Size1 & SEP_CHAR & END_CHAR
Call SendDataToAll(Packet)
Exit Sub
End If

    ' :::::::::::::::::::::::::::::
    ' :: characters Stats packet ::
    ' :::::::::::::::::::::::::::::
    If LCase(Parse(0)) = "charstats" Then
       Call SendCharStats(index)
    End If
    ...
    Select Case LCase(Parse(0))
        ...
        Case "logination"
            If Not IsLoggedIn(index) Then
                ...
                Else
                    Call AlertMsg(index, "Cheat Attempt Blocked!")
                    Exit Sub
                End If
...
End If
    ...
    Case "mapdata"
        ...
        For y = 0 To MAX_MAPY
            ...
            'Map(MapNum).Tile(x, y).Data4 = Val(Parse(n + 26))
            'Map(MapNum).Tile(x, y).Data5 = Val(Parse(n + 27))
            n = n + 26
            Next x
        Next y
        ...
        If n > 0 Then
            ' Check to make sure they aren't the starter
                ' Check to make sure that each of there party players match
                    ...
                    If GetPlayerLevel(index) + GetVar(App.Path & "\Data.ini", "ADDED", "PARTYRANGELVL") < GetPlayerLevel(n) Or GetPlayerLevel(index) - GetVar(App.Path & "\Data.ini", "ADDED", "PARTYRANGELVL") > GetPlayerLevel(n) Then
                        Call PlayerMsg(index, "There is more then a" & GetVar(App.Path & "\Data.ini", "ADDED", "PARTYRANGELVL") & "level gap between you two, you will not share experience.", Pink)
                        Call PlayerMsg(n, "There is more then a" & GetVar(App.Path & "\Data.ini", "ADDED", "PARTYRANGELVL") & "level gap between you two, you will not share experience.", Pink)
...
For i = 1 To MAX_PLAYERS
    If IsPlaying(i) Then
        Packet = Packet & SEP_CHAR & GetPlayerName(i) & SEP_CHAR & GetPlayerLevel(i) & SEP_CHAR & Class(GetPlayerClass(i)).Name & SEP_CHAR
        n = n + 1
    End If
Next i
...
Dim Weapon As Long
    ...
    Next i
End Sub
Sub SendCharStats(ByVal index As Long)
Dim Packet As String
Dim i As Long
Dim TNL As Long

Packet = "CHARSTATS" & SEP_CHAR
For i = 1 To MAX_CHARS
TNL = Player(index).Char(i).Exp
  Packet = Packet & Trim(Player(index).Char(i).Name) & SEP_CHAR & Player(index).Char(i).STR & SEP_CHAR & Player(index).Char(i).DEF & SEP_CHAR & Player(index).Char(i).Speed & SEP_CHAR & Player(index).Char(i).Magi & SEP_CHAR & Player(index).Char(i).HP & SEP_CHAR & Player(index).Char(i).MP & SEP_CHAR & Player(index).Char(i).SP & SEP_CHAR & TNL & SEP_CHAR & Player(index).Char(i).Level & SEP_CHAR & Class(GetPlayerClass(index)).Name & SEP_CHAR & Player(index).Char(i).POINTS & SEP_CHAR
Next i
  Packet = Packet & END_CHAR
Call SendDataTo(index, Packet)
End Sub
Sub SendBank(ByVal index As Long)
Call SendDataTo(index, "bankok" & SEP_CHAR & END_CHAR)
End Sub
