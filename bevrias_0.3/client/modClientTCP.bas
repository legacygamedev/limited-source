...
Dim z As Long
Dim CLA As String
Dim ST As Long, DE As Long, SP As Long, MA As Long, HP As Long, MP As Long, SS As Long, TNL As Long, LVL As Long, POI As Long
Dim p As Long
    ' Handle Data
    Parse = Split(Data, SEP_CHAR)
    
    ' ::::::::::::::::::::::::::
    ' :: Banker update packet ::
    ' ::::::::::::::::::::::::::
    If LCase(Parse(0)) = "bankok" Then
    Call bankup
        Exit Sub
    End If
    ...
    Player(MyIndex).bank(Parse(1)).Num = Parse(2)
    Player(MyIndex).bank(Parse(1)).Value = Parse(3)
    Player(MyIndex).bank(Parse(1)).dur = Parse(4)
        Exit Sub
    End If
...
' ::::::::::::::::::::::::::::::::::::::::
' :: Character Sizes Packet             ::
' ::::::::::::::::::::::::::::::::::::::::
If LCase(Parse(0)) = "sizeofsprites" Then
size1 = Val(Parse(1))
Exit Sub
End If
...
' ::::::::::::::::::::::::::::::::::::::::
    ...
    Dim m As Long
     m = frmChars.lstChars.ListIndex + 1
     n = frmChars.lstChars.ListIndex
     If n >= 1 Then m = (12 * n) + 1
       Name = Parse(m)
       If Name = "" Then
          Name = "No Character"
       End If
       ...
       CLA = Parse((m) + 10)
       POI = Parse((m) + 11)
       frmChars.CharStats.Caption = Name
       frmChars.CharStat.Caption = ST
       frmChars.CharStat1.Caption = DE
       frmChars.CharStat2.Caption = SP
       frmChars.CharStat3.Caption = MA
       frmChars.CharStat4.Caption = HP
       frmChars.CharStat5.Caption = MP
       frmChars.CharStat6.Caption = SS
       frmChars.CharStat7.Caption = TNL
       frmChars.CharStat8.Caption = LVL
       frmChars.CharStat9.Caption = CLA
       If CLA = "" Then
       CLA = "No Class"
       End If
       frmChars.CharStat10.Caption = POI
    End If
    ...
    If (LCase(Parse(0)) = "playerstatspacket") Then
       ...
       ' If frmMirage.shpTNL.Width > 117 Then
       ' frmMirage.shpTNL.Width = 118
        'End If
    ...
    If LCase(Parse(0)) = "mapdata" Then
        ...
        For y = 0 To MAX_MAPY
            For x = 0 To MAX_MAPX
                ...
                'Map(Val(Parse(1))).Tile(x, y).Data4 = Val(Parse(n + 26))
                'Map(Val(Parse(1))).Tile(x, y).Data5 = Val(Parse(n + 27))
                n = n + 26
            Next x
        Next y
    ...
    If (LCase(Parse(0)) = "spells") Then
        ...
        frmMirage.Picture30.Visible = True
    ...
    frmMirage.lstOnline.Clear
        ...
        z = Val(Parse(1))
        For x = n To (z + 1)
            frmMirage.lstOnline.AddItem Val(Parse(1)) & "." & Trim(Parse(n)) & "  " & "Level: " & Val(Parse(3)) & "  " & "Class: " & Trim(Parse(4))
            n = n + 2
        Next x
        Exit Sub
    End If
...
Dim Packet As String
    ...
    Packet = "CHARSTATS" & SEP_CHAR & index & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub
Sub SendBank(ByVal bankNum As Integer)
Dim Packet As String
    Packet = "bankitem" & SEP_CHAR & bankNum & SEP_CHAR & Player(MyIndex).bank(bankNum).Num & SEP_CHAR & Player(MyIndex).bank(bankNum).dur & SEP_CHAR & Player(MyIndex).bank(bankNum).Value & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub

Sub SendInv(ByVal invNum As Integer)
Dim Packet As String
    Packet = "invitem" & SEP_CHAR & invNum & SEP_CHAR & Player(MyIndex).Inv(invNum).Num & SEP_CHAR & Player(MyIndex).Inv(invNum).dur & SEP_CHAR & Player(MyIndex).Inv(invNum).Value & SEP_CHAR & END_CHAR
    Call SendData(Packet)
End Sub

Sub SendRequestBank(ByVal bankNum As Integer)
Call SendData("requestbank" & SEP_CHAR & bankNum & SEP_CHAR & END_CHAR)
End Sub
