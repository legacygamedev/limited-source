...
Public Const VK_TAB = &H9 'keyboard layout number and variable just look up the key code numbers and change the 9 to which ever number you want the key
...
Public ChestNum As Long
Public ChestValue As Long
...
Public HealthDamage As Long
Public ManaDamage As Long
Public StaminaDamage As Long
Public ExpDamage As Long
Public LevelDamage As Long
Public TextDamage As String
...
If size1 = 0 Then
        ' Blit out players 32*32
        For i = 1 To MAX_PLAYERS
            If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
                Call BltPlayer3232(i)
            End If
        Next i
End If
...
If size1 = 2 Then
        ...
        For i = 1 To MAX_PLAYERS
            If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
                Call BltPlayerPaperdoll(i)
            End If
        Next i
End If
        
If size1 = 1 Then
        ' Blit out players 32*64
        For i = 1 To MAX_PLAYERS
            If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
                Call BltPlayer3264(i)
            End If
        Next i
End If
...
End If
                ...
                For y = 0 To MAX_MAPY
                    For x = 0 To MAX_MAPX
                        With Map(GetPlayerMap(MyIndex)).Tile(x, y)
                            ...
                            If .Type = TILE_TYPE_QUEST Then Call DrawText(TexthDC, x * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "Q", QBColor(Black))
                            If .Type = TILE_TYPE_TRAP Then Call DrawText(TexthDC, x * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "T", QBColor(Brown))
    ...
    FileName = App.Path & "\config.ini"
        ...
        If ReadINI("GAMESPEED", "FPS", FileName) = 0 Then
        ' Lock fps
        ...
        If ReadINI("GAMESPEED", "FPS", FileName) = 1 Then
        ' Lock fps
        ...
        If ReadINI("GAMESPEED", "FPS", FileName) = 2 Then
        ' Lock fps
...
Dim M2AnimTileSet As Byte
    ...
    If (MapAnim = 0) Or (Anim2 <= 0) Then
        ...
        If Anim1 > 0 And TempTile(x, y).DoorOpen = NO Then
            If TileFile(MaskTileSet) = 0 Then
                Exit Sub
    End If
    ...
    Else
        ...
        If Anim2 > 0 Then
            If TileFile(AnimTileSet) = 0 Then
                Exit Sub
    End If
    ...
    Else
        ' Is there a second animation tile to plot?
        If M2Anim > 0 Then
            If TileFile(M2AnimTileSet) = 0 Then
                Exit Sub
    End If
...
Sub BltPlayerPaperdoll(ByVal index As Long)
...
Sub BltPlayer3232(ByVal index As Long)
Dim Anim As Byte
Dim x As Long, y As Long
Dim AttackSpeed As Long
...
Sub BltPlayer3264(ByVal index As Long)
Dim Anim As Byte
Dim x As Long, y As Long
Dim AttackSpeed As Long
...
Sub BltPlayerTop3264(ByVal index As Long)
Dim Anim As Byte
Dim x As Long, y As Long
Dim AttackSpeed As Long
...
If index = MyIndex Then
    ...
    If size1 = 0 Then
    TextY = NewY + sx - Int(PIC_Y / 2 - 3)
    End If
    If size1 = 1 Then
    TextY = NewY + sx - Int(PIC_Y / 2 + 26)
    End If
    If size1 = 2 Then
    TextY = NewY + sx - Int(PIC_Y / 2 - 3)
    End If
...
If index = MyIndex Then
    ...
    If size1 = 1 Then
    TextY = NewY + sx - Int(PIC_Y / 4) - 48
    End If
    If size1 = 0 Then
    TextY = NewY + sx - Int(PIC_Y / 4) - 18
    End If
...
MyText = frmMirage.txtMyTextBox.Text
    ...
    frmMirage.txtMyTextBox.Text = ""
        ...
        ' Map inn
        If LCase(Mid(MyText, 1, 10)) = "/sleep" Then
        Call SendInnSleep
        MyText = ""
        Exit Sub
        End If
...
Sub CheckMovement()
    If GettingMap = False Then
        If IsTryingToMove Then
            If CanMove Then
                         frmMirage.picBanker.Visible = False
             frmMirage.picAmmount.Visible = False
             frmMirage.picAmmountBank.Visible = False
        ...
        If frmMapEditor.MousePointer = 2 Then
            ...
            ElseIf frmMapEditor.mnuType(2).Checked = True Then
                With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
                    ...
                    If .Type = TILE_TYPE_QUEST Then
                        QuestNum = .Data1
                        frmAttributes.optQuest.Value = True
                    End If
                    ...
                    If .Type = TILE_TYPE_TRAP Then
                        ...
                        LevelDamage = .Data5
                        TextDamage = .String1
                        frmAttributes.optTrap.Value = True
                    End If
                    ...
                    frmAttributes.optChest.Value = True
                        ChestNum = .Data1
                        ChestValue = .Data2
                        ChestText = .String1
                    End If
                ...
                If frmMapEditor.shpSelected.Height <= PIC_Y And frmMapEditor.shpSelected.Width <= PIC_X Then
                    ...
                    ElseIf frmMapEditor.mnuType(2).Checked = True Then
                        With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
                            ...
                            If frmAttributes.optQuest.Value = True Then
                            ...
                            If frmAttributes.optTrap.Value = True Then
                                .Type = TILE_TYPE_TRAP
                                .Data1 = HealthDamage
                                .Data2 = ManaDamage
                                .Data3 = StaminaDamage
...
Dim index As Long
    ...
    Call UpdateShopTrade(index)
    
    frmShopEditor.Show vbModal
End Sub

Public Sub UpdateShopTrade(ByVal index As Long)
Dim i As Long, GetItem As Long, GetValue As Long, GiveItem As Long, GiveValue As Long, C As Long, ShopNum As Long

If Shop(ShopNum).bank = True Then
Call SendBank(index)
Exit Sub
End If
    ...
    Next i
            If Shop(EditorIndex).bank = True Then
    frmShopEditor.Frame2.Visible = False
frmShopEditor.SSTab1.Visible = False
frmShopEditor.chkBank.Value = 1
Else
frmShopEditor.chkBank.Value = 0
End If
...
Public Sub ShopEditorOk()
    ...
    If frmShopEditor.chkBank.Value = 1 Then
    Shop(EditorIndex).bank = True
    Else
    Shop(EditorIndex).bank = False
    End If
...
y = (GetPlayerY(index) * PIC_Y + sx + Player(index).YOffset) - (NewPlayerY * PIC_Y) - NewYOffset + 35
...
If Player(index).HP = 0 Then Exit Sub
    ...
    'draws MP
    'Call DD_BackBuffer.SetFillColor(RGB(0, 150, 255))
    'Call DD_BackBuffer.DrawBox(x, y - 2, x + ((Player(index).MP / 100) / (Player(index).MaxMP / 100) * 32), y - 6)
...
'Draw name
TextX = MapItem(index).x * PIC_X + MapItem(index).x + CLng(PIC_X / 2) - ((Len(Trim$(.Name)) / 2))
TextY = MapItem(index).y * PIC_Y + MapItem(index).y - CLng(PIC_Y / 2) - 4
DrawText TexthDC, TextX, TextY, Trim$(.Name), vbYellow
End With
End Sub
Sub bankup()
Dim i As Integer
For i = 1 To 49
Call SendRequestBank(i)
Next i
frmMirage.picBanker.Visible = True
