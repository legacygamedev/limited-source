...
Public Const TILE_TYPE_QUEST = 21
Public Const TILE_TYPE_TRAP = 22
...
For i = 1 To MAX_MAPS
    ...
    For y = 0 To MAX_MAPY
        For x = 0 To MAX_MAPX
            ...
            'Map(i).Tile(x, y).Data4 = 0
            'Map(i).Tile(x, y).Data5 = 0
...
Function GetPlayerBankItemNum(ByVal bankNum As Integer) As Long
GetPlayerBankItemNum = Player(MyIndex).bank(bankNum).Num
End Function

Function GetPlayerBankItemValue(ByVal bankNum As Integer) As Long
GetPlayerBankItemValue = Player(MyIndex).bank(bankNum).Value
End Function


Function SetPlayerBankItem(ByVal bankNum As Integer, ByVal Num As Integer, ByVal dur As Integer, ByVal Value As Integer)
Player(MyIndex).bank(bankNum).Num = Num
Player(MyIndex).bank(bankNum).dur = dur
Player(MyIndex).bank(bankNum).Value = Value
Call SendBank(bankNum)
End Function

Function SetPlayerBankItemValue(ByVal bankNum As Integer, ByVal Value As Integer)
Player(MyIndex).bank(bankNum).Value = Value
End Function

Function SetPlayerInvItem(ByVal bankNum As Integer, ByVal Num As Integer, ByVal dur As Integer, ByVal Value As Integer)
Player(MyIndex).Inv(bankNum).Num = Num
Player(MyIndex).Inv(bankNum).dur = dur
Player(MyIndex).Inv(bankNum).Value = Value
Call SendInv(bankNum)
End Function

Function GetOpenBankSlot(ByVal ItemNum As Integer) As Integer
Dim i As Integer
If Item(ItemNum).Type = ITEM_TYPE_CURRENCY Then
For i = 1 To 49
If GetPlayerBankItemNum(i) = ItemNum Then
  GetOpenBankSlot = i
Exit Function
End If
Next i
End If
For i = 1 To 49
If GetPlayerBankItemNum(i) = 0 Then
  GetOpenBankSlot = i
Exit Function
End If
Next i
...
Function GetOpenInvSlot(ByVal ItemNum As Integer) As Integer
Dim i As Integer
If Item(ItemNum).Type = ITEM_TYPE_CURRENCY Then
For i = 1 To MAX_INV
...
Sub BankItem(ByVal index As Long, Optional Value As Long)
Dim slot As Integer
If GetPlayerWeaponSlot(MyIndex) = index Then Exit Sub
If GetPlayerArmorSlot(MyIndex) = index Then Exit Sub
If GetPlayerShieldSlot(MyIndex) = index Then Exit Sub
If GetPlayerHelmetSlot(MyIndex) = index Then Exit Sub
slot = GetOpenBankSlot(Player(MyIndex).Inv(index).Num)
If Value = 0 Then
   Call SetPlayerBankItem(slot, Player(MyIndex).Inv(index).Num, Player(MyIndex).Inv(index).dur, 1)
   Call SetPlayerInvItem(index, 0, 0, 0)
Else
   Call SetPlayerBankItem(slot, Player(MyIndex).Inv(index).Num, Player(MyIndex).Inv(index).dur, Player(MyIndex).bank(slot).Value + Value)
   Call SetPlayerInvItem(index, Player(MyIndex).Inv(index).Num, Player(MyIndex).Inv(index).dur, Player(MyIndex).Inv(index).Value - Value)
   If GetPlayerInvItemValue(MyIndex, index) = 0 Then
         Call SetPlayerInvItem(index, 0, 0, 0)
   End If
End If
Call SendInv(index)
End Sub
Sub InvItem(ByVal index As Long, Optional Value As Long)
Dim slot As Integer
slot = GetOpenInvSlot(Player(MyIndex).bank(index).Num)
If Value = 0 Then
   Call SetPlayerInvItem(slot, Player(MyIndex).bank(index).Num, Player(MyIndex).bank(index).dur, 1)
   Call SetPlayerBankItem(index, 0, 0, 0)
Else
   Call SetPlayerInvItem(slot, Player(MyIndex).bank(index).Num, Player(MyIndex).bank(index).dur, Player(MyIndex).Inv(slot).Value + Value)
   Call SetPlayerBankItem(index, Player(MyIndex).bank(index).Num, Player(MyIndex).bank(index).dur, Player(MyIndex).bank(index).Value - Value)
   If GetPlayerBankItemValue(index) = 0 Then
         Call SetPlayerBankItem(index, 0, 0, 0)
   End If
End If
Call SendBank(index)
