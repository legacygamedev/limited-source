        ...
        End If
            ...
            If ConnectToServer = True Then
                ...
                If frmNewChar.optMale.Value = True Then
                    Call SendAddChar(frmNewChar.txtName, 0, frmNewChar.cmbClass.ListIndex + 1, frmChars.lstChars.ListIndex + 1, 1)
                   End If
                Else
    ...
    If GettingMap = False Then
        If ScreenMode = 0 Then
           ...
           ' If Val(GetVar(App.Path & "\Main\Config\config.ini", "CONFIG", "FPS")) = 1 Then
            Call DrawText(TexthDC, 5, 5, "FPS: " & GameFPS, QBColor(BrightCyan))
           ' End If
...
Sub BltItem(ByVal itemnum As Long)
    ...
    'Call DD_middlebuffer.Blt(rec_pos, DD_ItemSurf, rec, DDBLT_WAIT Or DDBLT_KEYSRC)
    Call DD_MiddleBuffer.BltFast((MapItem(itemnum).x - NewPlayerX) * PIC_X + sx - NewXOffset, (MapItem(itemnum).y - NewPlayerY) * PIC_Y + sx - NewYOffset, DD_ItemSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
End Sub
...
Dim AttackSpeed As Long
    ...
    If PAPERDOLL = 1 Then
        If Player(Index).WeaponNum > 0 Then
            ...
            rec.Bottom = rec.Top + PIC_Y
            Call DD_MiddleBuffer.BltFast(x - (NewPlayerX * PIC_X) - NewXOffset, y - (NewPlayerY * PIC_Y) - NewYOffset, DD_ItemSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
        End If
...
End If
    ...
    Call DD_MiddleBuffer.BltFast(x - (NewPlayerX * PIC_X) - NewXOffset, y - (NewPlayerY * PIC_Y) - NewYOffset, DD_SpriteSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
...
Dim x As Long, y As Long
    ...
    If Npc(MapNpc(MapNpcNum).num).Big = 0 Then
        ...
        'Call DD_middlebuffer.Blt(rec_pos, DD_SpriteSurf, rec, DDBLT_WAIT Or DDBLT_KEYSRC)
        Call DD_MiddleBuffer.BltFast(x - (NewPlayerX * PIC_X) - NewXOffset, y - (NewPlayerY * PIC_Y) - NewYOffset, DD_SpriteSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
    Else
        ...
        Call DD_MiddleBuffer.BltFast(x - (NewPlayerX * PIC_X) - NewXOffset, y - (NewPlayerY * PIC_Y) - NewYOffset, DD_BigSpriteSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
    End If
End Sub
...
If TileFile(TileSet) = 0 Then Exit Sub
    ...
    rec.Right = rec.Left + PIC_X
    Call DD_MiddleBuffer.BltFast(x - (NewPlayerX * PIC_X) + sx - NewXOffset, y - (NewPlayerY * PIC_Y) + sx - NewYOffset, DD_TileSurf(TileSet), rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
End Sub
...
Dim x2 As Long, y2 As Long
    ...
    If Map(GetPlayerMap(MyIndex)).Tile(x, y).Type = TILE_TYPE_SPRITE_CHANGE Then
        ...
        Call DD_MiddleBuffer.BltFast(x2 - (NewPlayerX * PIC_X) - NewXOffset, y2 - (NewPlayerY * PIC_Y) - NewYOffset, DD_SpriteSurf, rec, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
    End If
End Sub
