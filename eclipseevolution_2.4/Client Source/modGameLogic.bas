...
Public NPCWhoX As Long
Public NPCWhoY As Long
...
Sub BltPlayer(ByVal Index As Long)
  ...
  Dim attack_item As Long
    ...
    ' Check to see if we want to stop making him attack '//!! The animation was doubled then the time it appeared on Right/Down, had to apply this offset change below

    If Player(Index).AttackTimer + ((AttackSpeed - 100) \ 2) < GetTickCount Then
...
Sub BltTile(ByVal x As Long, ByVal y As Long)
  ...
  Dim M2AnimTileSet As Byte
    ...
    Call DD_BackBuffer.BltFast((x - NewPlayerX) * PIC_X - NewXOffset, (y - NewPlayerY) * PIC_Y - NewYOffset, DD_TileSurf(GroundTileSet), rec, DDBLTFAST_WAIT)
  ...
  Dim d As Long
    ...
    If DirUp Then
        ...
        If GetPlayerY(MyIndex) > 0 Then
            ...
            ' Check to see if tile is guild block

            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_GUILDBLOCK And Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).String1 <> GetPlayerGuild(MyIndex) Then
                CanMove = False
            End If
            ...
            ' Check to see if a player is already on that tile, unless it's a walkthru

            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_WALKTHRU Then
                Exit Function
             Else
    ...
    If DirLeft Then
        ...
        If GetPlayerX(MyIndex) > 0 Then
            ...
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_GUILDBLOCK And Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).String1 <> GetPlayerGuild(MyIndex) Then
                CanMove = False
            End If
            ...
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_WALKTHRU Then
                Exit Function
             Else
    ...
    If DirRight Then
        ...
        If GetPlayerX(MyIndex) < MAX_MAPX Then
            ...
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex)).Type = TILE_TYPE_WALKTHRU Then
                Exit Function
             Else
...
Public Sub CanonShoot(Button As Integer, Shift As Integer, x As Single, y As Single)
  ...
  Dim y1 As Long
    ...
    x1 = Int(x / PIC_X)
    y1 = Int(y / PIC_Y)
    Call SendData(PacketID.CanonShoot & SEP_CHAR & x1 & SEP_CHAR & y1 & SEP_CHAR & END_CHAR)
...
Sub CheckAttack()
  ...
  Dim AttackSpeed As Long
    ...
    If ControlDown = True And Player(MyIndex).AttackTimer + AttackSpeed < GetTickCount And Player(MyIndex).Attacking = 0 Then
        ...
        Call SendData(PacketID.Attack & SEP_CHAR & END_CHAR)
    End If
...
Sub CheckMapGetItem()
    ...
    If GetTickCount > Player(MyIndex).MapGetTimer + 250 And Trim$(MyText) = vbNullString Then
        ...
        Call SendData(PacketID.MapGetItem & SEP_CHAR & END_CHAR)
    End If
    ...
    If GettingMap = False Then
        If IsTryingToMove Then
            If CanMove Then
                ...
                If frmNewShop.Visible = True Then frmNewShop.Visible = False
    ...
    Do While InGame
        ...
        If GettingMap = False Then
            If ScreenMode = 0 Then
                ...
                '//!! Replaced MapNPC(NPCWho).X/Y with NPCWhoX/Y to preserve the location the damage was made
                            ...
                            If frmMirage.chknpcname.Value = 0 Then
                                If Npc(MapNpc(NPCWho).num).Big = 0 Then
                                    If GetTickCount < DmgTime + 2000 Then
                                        Call DrawText(TexthDC, (NPCWhoX - NewPlayerX) * PIC_X + sx + (Int(Len(DmgDamage)) / 2) * 3 - NewXOffset, (NPCWhoY - NewPlayerY) * PIC_Y + sx - 20 - NewYOffset - iii, DmgDamage, QBColor(White))
                                    End If

                                 Else

                                    If GetTickCount < DmgTime + 2000 Then
                                        Call DrawText(TexthDC, (NPCWhoX - NewPlayerX) * PIC_X + sx + (Int(Len(DmgDamage)) / 2) * 3 - NewXOffset, (NPCWhoY - NewPlayerY) * PIC_Y + sx - 47 - NewYOffset - iii, DmgDamage, QBColor(White))
                                    End If
                             ...
                             Else
                                ...
                                If Npc(MapNpc(NPCWho).num).Big = 0 Then
                                    If GetTickCount < DmgTime + 2000 Then
                                        Call DrawText(TexthDC, (NPCWhoX - NewPlayerX) * PIC_X + sx + (Int(Len(DmgDamage)) / 2) * 3 - NewXOffset, (NPCWhoY - NewPlayerY) * PIC_Y + sx - 30 - NewYOffset - iii, DmgDamage, QBColor(White))
                                    End If

                                 Else

                                    If GetTickCount < DmgTime + 2000 Then
                                        Call DrawText(TexthDC, (NPCWhoX - NewPlayerX) * PIC_X + sx + (Int(Len(DmgDamage)) / 2) * 3 - NewXOffset, (NPCWhoY - NewPlayerY) * PIC_Y + sx - 57 - NewYOffset - iii, DmgDamage, QBColor(White))
                                    End If
  ...
  Dim i As Long
    ...
    If (KeyAscii = vbKeyReturn) Then
        ...
        If LCase(Mid(MyText, 1, 5)) = "/info" Then
            ...
            Call SendData(PacketID.PlayerInfoRequest & SEP_CHAR & ChatText & SEP_CHAR & END_CHAR)
            MyText = vbNullString
            Exit Sub
        End If
        ...
        If LCase(Mid(MyText, 1, 6)) = "/stats" Then
            Call SendData(PacketID.GetStats & SEP_CHAR & END_CHAR)
            MyText = vbNullString
            Exit Sub
        End If
        ...
        If LCase(Mid(MyText, 1, 8)) = "/refresh" Then
            Call SendData(PacketID.Refresh & SEP_CHAR & END_CHAR)
            MyText = vbNullString
            Exit Sub
        End If
        ...
        If LCase(Mid(MyText, 1, 12)) = "/chatdecline" Then
            Call SendData(PacketID.DChat & SEP_CHAR & END_CHAR)
            MyText = vbNullString
            Exit Sub
        End If
        ...
        If LCase(Mid(MyText, 1, 5)) = "/chat" Then
            Call SendData(PacketID.AChat & SEP_CHAR & END_CHAR)
            MyText = vbNullString
            Exit Sub
        End If
        ...
        If GetPlayerAccess(MyIndex) > 0 Then
            ...
            If LCase(Mid(MyText, 1, 8)) = "/weather" Then
                If Len(MyText) > 8 Then
                    ...
                    If IsNumeric(MyText) = True Then
                        Call SendData(PacketID.Weather & SEP_CHAR & val#(MyText) & SEP_CHAR & END_CHAR)
                     Else
            ...
            If LCase(Mid(MyText, 1, 11)) = "/clearowner" Then
                Call SendData(PacketID.ClearOwner & SEP_CHAR & END_CHAR)
                MyText = vbNullString
                Exit Sub
            End If
        ...
        If GetPlayerAccess(MyIndex) >= 0 Then '//!!  ADMIN_MAPPER Then
            ' Location
            ...
            ' Reboot the server
            '//!! Didn't find any reference to this packet on the server
        ...
        If GetPlayerAccess(MyIndex) >= ADMIN_DEVELOPER Then
            ...
            If LCase(Mid(MyText, 1, 9)) = "/daynight" Then
                Call SendData(PacketID.DayNight & SEP_CHAR & END_CHAR)
                MyText = vbNullString
                Exit Sub
            End If
        ...
        If Left$(Trim$(MyText), 1) = "/" Then
            ...
            For i = 0 To MAX_EMOTICONS
                ...
                If Trim$(Emoticons(i).Command) = Trim$(MyText) And Trim$(Emoticons(i).Command) <> "/" Then
                    Call SendData(PacketID.CheckEmoticons & SEP_CHAR & i & SEP_CHAR & END_CHAR)
                    MyText = vbNullString
                    Exit Sub
                End If

            Next i
            Call SendData(PacketID.CheckCommands & SEP_CHAR & MyText & SEP_CHAR & END_CHAR)
            MyText = vbNullString
            Exit Sub
        End If
...
configErr:
    Call MsgBox("Error reading from config.ini. Make sure all variables are valid! Err: " & Err.Number & ", Desc: " & Err.Description, vbCritical)
    End
...
Public Sub MainMenuInit()
  ...
  Dim Blue As Integer
    ...
    frmMainMenu.picNews.ForeColor = RGB(Red, Green, Blue)
    ...
    Select Case State
     Case MENU_STATE_NEWACCOUNT
        ...
        If ConnectToServer = True Then
            ...
            If frmNewAccount.txtEmail.Text = "" And frmNewAccount.txtEmail.Visible = False Then
...
Public Sub NpcEditorBltSprite()
    ...
    If frmNpcEditor.BigNpc.Value = Checked Then
     ...
     Else
        ...
        If Spritesize = 1 Then
            '//!! There was 2 "* 64"s located in here that changing to 32 seemed to fix things - either that was the problem or that
            '     the spritesize = 1 when it shouldn't be
            frmNpcEditor.picSprites.Top = frmNpcEditor.scrlSprite.Value * 32
            Call BitBlt(frmNpcEditor.picSprite.hDC, 0, 0, PIC_X, 64, frmNpcEditor.picSprites.hDC, 3 * PIC_X, frmNpcEditor.scrlSprite.Value * 32, SRCCOPY)
         Else
...
Public Sub ParseNews()
  ...
  Dim ret
    ...
    'Set news font & size
    On Error Resume Next
    frmMainMenu.picNews.Font = ReadINI("FONT", "Font", App.Path & "\News.ini")
    frmMainMenu.picNews.fontsize = val(ReadINI("FONT", "Font", App.Path & "\News.ini"))
...
Sub PlayerSearch(Button As Integer, Shift As Integer, x As Single, y As Single)
  ...
  Dim y1 As Long
    ...
    If (x1 >= 0) And (x1 <= MAX_MAPX) And (y1 >= 0) And (y1 <= MAX_MAPY) Then
        ...
        Call SendData(PacketID.Search & SEP_CHAR & x1 & SEP_CHAR & y1 & SEP_CHAR & END_CHAR)
    End If
...
Sub SendGameTime()
  ...
  Dim packet As String
    ...
    packet = PacketID.GMTime & SEP_CHAR & GameTime & SEP_CHAR & END_CHAR
