                ...
                If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
                    If GetPlayerX(i) = x And GetPlayerY(i) = y Then
                        If Index = MyIndex Then
                            Call SendData(PacketID.ArrowHit & SEP_CHAR & 0 & SEP_CHAR & i & SEP_CHAR & x & SEP_CHAR & y & SEP_CHAR & END_CHAR)
                        End If
            ...
            For i = 1 To MAX_MAP_NPCS
                ...
                If MapNpc(i).num > 0 Then
                    If MapNpc(i).x = x And MapNpc(i).y = y Then
                        If Index = MyIndex Then
                            Call SendData(PacketID.ArrowHit & SEP_CHAR & 1 & SEP_CHAR & i & SEP_CHAR & x & SEP_CHAR & y & SEP_CHAR & END_CHAR)
                        End If
                    ...
                    If Map(GetPlayerMap(MyIndex)).Tile(BX, BY).Type = TILE_TYPE_NPC_SPAWN Then
                        ...
                        For i = 1 To MAX_ATTRIBUTE_NPCS
                            ...
                            If MapAttributeNpc(i, BX, BY).x = x And MapAttributeNpc(i, BX, BY).y = y Then
                                If Index = MyIndex Then
                                    Call SendData(PacketID.ArrowHit & SEP_CHAR & 2 & SEP_CHAR & i & SEP_CHAR & x & SEP_CHAR & y & SEP_CHAR & BX & SEP_CHAR & BY & SEP_CHAR & END_CHAR)
                                End If
