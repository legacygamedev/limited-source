...
Public Sub Buy(shopItemIndex As Integer)
    ...
    Call SendData(PacketID.Buy & SEP_CHAR & shopNum & SEP_CHAR & shopItemIndex & SEP_CHAR & END_CHAR)

End Sub

Public Sub Buyback(ByVal item As Integer, ByVal slot As Integer, Optional ByVal amt As Integer = 1)

    Call SendData(PacketID.SellItem & SEP_CHAR & shopNum & SEP_CHAR & item & SEP_CHAR & slot & SEP_CHAR & amt & SEP_CHAR & END_CHAR)
...
Public Sub FixItem(ByVal item As Integer)
    ...
    Call SendData(PacketID.FixItem & SEP_CHAR & item & SEP_CHAR & END_CHAR)
...
Private Sub lblFix_Click()
    ...
    If fixItems Then
        fixItems = False
        lblFix.Appearance = 0
        lblFix.BackColor = &HE0E0E0
     Else
        fixItems = True
        lblFix.Appearance = 1

        'Reset the sell items button if need be

        If SellItems Then
            SellItems = False
            lblSell.Appearance = 0
            lblSell.BackColor = &HE0E0E0
        End If
...
Private Sub lblSell_Click()
    ...
    If SellItems Then
     ...
     Else
        SellItems = True
        fixItems = False
        lblSell.Appearance = 1

        'Reset the fix items button if need be
    ...
    For i = 1 To 5
        ...
        If Shop(shopNum).ShopItem(page * 5 + i).ItemNum = 0 Then
         ...
         Else
            ...
            Call BltIcon(pageIndex * 5 + i - 1, Shop(shopNum).ShopItem(pageIndex * 5 + i).ItemNum)
        End If
