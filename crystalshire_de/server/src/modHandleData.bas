...
Private Sub HandleDelAccount(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If Not IsPlaying(index) Then
        If Not IsLoggedIn(index) Then
            ...
            ' Everything went ok
            Call Kill(App.Path & "\data\Accounts\" & Trim$(Name) & ".ini")
...
Dim i As Long, n As Long, damage As Long, TempIndex As Long, x As Long, y As Long, mapNum As Long, dirReq As Long
    ...
    x = GetPlayerX(index)
    y = GetPlayerY(index)
    If Map(mapNum).Tile(x, y).Type = TILE_TYPE_CHAT Then
        dirReq = Map(mapNum).Tile(x, y).data2
        If Player(index).Dir = dirReq Then
            InitChat index, mapNum, Map(mapNum).Tile(x, y).data1, True
            Exit Sub
        End If
    End If
...
Dim tradeTarget As Long, sX As Long, sY As Long, tX As Long, tY As Long
    ...
    PlayerMsg index, "System not added yet.", BrightRed
    Exit Sub
