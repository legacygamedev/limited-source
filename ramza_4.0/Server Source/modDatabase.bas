...
Dim n As Long
    ...
    For i = 1 To MAX_CHARS
        ...
        Call PutVar(FileName, "CHAR" & i, "BeltSlot", STR(Player(index).Char(i).BeltSlot))
...
Dim n As Long
    ...
    For i = 1 To MAX_CHARS
        ...
        Player(index).Char(i).BeltSlot = Val(GetVar(FileName, "CHAR" & i, "BeltSlot"))
...
Dim f As Long
    ...
    For i = 1 To MAX_MAPS
        Call SetStatus("Cargando Mapas... " & i & "/" & MAX_MAPS)
...
Dim n As Long
    ...
    For i = 1 To MAX_MAPS
        ...
        If Not FileExist(FileName) Then
            Call SetStatus("Guardando Mapas... " & i & "/" & MAX_MAPS)
            DoEvents
...
Dim f As Long, i As Long
    ...
    Call GlobalMsg(GetPlayerName(BanPlayerIndex) & " fue baneado de " & GAME_NAME & " por " & GetPlayerName(BannedByIndex) & "!", White)
    Call AddLog(GetPlayerName(BannedByIndex) & " ha baneado a " & GetPlayerName(BanPlayerIndex) & ".", ADMIN_LOG)
    Call AlertMsg(BanPlayerIndex, "Fuiste baneado por " & GetPlayerName(BannedByIndex) & "!")
End Sub
...
Dim f As Long, i As Long
    ...
    If Trim(Reason) <> "" Then
        Call GlobalMsg(GetPlayerName(BanPlayerIndex) & " fue baneado de " & GAME_NAME & " por el server! Razon(" & Reason & ")", White)
        Call AlertMsg(BanPlayerIndex, "Fuiste baneado por el servidor!  Razon(" & Reason & ")")
        Call AddLog("El servidor a baneado a " & GetPlayerName(BanPlayerIndex) & ".  Razon(" & Reason & ")", ADMIN_LOG)
    Else
        Call GlobalMsg(GetPlayerName(BanPlayerIndex) & " fue baneado de " & GAME_NAME & " por el servidor!", White)
        Call AlertMsg(BanPlayerIndex, "Fuiste baneado por el servidor!")
        Call AddLog("El servidor a baneado a " & GetPlayerName(BanPlayerIndex) & ".", ADMIN_LOG)
    End If
End Sub
...
Dim i As Long
    ...
    For i = 1 To MAX_ARROWS
        Call SetStatus("Cargando Flechas... " & i & "/" & MAX_ARROWS)
...
Sub CheckArrows()
    If Not FileExist("Arrows.ini") Then
        ...
        For i = 1 To MAX_ARROWS
            Call SetStatus("Guardando Flechas... " & i & "/" & MAX_ARROWS)
            DoEvents
