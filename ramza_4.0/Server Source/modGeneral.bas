...
Dim f As Long
    ...
    'Scripting
    If Scripting = 1 Then
        Call SetStatus("Cargando scripts...")
    ...
    For i = 1 To MAX_PLAYERS
        Call SetStatus("Iniciando jugadores...")
        Call ClearPlayer(i)
    ...
    If Not FileExist("CMessages.ini") Then
        For i = 1 To 6
            PutVar App.Path & "\CMessages.ini", "MESSAGES", "Title" & i, "CustomMsg"
    ...
    frmServer.lstTopics.AddItem "Empezando"
    frmServer.lstTopics.AddItem "Editinando tu servidor"
    frmServer.lstTopics.AddItem "Accesos"
    frmServer.lstTopics.AddItem "Crear un servidor"
    frmServer.lstTopics.AddItem "Controles"
    frmServer.lstTopics.AddItem "Comandos"
    frmServer.lstTopics.AddItem "Charlando"
    frmServer.lstTopics.AddItem "Bugs/Errores"
    frmServer.lstTopics.AddItem "Convertir Mapas"
    frmServer.lstTopics.AddItem "Editar Mapa"
    frmServer.lstTopics.AddItem "Comandos de Scripting"
    frmServer.lstTopics.AddItem "Preguntas?"
    frmServer.lstTopics.AddItem "Novedades"
    ...
    Call ClearMaps
    Call SetStatus("Limpiando Items del Mapa...")
    Call ClearMapItems
    Call SetStatus("Limpiando Npcs del Mapa...")
    Call ClearMapNpcs
    Call SetStatus("Limpiando atributos de Npc...")
    Call ClearMapAttributeNpcs
    Call SetStatus("Limpiando Npcs...")
    Call ClearNpcs
    Call SetStatus("Limpiando Items...")
    Call ClearItems
    ...
    Call ClearExps
    Call SetStatus("Limpiando Emoticones...")
    Call ClearEmos
    Call SetStatus("Cargando Emoticones...")
    Call LoadEmos
    ...
    Call LoadSpells
    Call SetStatus("Cargando Items de Mapas...")
    Call SpawnAllMapsItems
    Call SetStatus("Cargando Npcs de Mapas...")
    Call SpawnAllMapNpcs
    Call SetStatus("Cargando Atributos de Npcs...")
...
Dim i As Long
    ...
    Call SetStatus("Apagando...")
    frmLoad.Visible = True
    frmServer.Visible = False
    DoEvents
    
    Call SetStatus("Guardando Jugadores Online...")
    Call SaveAllPlayersOnline
    Call SetStatus("Limpiando Mapas...")
    Call ClearMaps
    Call SetStatus("Limpinado Items de Mapa...")
    Call ClearMapItems
    Call SetStatus("Limpiando Npcs de Mapa...")
    Call ClearMapNpcs
    Call SetStatus("Limpiando atributos de Npcs...")
    Call ClearMapAttributeNpcs
    Call SetStatus("Limpiando Npcs...")
    Call ClearNpcs
    Call SetStatus("Limpiando Items...")
    ...
    Call SetStatus("Limpiando Tiendas...")
    Call ClearShops
    Call SetStatus("Descargando sockets y timers...")
    For i = 1 To MAX_PLAYERS
        Call SetStatus("Descargando sockets y timers... " & i & "/" & MAX_PLAYERS)
        DoEvents
                                    ...
                                    If DistanceX <= n And DistanceY <= n Then
                                        If Npc(NpcNum).Behavior = NPC_BEHAVIOR_ATTACKONSIGHT Or GetPlayerPK(i) = YES Then
                                            If Trim(Npc(NpcNum).AttackSay) <> "" Then
                                                Call PlayerMsg(i, "Un/a " & Trim(Npc(NpcNum).Name) & " : " & Trim(Npc(NpcNum).AttackSay) & "", SayColor)
                                            End If
                    ...
                    If Target > 0 Then
                        ...
                        If IsPlaying(Target) And GetPlayerMap(Target) = y Then
                            ...
                            If CanNpcAttackPlayer(x, Target) Then
                                ...
                                Call BattleMsg(Target, "Esquivaste el golpe de " & Trim(Npc(NpcNum).Name) & "!", Yellow, 1)
                                End If
                                ...
                                End If
                                    ...
                                    Else
                                        Call BattleMsg(Target, "El " & Trim(Npc(NpcNum).Name) & " no pudo lastimarte!", BrightBlue, 1)
                                        
                                        'Call PlayerMsg(Target, "El " & Trim(Npc(NpcNum).Name) & " ni siquiera pudo rozarte!", BrightBlue)
                                    End If
                                Else
                                    Call BattleMsg(Target, "Bloqueaste el golpe de " & Trim(Npc(NpcNum).Name) & "!", Yellow, 1)
                                    
                                    'Call PlayerMsg(Target, "Tu " & Trim(Item(GetPlayerInvItemNum(Target, GetPlayerShieldSlot(Target))).Name) & " bloqueo el golpe de " & Trim(Npc(NpcNum).Name) & "!", BrightCyan)
                                End If
                            End If
                        Else
