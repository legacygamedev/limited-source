...
Begin VB.Form frmMirage 
   ...
   Begin VB.PictureBox picOptions 
      ...
      Begin VB.CheckBox chknpcdamage 
         BackColor       =   &H00789298&
         Caption         =   "Daño sobre las cabezas"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   6.75
            Charset         =   0
   ...
   Begin VB.PictureBox picPlayerSpells 
      ...
      Begin VB.ListBox lstSpells 
         ...
         ForeColor       =   &H00000000&
         Height          =   2190
         ItemData        =   "frmMirage.frx":A424
         Left            =   45
         List            =   "frmMirage.frx":A426
         TabIndex        =   5
         Top             =   60
         Width           =   2535
      End
   ...
   Begin VB.PictureBox picWhosOnline 
      ...
      Begin VB.ListBox lstOnline 
         ...
         ForeColor       =   &H00000000&
         Height          =   2340
         ItemData        =   "frmMirage.frx":A428
         Left            =   80
         List            =   "frmMirage.frx":A42A
         TabIndex        =   25
         Top             =   75
         Width           =   2460
      End
...
Private Sub lstSpells_DblClick()
    If Player(MyIndex).Spell(lstSpells.ListIndex + 1) > 0 Then
        ...
        Call AddText("El hechizo fue memorizado!", White)
    Else
        Call AddText("No hay hechizo para memorizar.", BrightRed)
    End If
End Sub
    ...
    If Player(MyIndex).Inv(d + 1).Num > 0 Then
        ...
        Else
            If GetPlayerWeaponSlot(MyIndex) = d + 1 Then
                descName.Caption = Trim(Item(GetPlayerInvItemNum(MyIndex, d + 1)).Name) & " (equip)"
            ...
            End If
        End If
        descStr.Caption = "Fuer " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).StrReq
        descDef.Caption = "Def " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).DefReq
        descSpeed.Caption = "Vel " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).SpeedReq
        descMagi.Caption = "Mag " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).MagiReq
        descLvl.Caption = "Nivel " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).LvlReq
        ...
        descSD.Caption = "Fuer: " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).AddStr & " Def: " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).AddDef
        descMS.Caption = "Vel: " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).AddSpeed & " Mag: " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).AddMagi
        descDUR.Caption = "Durabilidad: " & GetPlayerInvItemDur(MyIndex, d + 1) & " / " & Item(GetPlayerInvItemNum(MyIndex, d + 1)).Data1
...
Dim ii As Long
    ...
    End If
        ...
        Call SendUseItem(i)
        Call AddText("Usaste una pocion curativa!", Yellow)
        Exit Sub
        End If
        End If
        Next i
        Call AddText("No tienes mas pociones!", Red)
        End If
        ...
        Call SendUseItem(i)
        Call AddText("Usaste una pocion de mana!", Yellow)
        Exit Sub
        End If
        End If
        Next i
    ...
    If KeyCode = vbKeyInsert Then
        If SpellMemorized > 0 Then
            If GetTickCount > Player(MyIndex).AttackTimer + 100 Then
                ...
                Else
                    Call AddText("No puedes lanzar mientras caminas!", BrightRed)
                End If
            End If
        Else
            Call AddText("No hay hechizo memorizado.", BrightRed)
        End If
    Else
        Call CheckInput(0, KeyCode, Shift)
    End If
...
Dim i As Long
 ...
 '   lblLines.Caption = "Cantidad de lineas de texto en pantalla: " & scrlBltText.Value
End Sub
...
If Inventory <= 0 Then Exit Sub
    ...
    If GetPlayerInvItemNum(MyIndex, InvNum) > 0 And GetPlayerInvItemNum(MyIndex, InvNum) <= MAX_ITEMS Then
        If Item(GetPlayerInvItemNum(MyIndex, InvNum)).Type = ITEM_TYPE_CURRENCY Then
            GoldAmount = InputBox("Cuantos/as " & Trim(Item(GetPlayerInvItemNum(MyIndex, InvNum)).Name) & "(" & GetPlayerInvItemValue(MyIndex, InvNum) & ") deseas soltar?", "Solto " & Trim(Item(GetPlayerInvItemNum(MyIndex, InvNum)).Name), 0, frmMirage.Left, frmMirage.Top)
...
Private Sub lblCast_Click()
    If Player(MyIndex).Spell(lstSpells.ListIndex + 1) > 0 Then
        ...
        End If
    Else
        Call AddText("No hay un hechizo aqui.", BrightRed)
    End If
End Sub
