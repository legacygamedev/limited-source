...
Dim Ending As String
    ...
    Call SetStatus("Chequeando Carpetas...")
    DoEvents
 ...
 '       frmMirage.chkbubblebar.Value = ReadINI("CONFIG", "SpeechBubbles", FileName)
  ...
  '      frmMirage.chkAutoScroll.Value = ReadINI("CONFIG", "AutoScroll", FileName)
        ...
        End If
    Else
        WriteINI "UPDATER", "FileName", "RamzaEngine.exe", App.Path & "\config.ini"
   ...
   '     WriteINI "CONFIG", "SpeechBubbles", 0, App.Path & "\config.ini"
 ...
 '   frmMirage.chkAutoScroll.BackColor = RGB(R1, G1, B1)
    ...
    Call SetStatus("Chequeadno Estado...")
    DoEvents
    ...
    Call SetStatus("Iniciando configuraciones TCP...")
    DoEvents
...
Sub MenuState(ByVal State As Long)
    ...
    Select Case State
        Case MENU_STATE_NEWACCOUNT
            ...
            If ConnectToServer = True Then
                Call SetStatus("Conectado, enviando informacion de nueva cuenta...")
        ...
        Case MENU_STATE_DELACCOUNT
            ...
            If ConnectToServer = True Then
                Call SetStatus("Conectado, enviando pedido de borrado de cuenta ...")
        ...
        Case MENU_STATE_LOGIN
            ...
            If ConnectToServer = True Then
                Call SetStatus("Conectado, enviando informacion de entrada...")
        ...
        Case MENU_STATE_NEWCHAR
            ...
            If ConnectToServer = True Then
                Call SetStatus("Conectado, obteniendo las clases...")
                Call SendGetClasses
            End If
        ...
        Case MENU_STATE_ADDCHAR
            ...
            If ConnectToServer = True Then
                Call SetStatus("Conectado, enviando informacion de nuevo personaje...")
        ...
        Case MENU_STATE_DELCHAR
            frmChars.Visible = False
            If ConnectToServer = True Then
                Call SetStatus("Conectado, enviando pedido de borrado de personaje...")
        ...
        Case MENU_STATE_USECHAR
            frmChars.Visible = False
            If ConnectToServer = True Then
                Call SetStatus("Conectado, enviando datos de personaje...")
    ...
    If Not IsConnected And Connucted = True Then
        frmMainMenu.Visible = True
        frmSendGetData.Visible = False
        Call MsgBox("El servidor parece estar caido. Trata de conectarte en unos minutos o visita " & WEBSITE, vbOKOnly, GAME_NAME)
    End If
End Sub
Sub GameInit()
...
Dim rec_back As RECT
                ...
                For y = 0 To MAX_MAPY
                    For x = 0 To MAX_MAPX
                        With Map(GetPlayerMap(MyIndex)).Tile(x, y)
                            ...
                            If .Type = TILE_TYPE_ARROWPASS Then Call DrawText(TexthDC, x * PIC_X + sx + 8 - (NewPlayerX * PIC_X) - NewXOffset, y * PIC_Y + sx + 8 - (NewPlayerY * PIC_Y) - NewYOffset, "AP", QBColor(BrightRed))
    ...
    End If
        ...
        If GettingMap = True Then
            Call DrawText(TexthDC, 36, 36, "Reciviendo mapa...", QBColor(BrightCyan))
        End If
    ...
    frmSendGetData.Visible = True
    Call SetStatus("Destruyendo datos del juego...")
    ...
    If IsConnected = False Then
        Call MsgBox("Gracias por jugar " & GAME_NAME & "!", vbOKOnly, GAME_NAME)
    End If
End Sub
    ...
    frmMirage.txtMyTextBox.Text = ""
        If Player(MyIndex).y - 1 > -1 Then
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_SIGN And Player(MyIndex).Dir = DIR_UP Then
                Call AddText("La señal dice:", Blue)
        ...
        If Mid(MyText, 1, 1) = "<" Then
            ...
            Else
                Call AddText("Uso: <nombredeljugadir> mensaje", AlertColor)
            End If
            MyText = ""
            Exit Sub
        End If
        ...
        ' Whos Online
        If LCase(Mid(MyText, 1, 7)) = "/online" Then
            Call SendWhosOnline
            MyText = ""
            Exit Sub
        End If
        ...
        ' Request stats
        If LCase(Mid(MyText, 1, 7)) = "/estado" Then
        ...
        ' Refresh Player
        If LCase(Mid(MyText, 1, 11)) = "/actualizar" Then
        ...
        ' Accept Chat
        If LCase(Mid(MyText, 1, 7)) = "/charla" Then
            Call SendData("achat" & SEP_CHAR & END_CHAR)
            MyText = ""
            Exit Sub
        End If
        
        If LCase(Mid(MyText, 1, 9)) = "/comercio" Then
            ' Make sure they are actually sending something
            If Len(MyText) > 10 Then
                ChatText = Mid(MyText, 11, Len(MyText) - 10)
                Call SendTradeRequest(ChatText)
            Else
                Call AddText("Usage: /comercio nombredeljugador", AlertColor)
            End If
            MyText = ""
            Exit Sub
        End If
        ...
        ' Decline Trade
        If LCase(Mid(MyText, 1, 9)) = "/declinar" Then
        ...
        If LCase(Mid(MyText, 1, 6)) = "/party" Then
            ...
            Else
                Call AddText("Usage: /party nombredeljugador", AlertColor)
            End If
            MyText = ""
            Exit Sub
        End If
        ...
        ' Join party
        If LCase(Mid(MyText, 1, 7)) = "/unirse" Then
            Call SendJoinParty
            MyText = ""
            Exit Sub
        End If
        ...
        ' Leave party
        If LCase(Mid(MyText, 1, 6)) = "/dejar" Then
            Call SendLeaveParty
            MyText = ""
            Exit Sub
        End If
        ...
        If GetPlayerAccess(MyIndex) > 0 Then
            ...
            ' weather command
            If LCase(Mid(MyText, 1, 6)) = "/clima" Then
                If Len(MyText) > 6 Then
                    ...
                    Else
                        If Trim(LCase(MyText)) = "nada" Then i = 0
                        If Trim(LCase(MyText)) = "lluvia" Then i = 1
                        If Trim(LCase(MyText)) = "nieve" Then i = 2
                        If Trim(LCase(MyText)) = "truenos" Then i = 3
            ...
            ' Kicking a player
            If LCase(Mid(MyText, 1, 7)) = "/hechar" Then
        ...
        If GetPlayerAccess(MyIndex) >= ADMIN_MAPPER Then
            ...
            ' Map Editor
            If LCase(Mid(MyText, 1, 11)) = "/editarmapa" Then
            ...
            ' Map report
            If LCase(Mid(MyText, 1, 6)) = "/mapas" Then
            ...
            ' Setting sprite
            If LCase(Mid(MyText, 1, 7)) = "/sprite" Then
                If Len(MyText) > 8 Then
                    ' Get sprite #
                    MyText = Mid(MyText, 9, Len(MyText) - 8)
            ...
            If LCase(Mid(MyText, 1, 14)) = "/jugadorsprite" Then
                If Len(MyText) > 17 Then
                    i = Val(Mid(MyText, 15, 1))
                
                    MyText = Mid(MyText, 16, Len(MyText) - 15)
            ...
            ' Respawn request
            If Mid(MyText, 1, 10) = "/reiniciar" Then
                Call SendMapRespawn
                MyText = ""
                Exit Sub
            End If
        ...
        If GetPlayerAccess(MyIndex) >= ADMIN_DEVELOPER Then
            ...
            If Mid(MyText, 1, 11) = "/editaritem" Then
            ...
            If Mid(MyText, 1, 15) = "/editaremoticon" Then
            ...
            'If Mid(MyText, 1, 14) = "/editarhechizo" Then
        ...
        If GetPlayerAccess(MyIndex) >= ADMIN_CREATOR Then
            ...
            If LCase(Mid(MyText, 1, 7)) = "/acceso" Then
                ' Get access #
                i = Val(Mid(MyText, 9, 1))
            ...
            ' Ban destroy
            If LCase(Mid(MyText, 1, 13)) = "/destruirlban" Then
                Call SendBanDestroy
                MyText = ""
                Exit Sub
            End If
...
Dim x As Long, y As Long
    ...
    If DirUp Then
        ...
        If GetPlayerY(MyIndex) > 0 Then
            ...
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_BLOCKED Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_ARROWPASS Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex), GetPlayerY(MyIndex) - 1).Type = TILE_TYPE_SIGN Then
                CanMove = False
    ...
    If DirLeft Then
        ...
        If GetPlayerX(MyIndex) > 0 Then
            ' Check to see if the map tile is blocked or not
            If Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_BLOCKED Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_ARROWPASS Or Map(GetPlayerMap(MyIndex)).Tile(GetPlayerX(MyIndex) - 1, GetPlayerY(MyIndex)).Type = TILE_TYPE_SIGN Then
                CanMove = False
    ...
    If GettingMap = False Then
        If IsTryingToMove Then
            If CanMove Then
                ...
                If ShiftDown Or toRun And GetPlayerSP(MyIndex) > 0 Then
        ...
        If frmMapEditor.MousePointer = 2 Then
            ...
            ElseIf frmMapEditor.mnuType(2).Checked = True Then
                With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
                    ...
                    If .Type = TILE_TYPE_ARROWPASS Then frmAttributes.optArrowPass.Value = True
                ...
                If frmMapEditor.shpSelected.Height <= PIC_Y And frmMapEditor.shpSelected.Width <= PIC_X Then
                    ...
                    ElseIf frmMapEditor.mnuType(2).Checked = True Then
                        With Map(GetPlayerMap(MyIndex)).Tile(x1, y1)
                            ...
                            If frmAttributes.optArrowPass.Value = True Then .Type = TILE_TYPE_ARROWPASS
...
Dim YesNo As Long, x As Long, y As Long
    ...
    ' Ground layer
    If frmAttributes.optGround.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de suelo?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
    ...
    ' Mask layer
    If frmAttributes.optMask.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de mascara?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
    ...
    If frmAttributes.optAnim.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de animacion?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
    ...
    ' Mask 2 layer
    If frmAttributes.optMask2.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de mascara 2?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
    ...
    If frmAttributes.optM2Anim.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de animacion de mascara 2?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
    ...
    ' Fringe layer
    If frmAttributes.optFringe.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de margen?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
    ...
    If frmAttributes.optFAnim.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de animacion de margen?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
    ...
    ' Fringe 2 layer
    If frmAttributes.optFringe2.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de margen 2?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
    ...
    If frmAttributes.optF2Anim.Value = True Then
    YesNo = MsgBox("Estas seguro que deseas limpiar la capa de animacion de margen 2?", vbYesNo, GAME_NAME)
        If YesNo = vbYes Then
            For y = 0 To MAX_MAPY
                For x = 0 To MAX_MAPX
...
Dim YesNo As Long, x As Long, y As Long
    ...
    YesNo = MsgBox("Estas seguro que deseas limpiar los atributos del mapa?", vbYesNo, GAME_NAME)
...
Dim i As Long
    ...
    If (frmItemEditor.cmbType.ListIndex >= ITEM_TYPE_WEAPON) And (frmItemEditor.cmbType.ListIndex <= ITEM_TYPE_SHIELD) Or (frmItemEditor.cmbType.ListIndex >= ITEM_TYPE_GLOVES) And (frmItemEditor.cmbType.ListIndex <= ITEM_TYPE_AMULET) Then
        ...
        frmItemEditor.ScrlFireSTR.Value = Item(EditorIndex).FireSTR
        frmItemEditor.ScrlFireDEF.Value = Item(EditorIndex).FireDEF
        frmItemEditor.ScrlWaterSTR.Value = Item(EditorIndex).WaterSTR
        frmItemEditor.ScrlWaterDEF.Value = Item(EditorIndex).WaterDEF
        frmItemEditor.ScrlEarthSTR.Value = Item(EditorIndex).EarthSTR
        frmItemEditor.ScrlEarthDEF.Value = Item(EditorIndex).EarthDEF
        frmItemEditor.ScrlAirSTR.Value = Item(EditorIndex).AirSTR
        frmItemEditor.ScrlAirDEF.Value = Item(EditorIndex).AirDEF
        frmItemEditor.ScrlHeatSTR.Value = Item(EditorIndex).HeatSTR
        frmItemEditor.ScrlHeatDEF.Value = Item(EditorIndex).HeatDEF
        frmItemEditor.ScrlColdSTR.Value = Item(EditorIndex).ColdSTR
        frmItemEditor.ScrlColdDEF.Value = Item(EditorIndex).ColdDEF
        frmItemEditor.ScrlLightSTR.Value = Item(EditorIndex).LightSTR
        frmItemEditor.ScrlLightDEF.Value = Item(EditorIndex).LightDEF
        frmItemEditor.ScrlDarkSTR.Value = Item(EditorIndex).DarkSTR
        frmItemEditor.ScrlDarkDEF.Value = Item(EditorIndex).DarkDEF
        ...
        frmItemEditor.ScrlLvlReq.Value = Item(EditorIndex).LvlReq
...
Public Sub ItemEditorOk()
    ...
    If (frmItemEditor.cmbType.ListIndex >= ITEM_TYPE_WEAPON) And (frmItemEditor.cmbType.ListIndex <= ITEM_TYPE_SHIELD) Or (frmItemEditor.cmbType.ListIndex >= ITEM_TYPE_GLOVES) And (frmItemEditor.cmbType.ListIndex <= ITEM_TYPE_AMULET) Then
        ...
        Item(EditorIndex).LvlReq = frmItemEditor.ScrlLvlReq.Value
        Item(EditorIndex).ClassReq = frmItemEditor.scrlClassReq.Value
        Item(EditorIndex).AccessReq = frmItemEditor.scrlAccessReq.Value
        
        Item(EditorIndex).FireSTR = frmItemEditor.ScrlFireSTR.Value
        Item(EditorIndex).FireDEF = frmItemEditor.ScrlFireDEF.Value
        Item(EditorIndex).WaterSTR = frmItemEditor.ScrlWaterSTR.Value
        Item(EditorIndex).WaterDEF = frmItemEditor.ScrlWaterDEF.Value
        Item(EditorIndex).EarthSTR = frmItemEditor.ScrlEarthSTR.Value
        Item(EditorIndex).EarthDEF = frmItemEditor.ScrlEarthDEF.Value
        Item(EditorIndex).AirSTR = frmItemEditor.ScrlAirSTR.Value
        Item(EditorIndex).AirDEF = frmItemEditor.ScrlAirDEF.Value
        Item(EditorIndex).HeatSTR = frmItemEditor.ScrlHeatSTR.Value
        Item(EditorIndex).HeatDEF = frmItemEditor.ScrlHeatDEF.Value
        Item(EditorIndex).ColdSTR = frmItemEditor.ScrlColdSTR.Value
        Item(EditorIndex).ColdDEF = frmItemEditor.ScrlColdDEF.Value
        Item(EditorIndex).LightSTR = frmItemEditor.ScrlLightSTR.Value
        Item(EditorIndex).LightDEF = frmItemEditor.ScrlLightDEF.Value
        Item(EditorIndex).DarkSTR = frmItemEditor.ScrlDarkSTR.Value
        Item(EditorIndex).DarkDEF = frmItemEditor.ScrlDarkDEF.Value
    ...
    If (frmItemEditor.cmbType.ListIndex >= ITEM_TYPE_POTIONADDHP) And (frmItemEditor.cmbType.ListIndex <= ITEM_TYPE_POTIONSUBSP) Then
        ...
        Item(EditorIndex).LvlReq = 0
        Item(EditorIndex).ClassReq = -1
        Item(EditorIndex).AccessReq = 0
        
        Item(EditorIndex).FireSTR = 0
        Item(EditorIndex).FireDEF = 0
        Item(EditorIndex).WaterSTR = 0
        Item(EditorIndex).WaterDEF = 0
        Item(EditorIndex).EarthSTR = 0
        Item(EditorIndex).EarthDEF = 0
        Item(EditorIndex).AirSTR = 0
        Item(EditorIndex).AirDEF = 0
        Item(EditorIndex).HeatSTR = 0
        Item(EditorIndex).HeatDEF = 0
        Item(EditorIndex).ColdSTR = 0
        Item(EditorIndex).ColdDEF = 0
        Item(EditorIndex).LightSTR = 0
        Item(EditorIndex).LightDEF = 0
        Item(EditorIndex).DarkSTR = 0
        Item(EditorIndex).DarkDEF = 0
...
Public Sub NpcEditorInit()
    ...
    frmNpcEditor.ScrlFireSTR.Value = Npc(EditorIndex).FireSTR
    frmNpcEditor.ScrlFireDEF.Value = Npc(EditorIndex).FireDEF
    frmNpcEditor.ScrlWaterSTR.Value = Npc(EditorIndex).WaterSTR
    frmNpcEditor.ScrlWaterDEF.Value = Npc(EditorIndex).WaterDEF
    frmNpcEditor.ScrlEarthSTR.Value = Npc(EditorIndex).EarthSTR
    frmNpcEditor.ScrlEarthDEF.Value = Npc(EditorIndex).EarthDEF
    frmNpcEditor.ScrlAirSTR.Value = Npc(EditorIndex).AirSTR
    frmNpcEditor.ScrlAirDEF.Value = Npc(EditorIndex).AirDEF
    frmNpcEditor.ScrlHeatSTR.Value = Npc(EditorIndex).HeatSTR
    frmNpcEditor.ScrlHeatDEF.Value = Npc(EditorIndex).HeatDEF
    frmNpcEditor.ScrlColdSTR.Value = Npc(EditorIndex).ColdSTR
    frmNpcEditor.ScrlColdDEF.Value = Npc(EditorIndex).ColdDEF
    frmNpcEditor.ScrlLightSTR.Value = Npc(EditorIndex).LightSTR
    frmNpcEditor.ScrlLightDEF.Value = Npc(EditorIndex).LightDEF
    frmNpcEditor.ScrlDarkSTR.Value = Npc(EditorIndex).DarkSTR
    frmNpcEditor.ScrlDarkDEF.Value = Npc(EditorIndex).DarkDEF
...
Public Sub NpcEditorOk()
    ...
    Npc(EditorIndex).FireSTR = frmNpcEditor.ScrlFireSTR.Value
    Npc(EditorIndex).FireDEF = frmNpcEditor.ScrlFireDEF.Value
    Npc(EditorIndex).WaterSTR = frmNpcEditor.ScrlWaterSTR.Value
    Npc(EditorIndex).WaterDEF = frmNpcEditor.ScrlWaterDEF.Value
    Npc(EditorIndex).EarthSTR = frmNpcEditor.ScrlEarthSTR.Value
    Npc(EditorIndex).EarthDEF = frmNpcEditor.ScrlEarthDEF.Value
    Npc(EditorIndex).AirSTR = frmNpcEditor.ScrlAirSTR.Value
    Npc(EditorIndex).AirDEF = frmNpcEditor.ScrlAirDEF.Value
    Npc(EditorIndex).HeatSTR = frmNpcEditor.ScrlHeatSTR.Value
    Npc(EditorIndex).HeatDEF = frmNpcEditor.ScrlHeatDEF.Value
    Npc(EditorIndex).ColdSTR = frmNpcEditor.ScrlColdSTR.Value
    Npc(EditorIndex).ColdDEF = frmNpcEditor.ScrlColdDEF.Value
    Npc(EditorIndex).LightSTR = frmNpcEditor.ScrlLightSTR.Value
    Npc(EditorIndex).LightDEF = frmNpcEditor.ScrlLightDEF.Value
    Npc(EditorIndex).DarkSTR = frmNpcEditor.ScrlDarkSTR.Value
    Npc(EditorIndex).DarkDEF = frmNpcEditor.ScrlDarkDEF.Value
...
Dim i As Long, GetItem As Long, GetValue As Long, GiveItem As Long, GiveValue As Long, C As Long
    ...
    For C = 1 To 6
        For i = 1 To MAX_TRADES
            ...
            Else
                frmShopEditor.lstTradeItem(C - 1).AddItem "Casilla de comercio Vacia"
            End If
        Next i
    Next C
...
Dim i As Long
    ...
    frmSpellEditor.ScrlFireSTR.Value = Spell(EditorIndex).FireSTR
    frmSpellEditor.ScrlWaterSTR.Value = Spell(EditorIndex).WaterSTR
    frmSpellEditor.ScrlEarthSTR.Value = Spell(EditorIndex).EarthSTR
    frmSpellEditor.ScrlAirSTR.Value = Spell(EditorIndex).AirSTR
    frmSpellEditor.ScrlHeatSTR.Value = Spell(EditorIndex).HeatSTR
    frmSpellEditor.ScrlColdSTR.Value = Spell(EditorIndex).ColdSTR
    frmSpellEditor.ScrlLightSTR.Value = Spell(EditorIndex).LightSTR
    frmSpellEditor.ScrlDarkSTR.Value = Spell(EditorIndex).DarkSTR
...
Public Sub SpellEditorOk()
    ...
    Spell(EditorIndex).FireSTR = frmSpellEditor.ScrlFireSTR.Value
    Spell(EditorIndex).WaterSTR = frmSpellEditor.ScrlWaterSTR.Value
    Spell(EditorIndex).EarthSTR = frmSpellEditor.ScrlEarthSTR.Value
    Spell(EditorIndex).AirSTR = frmSpellEditor.ScrlAirSTR.Value
    Spell(EditorIndex).HeatSTR = frmSpellEditor.ScrlHeatSTR.Value
    Spell(EditorIndex).ColdSTR = frmSpellEditor.ScrlColdSTR.Value
    Spell(EditorIndex).LightSTR = frmSpellEditor.ScrlLightSTR.Value
    Spell(EditorIndex).DarkSTR = frmSpellEditor.ScrlDarkSTR.Value
    ...
    If GetPlayerInvItemNum(MyIndex, i) > 0 And GetPlayerInvItemNum(MyIndex, i) <= MAX_ITEMS Then
        ...
        Else
            If GetPlayerWeaponSlot(MyIndex) = i Or GetPlayerArmorSlot(MyIndex) = i Or GetPlayerHelmetSlot(MyIndex) = i Or GetPlayerShieldSlot(MyIndex) = i Or GetPlayerGlovesSlot(MyIndex) = i Or GetPlayerBootsSlot(MyIndex) = i Or GetPlayerBeltSlot(MyIndex) = i Or GetPlayerLegsSlot(MyIndex) = i Or GetPlayerRingSlot(MyIndex) = i Or GetPlayerAmuletSlot(MyIndex) = i Then
                frmPlayerTrade.PlayerInv1.AddItem i & ": " & Trim(Item(GetPlayerInvItemNum(MyIndex, i)).Name) & " (equip)"
            Else
...
If MapNpc(Index).Num < 1 Then Exit Sub
    ...
    If Npc(MapNpc(Index).Num).Big = 1 Then
        ...
        Call DD_BackBuffer.DrawBox(x, y + 2, x + 32, y - 2)
        Call DD_BackBuffer.SetFillColor(RGB(0, 255, 0))
        Call DD_BackBuffer.DrawBox(x, y + 2, x + ((MapNpc(Index).HP / 100) / (MapNpc(Index).MaxHp / 100) * 50), y - 2)
    Else
        ...
        Call DD_BackBuffer.DrawBox(x, y + 2, x + 32, y - 2)
        Call DD_BackBuffer.SetFillColor(RGB(0, 255, 0))
        Call DD_BackBuffer.DrawBox(x, y + 2, x + ((MapNpc(Index).HP / 100) / (MapNpc(Index).MaxHp / 100) * 32), y - 2)
    End If
End Sub
...
Dim d As Long
    ...
    For Index = 1 To MAX_INV
        ...
        If GetPlayerBeltSlot(MyIndex) <> Index Then frmMirage.BeltImage.Picture = LoadPicture()
    ...
    For Index = 1 To MAX_INV
        ...
        If GetPlayerBeltSlot(MyIndex) = Index Then Call BitBlt(frmMirage.BeltImage.hDC, 0, 0, PIC_X, PIC_Y, frmMirage.picItems.hDC, (Item(GetPlayerInvItemNum(MyIndex, Index)).Pic - Int(Item(GetPlayerInvItemNum(MyIndex, Index)).Pic / 6) * 6) * PIC_X, Int(Item(GetPlayerInvItemNum(MyIndex, Index)).Pic / 6) * PIC_Y, SRCCOPY)
        ...
        If Player(MyIndex).Inv(d + 1).Num > 0 Then
            ...
            'Else
                If GetPlayerWeaponSlot(MyIndex) = d + 1 Then
                    'frmMirage.picInv(d).ToolTipText = Trim(Item(GetPlayerInvItemNum(MyIndex, d + 1)).Name) & " (equip)"
...
index2 = Trade(Selected).Items(Index).ItemGetNum
    ...
    frmTrade.descMagi.Caption = Item(index2).MagiReq
    frmTrade.descLvl.Caption = Item(index2).LvlReq
...
For i = 1 To MAX_INV
   If GetPlayerInvItemNum(MyIndex, i) > 0 Then
       ...
       Else
    '       If GetPlayerWeaponSlot(MyIndex) = i Or GetPlayerArmorSlot(MyIndex) = i Or GetPlayerHelmetSlot(MyIndex) = i Or GetPlayerShieldSlot(MyIndex) = i Or GetPlayerGlovesSlot(MyIndex) = i Or GetPlayerBootsSlot(MyIndex) = i Or GetPlayerBeltSlot(MyIndex) = i Or GetPlayerLegsSlot(MyIndex) = i Or GetPlayerRingSlot(MyIndex) = i Or GetPlayerAmuletSlot(MyIndex) = i Then
    '           frmBank.lstInventory.AddItem i & "> " & Trim(Item(GetPlayerInvItemNum(MyIndex, i)).Name) & " (equip)"
    '       Else
...
For i = 1 To MAX_BANK
   If GetPlayerBankItemNum(MyIndex, i) > 0 Then
       ...
       Else
    '       If GetPlayerWeaponSlot(MyIndex) = i Or GetPlayerArmorSlot(MyIndex) = i Or GetPlayerHelmetSlot(MyIndex) = i Or GetPlayerShieldSlot(MyIndex) = i Or GetPlayerGlovesSlot(MyIndex) = i Or GetPlayerBootsSlot(MyIndex) = i Or GetPlayerBeltSlot(MyIndex) = i Or GetPlayerLegsSlot(MyIndex) = i Or GetPlayerRingSlot(MyIndex) = i Or GetPlayerAmuletSlot(MyIndex) = i Then
    '           frmBank.lstBank.AddItem i & "> " & Trim(Item(GetPlayerBankItemNum(MyIndex, i)).Name) & " (equip)"
    '       Else
