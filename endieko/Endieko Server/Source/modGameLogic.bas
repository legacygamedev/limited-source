        ...
        If GetPlayerInvItemNum(Index, i) = ItemNum Then
            ...
            Else
                ...
                Select Case Item(GetPlayerInvItemNum(Index, i)).Type
                    ...
                    Case ITEM_TYPE_LEGS
                        If GetPlayerLegSlot(Index) > 0 Then
                            If i = GetPlayerLegSlot(Index) Then
                                Call SetPlayerLegSlot(Index, 0)
                                Call SendWornEquipment(Index)
                                TakeItem = True
                            Else
                                If ItemNum <> GetPlayerInvItemNum(Index, GetPlayerLegSlot(Index)) Then
                                    TakeItem = True
                                End If
                            End If
                        Else
                        ...
                        If GetPlayerBootSlot(Index) > 0 Then
                            If i = GetPlayerBootSlot(Index) Then
                                Call SetPLayerBootSlot(Index, 0)
                                Call SendWornEquipment(Index)
                                TakeItem = True
                            Else
                                If ItemNum <> GetPlayerInvItemNum(Index, GetPlayerBootSlot(Index)) Then
                                    TakeItem = True
                                End If
                            End If
                        Else
                ...
                If (n <> ITEM_TYPE_WEAPON) And (n <> ITEM_TYPE_ARMOR) And (n <> ITEM_TYPE_HELMET) And (n <> ITEM_TYPE_SHIELD) And (n <> ITEM_TYPE_LEGS) And (n <> ITEM_TYPE_BOOTS) Then
                    TakeItem = True
                End If
            End If
    ...
    If Damage >= GetPlayerHP(Victim) Then
        ...
        If Map(GetPlayerMap(Victim)).Moral <> MAP_MORAL_NO_PENALTY Then
            If GetPlayerWeaponSlot(Victim) > 0 Then
                If Item(GetPlayerWeaponSlot(Victim)).DropOnDeath = 0 Then
                    Call PlayerMapDropItem(Victim, GetPlayerWeaponSlot(Victim), 0)
                End If
            End If
        
            If GetPlayerArmorSlot(Victim) > 0 Then
                If Item(GetPlayerArmorSlot(Victim)).DropOnDeath = 0 Then
                    Call PlayerMapDropItem(Victim, GetPlayerArmorSlot(Victim), 0)
                End If
            End If
            
            If GetPlayerHelmetSlot(Victim) > 0 Then
                If Item(GetPlayerHelmetSlot(Victim)).DropOnDeath = 0 Then
                    Call PlayerMapDropItem(Victim, GetPlayerHelmetSlot(Victim), 0)
                End If
            End If
        
            If GetPlayerShieldSlot(Victim) > 0 Then
                If Item(GetPlayerShieldSlot(Victim)).DropOnDeath = 0 Then
                    Call PlayerMapDropItem(Victim, GetPlayerShieldSlot(Victim), 0)
                End If
            End If
            
            If GetPlayerLegSlot(Victim) > 0 Then
                If Item(GetPlayerLegSlot(Victim)).DropOnDeath = 0 Then
                    Call PlayerMapDropItem(Victim, GetPlayerLegSlot(Victim), 0)
                End If
            End If
            
            If GetPlayerBootSlot(Victim) > 0 Then
                If Item(GetPlayerBootSlot(Victim)).DropOnDeath = 0 Then
                    Call PlayerMapDropItem(Victim, GetPlayerBootSlot(Victim), 0)
                End If
            End If
...
Dim STR As Long, DEF As Long, MapNum As Long, NpcNum As Long
    ...
    If Damage >= MapNpc(MapNum, MapNpcNum).HP Then
        ...
        ' Check and Give them Aligment Points
        If Npc(NpcNum).Alignment > 0 Then
            Call SetPlayerAlignment(Attacker, GetPlayerAlignment(Attacker) + Npc(NpcNum).Alignment)
        Else
            Call SetPlayerAlignment(Attacker, GetPlayerAlignment(Attacker) - Npc(NpcNum).Alignment)
        End If
...
Dim Casted As Boolean
    ...
    If Player(Index).TargetType = TARGET_TYPE_PLAYER Then
        If IsPlaying(n) Then
            If GetPlayerHP(n) > 0 And GetPlayerMap(Index) = GetPlayerMap(n) And (Map(GetPlayerMap(Index)).Moral = MAP_MORAL_NONE Or Map(GetPlayerMap(Index)).Moral = MAP_MORAL_NO_PENALTY) And GetPlayerAccess(Index) <= 0 And GetPlayerAccess(n) <= 0 Then
                        ...
                        Select Case Spell(SpellNum).Type
                            Case SPELL_TYPE_SUBHP
                                ...
                                If Damage > 0 Then
                                    ...
                                    Call SendDataTo(Index, "BLITPLAYERDMG" & SEP_CHAR & Damage & SEP_CHAR & n & SEP_CHAR & END_CHAR)
                                Else
                            ...
                            Case SPELL_TYPE_SUBMP
                                ...
                                Call SendDataTo(Index, "BLITPLAYERDMG" & SEP_CHAR & GetPlayerMP(n) - Spell(SpellNum).Data1 & SEP_CHAR & n & SEP_CHAR & END_CHAR)
