...
Dim f As Long
    ...
    ' Check if the arrows directory is there, if its not make it
    If LCase$(Dir(App.Path & "\data\Arrows", vbDirectory)) <> "arrows" Then
        Call MkDir(App.Path & "\data\Arrows")
    End If
    
    ' Check if the effects directory is there, if its not make it
    If LCase$(Dir(App.Path & "\data\Effects", vbDirectory)) <> "effects" Then
        Call MkDir(App.Path & "\data\Effects")
    End If
    
    ' Check if the emoticons directory is there, if its not make it
    If LCase$(Dir(App.Path & "\data\Emoticons", vbDirectory)) <> "emoticons" Then
        Call MkDir(App.Path & "\data\Emoticons")
    End If
    ...
    'frmServer.Socket(0).RemoteHost = frmServer.Socket(0).LocalIP --- Pre-IOCP
    ...
    ' Check if the master guildlist file exists for checking duplicate guild names, and if it doesnt make it
    ...
    ' Start listening
    'frmServer.Socket(0).Listen --Pre-IOCP
...
'    For i = 1 To MAX_PLAYERS --- Pre-IOCP
...
Sub SetStatus(ByVal Status As String)
    ...
    frmLoad.txtStatus.Text = frmLoad.txtStatus.Text & Chr(13) & Chr(10)
    frmLoad.txtStatus.Text = frmLoad.txtStatus.Text & Status
    frmLoad.txtStatus.SelStart = Len(frmLoad.txtStatus.Text)
    'frmLoad.lblStatus.Caption = Status
End Sub

Sub ServerLogic()
'Dim i As Long - Pre-IOCP
'
