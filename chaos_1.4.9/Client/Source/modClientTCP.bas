...
On Error GoTo ErrHandler:
    ...
    If Parse(0) = "plainmsg" Then
        ...
        If n = 2 Then frmLogin.Show
        If n = 3 Then frmNewChar.Show
        If n = 4 Then frmChars.Show
    ...
    If Parse(0) = "allchars" Then
        ...
        For I = 1 To MAX_CHARS
            ...
            HelmetLogin(I) = Val(Parse(n + 4))
            LegsLogin(I) = Val(Parse(n + 5))
            ArmorLogin(I) = Val(Parse(n + 6))
            ShieldLogin(I) = Val(Parse(n + 7))
            WeaponLogin(I) = Val(Parse(n + 8))
    ...
    If Parse(0) = "b6" Then
        ...
        Player(MyIndex).Party.MemberNames(I) = "Blank Slot"
        frmParty.MemName(I).Caption = "Blank Slot"
        Player(MyIndex).Party.Level(I) = 0
        frmParty.Level(I).Caption = "?"
  ...
  End If
   ...
   Player(Val(Parse(1))).FP = Val(Parse(3))
    ...
    If (Parse(0) = "playerstatspacket") Then
        ...
        Player(MyIndex).Poisoned = Val(Parse(42))
        Player(MyIndex).Diseased = Val(Parse(43))
        Exit Sub
    End If
    ...
    If (Parse(0) = "editnpc") Then
        ...
        For I = 1 To MAX_NPC_DROPS
            ...
            picDrop(I) = Npc(n).ItemNPC(I).itemnum
...
ErrHandler:
If Parse(0) = "mapdata" Then Call SendData("REFRESH" & SEP_CHAR & END_CHAR)
End Sub
...
Dim I As Long
    ...
    Packet = "SAVENPC" & SEP_CHAR & NpcNum & SEP_CHAR & Trim(Npc(NpcNum).Name) & SEP_CHAR & Trim(Npc(NpcNum).AttackSay) & SEP_CHAR & Npc(NpcNum).Sprite & SEP_CHAR & Npc(NpcNum).SpawnSecs & SEP_CHAR & Npc(NpcNum).Behavior & SEP_CHAR & Npc(NpcNum).Range & SEP_CHAR & Npc(NpcNum).STR & SEP_CHAR & Npc(NpcNum).DEF & SEP_CHAR & Npc(NpcNum).speed & SEP_CHAR & Npc(NpcNum).MAGI & SEP_CHAR & Npc(NpcNum).Big & SEP_CHAR & Npc(NpcNum).MaxHP & SEP_CHAR & Npc(NpcNum).EXP & SEP_CHAR & Npc(NpcNum).SpawnTime & SEP_CHAR & Npc(NpcNum).Speech & SEP_CHAR & Npc(NpcNum).Element & SEP_CHAR & Npc(NpcNum).Poison & SEP_CHAR & Npc(NpcNum).AP & SEP_CHAR & Npc(NpcNum).Disease & SEP_CHAR & Npc(NpcNum).Quest & SEP_CHAR & Npc(NpcNum).NpcDIR & SEP_CHAR & Npc(NpcNum).AilmentDamage & SEP_CHAR & Npc(NpcNum).AilmentInterval & SEP_CHAR & Npc(NpcNum).AilmentMS & SEP_CHAR & Npc(NpcNum).Spell & SEP_CHAR
    For I = 1 To MAX_NPC_DROPS
