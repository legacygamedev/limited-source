...
Dim WeaponSlot As Long
    ...
    If GetPlayerWeaponSlot(Index) > 0 Then
        ...
        'If item is not unbreakable then we reduce the Dur...
        If Item(GetPlayerInvItemNum(Index, WeaponSlot)).Unbreakable = 0 Then
...
Dim ArmorSlot As Long, HelmSlot As Long
    ...
    If ArmorSlot > 0 Then
        ...
        'If item is breakable, we reduce the dur
        If Item(GetPlayerInvItemNum(Index, ArmorSlot)).Unbreakable = 0 Then
    ...
    If HelmSlot > 0 Then
        ...
        'If helmet is breakable reduce dur.
        If Item(GetPlayerInvItemNum(Index, HelmSlot)).Unbreakable = 0 Then
...
Call SetPlayers(TotalOnlinePlayers)
End Function
...
Dim i As Long
    ...
    If i <> 0 Then
        ...
        Call SendInventoryUpdate(Index, i)
    Else
        Call PlayerMsg(Index, "Your backpack is full.", BrightRed)
    End If
End Sub
...
Dim Spawned As Boolean
    ...
    If NpcNum > 0 Then
        ...
        If Spawned Then
            ...
            'Also set the xmap vars
            'xMap(MapNum).Tile(MapNpc(MapNum, MapNpcNum).X, MapNpc(MapNum, MapNpcNum).Y) = True
        End If
    End If
End Sub
...
Dim i As Long
    ...
    If Damage >= GetPlayerHP(Victim) Then
        ...
        'Set the xMap variable, should be blank.
        'xMap(GetPlayerMap(Victim)).Tile(GetPlayerX(Victim), GetPlayerY(Victim)) = False
...
Dim str As Long, DEF As Long, MapNum As Long, NpcNum As Long
    ...
    If Damage >= MapNpc(MapNum, MapNpcNum).HP Then
        ...
        If N = 1 Then
            'Give player who attacked the npc, the item if the inventory is not full... :)
            If FindOpenInvSlot(Attacker, Npc(NpcNum).DropItem) > 0 Then
                ...
                Call PlayerMsg(Attacker, "You have recieved " & Item(Npc(NpcNum).DropItem).Name & ".", BrightBlue)
            Else
        ...
        If Player(Attacker).TargetType = TARGET_TYPE_NPC And Player(Attacker).Target = MapNpcNum Then
            ...
            'Since he died, he no longer exist, so setup the xMap
            'xMap(MapNum).Tile(MapNpc(MapNum, MapNpcNum).X, MapNpc(MapNum, MapNpcNum).Y) = False
        End If
    Else
...
Dim ShopNum As Long, OldMap As Long
    ...
    'Make sure xMap is uptodate
    'xMap(OldMap).Tile(GetPlayerX(Index), GetPlayerY(Index)) = False
    
    Call SetPlayerMap(Index, MapNum)
    Call SetPlayerX(Index, X)
    Call SetPlayerY(Index, Y)
    
    'Now setup X, Y for xMap
    'xMap(MapNum).Tile(X, Y) = True
...
Sub PlayerMoveNEW(ByVal Index As Long, ByVal Dir As Long, ByVal Movement As Long)
Dim Packet As String
Dim MapNum As Long
Dim X As Long
Dim Y As Long
...
Dim Moved As Byte
    ...
    'TODO: A change direction button, should help when fighting!
    
    'debug.print "PLAYERMOVE: X: " & GetPlayerX(Index) & ", Y: " & GetPlayerY(Index)
    
    'Only change direction if needed :#
    If GetPlayerDir(Index) <> Dir Then
        Call SetPlayerDir(Index, Dir)
    End If
    ...
    Select Case Dir
        Case DIR_UP
            ' Check to make sure not outside of boundries
            If GetPlayerY(Index) > 0 Then
                'debug.print "1 PM UP: Y is greater then 0"
                'debug.print "X PM UP: Type: " & Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) - 1).Type
                'debug.print "X PM UP: Occupied: " & xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) - 1)
                ' Check to make sure that the tile is walkable
                If (Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) - 1).Type <> TILE_TYPE_BLOCKED And xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) - 1) = False) Or (xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) - 1) = True And Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) - 1).Type = TILE_TYPE_PASS) Then
                    'debug.print "2 PM UP: Is blocked and empty OR full and passable"
                    
                    ' Check to see if the tile is a key and if it is check if its opened
                    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) - 1).Type <> TILE_TYPE_KEY Or (Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) - 1).Type = TILE_TYPE_KEY And TempTile(GetPlayerMap(Index)).DoorOpen(GetPlayerX(Index), GetPlayerY(Index) - 1) = YES) Then
                        'Update xMap
                        xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)) = False
                        Call SetPlayerY(Index, GetPlayerY(Index) - 1)
                        'Update xMap
                        xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)) = True
                ...
                End If
            Else
                'TODO: Check to see if there is anybody on the tile above them in above map.. do all the checking :D
                'debug.print "3 PM UP: Y is 0, seeing if we can warp."
        ...
        Case DIR_DOWN
            ' Check to make sure not outside of boundries
            If GetPlayerY(Index) < MAX_MAPY Then
                'debug.print "1 PM DOWN: Y is greater then MAX_MAPY: " & MAX_MAPY
                'debug.print "X PM DOWN: Type: " & Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) + 1).Type
                'debug.print "X PM DOWN: Occupied: " & xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) + 1)
                
                ' Check to make sure that the tile is walkable
                If (Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) + 1).Type <> TILE_TYPE_BLOCKED And xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) + 1) = False) Or (xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) + 1) = True And Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index) + 1).Type = TILE_TYPE_PASS) Then
                    'debug.print "2 PM DOWN: Is blocked and empty OR full and passable"
            ...
            Else
                'TODO: Check to see if there is anybody on the tile above them in above map.. do all the checking :D
                'debug.print "3 PM DOWN: Y is 0, seeing if we can warp."
        ...
        Case DIR_LEFT
            ' Check to make sure not outside of boundries
            If GetPlayerX(Index) > 0 Then
                'debug.print "1 PM LEFT: X is greater then 0"
                'debug.print "X PM LEFT: Type: " & Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index)).Type
                'debug.print "X PM LEFT: Occupied: " & xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index))
                 
                ' Check to make sure that the tile is walkable
                If (Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index)).Type <> TILE_TYPE_BLOCKED And xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index)) = False) Or (xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index)) = True And Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index)).Type = TILE_TYPE_PASS) Then
                    'debug.print "2 PM LEFT: Is blocked and empty OR full and passable"
                    ' Check to see if the tile is a key and if it is check if its opened
                    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index)).Type <> TILE_TYPE_KEY Or (Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index)).Type = TILE_TYPE_KEY And TempTile(GetPlayerMap(Index)).DoorOpen(GetPlayerX(Index) - 1, GetPlayerY(Index)) = YES) Then
                        'Update xMap
                        xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)) = False
            ...
            Else
                'TODO: Check to see if there is anybody on the tile above them in above map.. do all the checking :D
                'debug.print "3 PM LEFT: X is 0, seeing if we can warp."
        ...
        Case DIR_RIGHT
            ' Check to make sure not outside of boundries
            If GetPlayerX(Index) < MAX_MAPX Then
                'debug.print "1 PM RIGHT: X is less then MAX_MAPX: " & MAX_MAPX
                'debug.print "X PM RIGHT: Type: " & Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index)).Type
                'debug.print "X PM RIGHT: Occupied: " & xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index) - 1, GetPlayerY(Index))
                
                ' Check to make sure that the tile is walkable
                If (Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) + 1, GetPlayerY(Index)).Type <> TILE_TYPE_BLOCKED And xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index) + 1, GetPlayerY(Index)) = False) Or (xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index) + 1, GetPlayerY(Index)) = True And Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) + 1, GetPlayerY(Index)).Type = TILE_TYPE_PASS) Then
                    'debug.print "2 PM RIGHT: Is blocked and empty OR full and passable"
                    ' Check to see if the tile is a key and if it is check if its opened
                    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) + 1, GetPlayerY(Index)).Type <> TILE_TYPE_KEY Or (Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index) + 1, GetPlayerY(Index)).Type = TILE_TYPE_KEY And TempTile(GetPlayerMap(Index)).DoorOpen(GetPlayerX(Index) + 1, GetPlayerY(Index)) = YES) Then
                        'Update xMap
                        xMap(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)) = False
            ...
            Else
                'TODO: Check to see if there is anybody on the tile above them in above map.. do all the checking :D
                'debug.print "3 PM RIGHT: X is MAX_MAP or greater, seeing if we can warp."
                ' Check to see if we can move them to the another map
                If Map(GetPlayerMap(Index)).Right > 0 Then
                    Call PlayerWarp(Index, Map(GetPlayerMap(Index)).Right, 0, GetPlayerY(Index))
                    Moved = YES
...
Dim EKey As String
    ...
    EKey = CreateRandomKey(10)
    Player(Index).EKey = EKey
    Call SendDataTo(Index, "LOGINOK" & SEP_CHAR & Index & SEP_CHAR & Player(Index).EKey & SEP_CHAR & END_CHAR)
