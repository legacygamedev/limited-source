...
Dim f As Long
    ...
    If LCase(Dir(App.Path & "\storage", vbDirectory)) <> "storage" Then
        Call MkDir(App.Path & "\storage")
    End If
    ...
    If GAME_NAME = "" Then GAME_NAME = "Untitled Game"
    ...
    MAX_CONDITIONS = Int(GetVar(App.Path & "\Conditions\cond.ini", "CONFIG", "max_conditions"))
    ...
    ReDim Condition(0 To MAX_CONDITIONS)
    ...
    Call LoadSpells
    Call SetStatus("Loading conditions...")
    Call LoadConditions
    ...
    'Load exp rate setting
    exp_rate = 1
    exp_rate = GetVar(App.Path & "\Data.ini", "CONFIG", "EXP_Rate")
