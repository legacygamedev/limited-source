...
Dim i As Long
    ...
    FileName = App.Path & "\conditions\cond.ini"
    
    For i = 1 To MAX_CONDITIONS
        Call SetStatus("Loading Conditions... " & i & "/" & MAX_CONDITIONS)
        Condition(i).Name = GetVar(FileName, "Condition" & i, "name")
        Condition(i).HP = GetVar(FileName, "Condition" & i, "HP")
        Condition(i).MP = GetVar(FileName, "Condition" & i, "MP")
        Condition(i).SP = GetVar(FileName, "Condition" & i, "SP")
        Condition(i).STR = GetVar(FileName, "Condition" & i, "STR")
        Condition(i).DEF = GetVar(FileName, "Condition" & i, "DEF")
        Condition(i).Speed = GetVar(FileName, "Condition" & i, "Speed")
        Condition(i).Magi = GetVar(FileName, "Condition" & i, "Magi")
        Condition(i).time = GetVar(FileName, "Condition" & i, "time")
        Condition(i).BeginMSG = GetVar(FileName, "Condition" & i, "BeginMSG")
        Condition(i).EndMSG = GetVar(FileName, "Condition" & i, "EndMSG")
...
Sub CheckConditions()
    If Not FileExist("\conditions\cond.ini") Then
        Dim i As Long
        Dim FileName As String
        ...
        For i = 1 To MAX_CONDITIONS
            Call SetStatus("Saving conditions... " & i & "/" & MAX_ARROWS)
            DoEvents
            'Call PutVar(App.Path & "\Arrows.ini", "Arrow" & I, "ArrowName", "")
            Call PutVar(FileName, "Condition" & i, "name", "")
            Call PutVar(FileName, "Condition" & i, "HP", 0)
            Call PutVar(FileName, "Condition" & i, "MP", 0)
            Call PutVar(FileName, "Condition" & i, "SP", 0)
            Call PutVar(FileName, "Condition" & i, "STR", 0)
            Call PutVar(FileName, "Condition" & i, "DEF", 0)
            Call PutVar(FileName, "Condition" & i, "Speed", 0)
            Call PutVar(FileName, "Condition" & i, "Magi", 0)
            Call PutVar(FileName, "Condition" & i, "time", 0)
            Call PutVar(FileName, "Condition" & i, "BeginMSG", "")
            Call PutVar(FileName, "Condition" & i, "EndMSG", "")
