                ...
                For d = 1 To MAX_PARTY_MEMBERS
                    If Player(Attacker).Party.Member(d) > 0 Then
                        If Player(Player(Attacker).Party.Member(d)).Party.ShareExp = True Then
                            ...
                            Else
                                Call SetPlayerExp(Player(Attacker).Party.Member(d), GetPlayerExp(Player(Attacker).Party.Member(d)) + (Exp * exp_rate))
                                Call BattleMsg(Player(Attacker).Party.Member(d), "You have gained " & (Exp * exp_rate) & " party experience.", BrightBlue, 0)
                            End If
                        End If
                    End If
                Next d
                        ...
                        If Player(Attacker).Party.Member(d) > 0 Then
                            If Player(Player(Attacker).Party.Member(d)).Party.ShareExp = True Then
                                If GetPlayerLevel(Player(Attacker).Party.Member(d)) = MAX_LEVEL Then
                                    ...
                                    Call BattleMsg(Player(Attacker).Party.Member(d), "You cant gain anymore experience!", BrightBlue, 0)
                                Else
                                    Call SetPlayerExp(Player(Attacker).Party.Member(d), GetPlayerExp(n) + (Exp * exp_rate))
                                    Call BattleMsg(Player(Attacker).Party.Member(d), "You have gained " & (Exp * exp_rate) & " party experience.", BrightBlue, 0)
                                End If
                            End If
                        End If
