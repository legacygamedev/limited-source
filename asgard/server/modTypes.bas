...
Public Const TILE_TYPE_STORAGE = 21
...
Type CondRec
    ...
    time As Long 'time in 1/10 second
...
Type PlayerRec
    ...
    ' Current buff, might be changed to a buff record sooner or later
    CurCondition As Long
    CurCondTime As Long
...
Function GetPlayerCondition(ByVal index As Long) As Long
  GetPlayerCondition = Player(index).Char(Player(index).CharNum).CurCondition
End Function

Sub SetPlayerCondition(ByVal index As Long, cond As Long)
  Player(index).Char(Player(index).CharNum).CurCondition = cond
  Player(index).Char(Player(index).CharNum).CurCondTime = Condition(cond).time
  If Condition(cond).BeginMSG <> "" Then
      Call PlayerMsg(index, Condition(cond).BeginMSG, BrightRed)
  End If
End Sub
